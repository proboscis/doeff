...
def test_run_result_exposes_dispatch_lookup_stats(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("DOEFF_VM_DISPATCH_PROFILE", "1")
    result = doeff_vm.run(_with_handlers(counter_program(), doeff_vm.state))

    assert result.is_ok()
    stats = result.dispatch_lookup_stats
    assert isinstance(stats, dict)
    assert isinstance(stats["dispatch_origin_in_segment_calls"], int)
