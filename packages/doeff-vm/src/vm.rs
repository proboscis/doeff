...
impl VM {
        ...
        match mode {
            Mode::Deliver(value) => {
                self.current_seg_mut().mode = Mode::HandleYield(DoCtrl::Apply {
                    f: Box::new(DoCtrl::Pure { value: Value::Python(mapper.into_inner()) }),
        ...
        match mode {
            Mode::Deliver(value) => {
                ...
                seg.mode = Mode::HandleYield(DoCtrl::Expand {
                    factory: Box::new(DoCtrl::Pure { value: Value::Python(binder.into_inner()) }),
                    args: vec![DoCtrl::Pure { value }],
                    kwargs: vec![],
    ...
    fn start_interceptor_invocation_mode(
        ...
        Mode::HandleYield(DoCtrl::Apply {
            f: Box::new(DoCtrl::Pure { value: Value::Python(interceptor_callable) }),
            args: vec![DoCtrl::Pure { value: Value::Python(yielded_obj) }],
            kwargs: vec![],
    ...
    fn step_handle_yield(&mut self) -> StepEvent {
        ...
        match yielded {
            ...
            DoCtrl::IRStream { stream, metadata } => {
                self.handle_yield_ir_stream(stream, metadata)
            }
            DoCtrl::Eval {
                expr,
                handlers,
    ...
    fn handle_yield_apply(
        ...
        let func = match value {
            ...
            Value::Kleisli(kleisli) => {
                let args_values = Self::collect_pure_args(args);
                let kwargs_values = Self::collect_pure_kwargs(kwargs);
                ...
                match result {
                    Ok(doctrl) => return self.evaluate(doctrl),
                    Err(vm_err) => return StepEvent::Error(vm_err),
                }
            }
            other => {
        ...
        StepEvent::NeedsPython(PythonCall::CallFunc {
            func,
            args: Self::collect_pure_args(args),
            kwargs: Self::collect_pure_kwargs(kwargs),
        })
    }
    ...
    fn handle_yield_expand(
        ...
        let (func, handler_return) = match value {
    ...
    fn doctrl_to_eval_expr(doctrl: &DoCtrl) -> PyShared {
        match doctrl {
            DoCtrl::Eval { expr, .. } => expr.clone(),
            other => {
                panic!("doctrl_to_eval_expr: expected Eval DoCtrl for unevaluated arg, got {:?}", other);
            }
        }
    }
    ...
    fn collect_pure_args(args: Vec<DoCtrl>) -> Vec<Value> {
        let mut values = Vec::with_capacity(args.len());
        for arg in args {
            match arg {
                DoCtrl::Pure { value } => values.push(value),
                _ => unreachable!("collect_pure_args: all args must be Pure"),
            }
        }
        values
    }

    fn collect_pure_kwargs(kwargs: Vec<(String, DoCtrl)>) -> Vec<(String, Value)> {
        let mut values = Vec::with_capacity(kwargs.len());
        for (key, value) in kwargs {
            match value {
                DoCtrl::Pure { value: inner } => values.push((key, inner)),
                _ => unreachable!("collect_pure_kwargs: all kwargs must be Pure"),
            }
        }
        values
    }
    ...
    fn invoke_prompt_return_clause(&mut self, return_clause: PyShared, value: Value) -> StepEvent {
        ...
        self.current_seg_mut().mode = Mode::HandleYield(DoCtrl::Apply {
            f: Box::new(DoCtrl::Pure { value: Value::Python(callable) }),
            args: vec![DoCtrl::Pure { value }],
            kwargs: vec![],
