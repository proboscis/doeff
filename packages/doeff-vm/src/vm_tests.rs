...
fn test_apply_return_delivers_value_without_pushing_frame() {
    Python::attach(|py| {
        ...
        vm.current_seg_mut().mode = Mode::HandleYield(DoCtrl::Apply {
            f: Box::new(DoCtrl::Pure { value: Value::Python(dummy_f) }),
            args: vec![],
            kwargs: vec![],
            metadata,
...
fn test_expand_requires_doeff_generator_or_errors() {
    Python::attach(|py| {
        ...
        vm.current_seg_mut().mode = Mode::HandleYield(DoCtrl::Expand {
            factory: Box::new(DoCtrl::Pure { value: Value::Python(dummy_factory) }),
            args: vec![],
            kwargs: vec![],
...
fn test_r9a_apply_with_args_yields_call_func() {
    Python::attach(|py| {
        ...
        vm.current_seg_mut().mode = Mode::HandleYield(DoCtrl::Apply {
            f: Box::new(DoCtrl::Pure { value: Value::Python(dummy_f) }),
            args: vec![
                DoCtrl::Pure { value: Value::Int(42) },
                DoCtrl::Pure { value: Value::String("hello".to_string()) },
            ],
            kwargs: vec![],
            metadata,
            evaluate_result: false,
...
fn test_r9a_apply_kwargs_preserved_separately() {
    Python::attach(|py| {
        ...
        vm.current_seg_mut().mode = Mode::HandleYield(DoCtrl::Apply {
            f: Box::new(DoCtrl::Pure { value: Value::Python(dummy_f) }),
            args: vec![DoCtrl::Pure { value: Value::Int(1) }],
            kwargs: vec![
                ("key1".to_string(), DoCtrl::Pure { value: Value::Int(2) }),
                (
                    "key2".to_string(),
                    DoCtrl::Pure { value: Value::String("val".to_string()) },
                ),
            ],
            metadata,
            evaluate_result: false,
...
fn test_r9a_apply_kwargs_only_takes_callfunc_path() {
    Python::attach(|py| {
        ...
        vm.current_seg_mut().mode = Mode::HandleYield(DoCtrl::Apply {
            ...
            kwargs: vec![(
                "name".to_string(),
                DoCtrl::Pure { value: Value::String("test".to_string()) },
            )],
            metadata,
            evaluate_result: false,
        });
