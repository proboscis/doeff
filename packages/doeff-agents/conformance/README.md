...
| F-api-limit | `rate limit exceeded` / `quota exceeded` 等(tail 30 行。issue #557 で現行 claude TUI の exhausted 側文言 — `usage limit reached` / `limit reached · resets` 等 — を追補。approaching 側 `used NN% … · resets` は対象外。ACP ADR 0049 R9(2026-07-26 実 incident)でさらに `out of usage credits` を追補。**R9 改訂(2026-08-07)**: 所有格族 `you've hit/reached your … limit` は逐語列挙でなく **bounded 族照合**(`API-LIMIT-EXHAUSTED-FAMILY-RE` — 所有格〜limit 間へ空白区切り 0〜4 語・各語は英数 + 内部ピリオドのみの有界可変挿入を許す。逐語列挙は 2026-07-20 / 07-26 / 08-06 と 3 度同型で破れた: 8/6 は `You've reached your Fable 5 limit. /model to switch models.` が 16 逐語のどれにも部分一致せず 22 terminal が run_failed/retryable=false 落ち。動詞は exhausted 側 hit\|reached のみ — approaching 側 `used` は族外のまま) | status blocked_api(観測は行の `api_limit_observed_at` に durable latch — issue #557) |
...
| F-waiting | `Type your message` 等(raw 一致) | status blocked **ただし active marker が同時に立っていないときに限る**(**改訂 2026-08-12** — ADR-DOE-AGENTS-010 R-blocked-needs-no-work-evidence-4f1c / ACP issue 55b1bd): 判定材料 7 語のうち `accept edits` / `bypass permissions` / `shift+tab to cycle` は現行 claude TUI の permission-mode インジケータ = **作業中も常時描画される常設フッター**で、しかも waiting だけ capture 全文一致(他は tail 30 行窓)。旧契約(waiting 単独で blocked 確定)では全席が起動直後から終了まで blocked と記録され、その status を『指示未配達』と読む ACP の有界 dwell(ADR 9211b3・既定 1800s)が働いている席を終端した — 実測(2026-08-12 精密化 — 母集団は打ち切りの理由文で取る): この dwell が終端した席の全量を両 root の failure-reports の理由文『dwelled blocked (awaiting input)』から集め agentd と突合(**186 席・186/186 一致**)。うち **175 席(94%)は死の 60 秒前まで画面が変化**、**bound を超えて静止していた席は 0 件** = この壁は存在理由(固まった席の片付け)を一度も果たさないまま働く席だけを殺した。⚠ 寿命帯で母集団を切ると過小計上(1860..2000s では 186 のうち 168 しか入らない — 1800 秒は誤判定の開始時刻起点なので、働いてから誤判定された席は寿命が伸びる)。status で絞るのも不可(ACP の dwell 打ち切りは agentd 側で stopped として着地し failed にはならない)。連言に `F-turn-activity-claude`(⏺ / ⎿)は**含めない** — idle 画面にも残留する痕跡で、含めると blocked が到達不能になり本当に固まった席が不可視化する |
...
| S13 | claude launch(M1)→ fake が受領した argv に `--settings {"disableAllHooks":true}` と `--mcp-config`(doeff_result stdio)を確認。codex launch → `-c mcp_servers.doeff_result.command=` を確認。**hook 配布は 2 態**(2026-08-18 route-c03fe34745: 全 hook 無効化は安全側 hook〔破壊的 git / pattern kill / push 門〕まで切り、ACP 起動会話 241 母集団で発火 0 の実測): daemon env `DOEFF_AGENTD_SESSION_HOOKS=inherit` では claude の `--settings` pair が argv から外れ、spawn env が `AGENT_SESSION_CLASS=unattended` を宣言する(config-dir 所有者の hook 層が会話種別で self-gate する契約)。語彙外の値は launch fail-loud | 49b3549b 傷跡 / 0035 配線 | — | P | M1 |
...
| S28d | **pre-existing 孤児 reaper(fixture setup 側の宣言的 reconciler、2026-07-30 host-load-steward 観測起点: 25 daemon 全件 ppid==1 + parked conformance_agent 9 + 残渣 dir 13)**: harness 起動時(`AgentdHarness.__enter__`)に「--db が /tmp/agentd-conf-* 配下 かつ ppid==1(supervisor の死は init への reparent で証明)」の doeff-sessionhost serve を列挙し、launch 済み非 adopted active session を daemon 自身の cleanup 意味論(wire `session.cleanup` — mux session と対で pane 内 agent が消える)で刈ってから daemon を退場させ(graceful SIGTERM → lease 釈放、SIGKILL backstop、署名直前に pid の command line 再照合)、live daemon の居ない残渣 /tmp/agentd-conf-* dir を sweep(mtime 猶予 300s — 並走 harness の setup 中 dir は不可侵)。spawn 時 env knob(#580 の S28 自己終了境界)は既存孤児に後付けできない — 本行はその補完(knob 無し経路への defense in depth)。安全境界を反例で pin: **ppid!=1(生きた supervisor = S16 並走物理)不可侵 / production(`~/.local/state/doeff/agentd.sqlite`)は path で除外**(launchd 直下は構造的に ppid==1 のため path が弁別子)/ adopted 席不可侵(鏡原則) | 寿命境界の補完(spawn 時 knob = 将来孔 / 本行 = 既存蓄積の収束) | — | P(hy gate のみ — harness 側機構で daemon 不変) | fixture setup + ps/帯域外観測 |
| S29 | **外部命名席の adopt(herdr backend のみ・2026-08-17 protocol 17 追随便)**: `ai` 等が herdr の agent 名簿に付けた席名(`herdr agent list` の name — dotfiles の adopt reconciler が `session.adopt` の session_name に運ぶ)は doeff 所有の workspace を持たず、workspace label は repo 名(実測: 自席 = agent 名 s-7bfc5d5028 / label "doeff")。label だけの生死判定はこれらを全部「不在」と答え、adopt を全拒否(adopt_target_not_found)・既存 adopted 行の substrate_present を一斉 false に倒す。herdr substrate は label holder の無い名前に限り agent 名簿を**実在確認**として読む(substrate_herdr.hy herdr-external-agent-pane-id-io — 唯一の agent.get・semgrep waiver marker 直下。同一性・帰属・kill は label のまま)。fixture は label ≠ 名前の席(workspace.create {label: name-ws} → pane.report_agent → agent.rename name)。adopt 成功・`substrate_present: true`、帯域外で workspace を閉じると false へ倒れ行は非終端のまま(S27 の外部命名形) | ADR-DOE-AGENTS-004 law herdr-session-identity-is-workspace-label / ADR-007 R2・R4 | — | P(hy gate のみ・herdr backend のみ — tmux は名前空間が 1 つで同型なし) | 帯域外 herdr fixture + wire |
...
| S13 | ✅ green | test_s13_argv_wiring.py | journaled argv から claude の `--settings {"disableAllHooks":true}`/`--mcp-config`(doeff_result stdio)/`--strict-mcp-config`、codex の `-c mcp_servers."doeff_result".command=`/`.args=[...]` を厳密一致。claude 変種は M1 golden path 完走も兼ねる。inherit 変種(`DOEFF_AGENTD_SESSION_HOOKS=inherit`)は `--settings` 不在 + spawn env `AGENT_SESSION_CLASS=unattended` を確認 |
