...
| F-waiting | `Type your message` 等(raw 一致) | status blocked **ただし active marker が同時に立っていないときに限る**(**改訂 2026-08-12** — ADR-DOE-AGENTS-010 R-blocked-needs-no-work-evidence-4f1c / ACP issue 55b1bd): 判定材料 7 語のうち `accept edits` / `bypass permissions` / `shift+tab to cycle` は現行 claude TUI の permission-mode インジケータ = **作業中も常時描画される常設フッター**で、しかも waiting だけ capture 全文一致(他は tail 30 行窓)。旧契約(waiting 単独で blocked 確定)では全席が起動直後から終了まで blocked と記録され、その status を『指示未配達』と読む ACP の有界 dwell(ADR 9211b3・既定 1800s)が働いている席を終端した — 実測 2026-08-06..12 の壁帯 218 席(寿命 1860..2000s で cancelled)のうち **142 席(65%)は死の 10 秒前まで画面が変化**しており、真に静止していたのは 51 席のみ。連言に `F-turn-activity-claude`(⏺ / ⎿)は**含めない** — idle 画面にも残留する痕跡で、含めると blocked が到達不能になり本当に固まった席が不可視化する |
