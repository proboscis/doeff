...
(setv TERMINAL-CAUSE-RETRYABLE
      {"rate_limited" True
       ...
       ;; (計画された入れ替え — 書いていた session host の headless の器は agora-redesign #668 で退いた。語は既存の行のために残す)。
...
(deff session-env-admission-error [session-env verb]
  ...
  (when provider-auth
    (return (+ f"{verb}: provider auth env is forbidden in agent sessions "
               f"(offending: {(.join ", " provider-auth) }). "
               "Agent seats authenticate with subscription profiles via the "
               "typed `binding` field only — API-key-backed calls go through "
               "memoized LLM query handlers, never agent session environments "
