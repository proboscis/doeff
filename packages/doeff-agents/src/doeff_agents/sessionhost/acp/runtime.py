    ...
    def draining() -> bool:
        ...
        if present != marked:
            ...
            if present:
                ...
                log(f"agentd: drain file {drain_file} is gone — capacity is the declared value again")
        return present
...
class AgentdRun:
    ...
    def close_for_exit(self, reason: str) -> None:
        ...
        閉じる)と**別の腕**として分ける。"""
        _stderr(f"agentd: exit ({reason}) — leaving the running turns to the host process; not closing any job")
        self.stop.set()
...
def wait_for_host_socket(
    ...
    while not socket_is_listening(socket_path):
        ...
        if stopping():
            log(f"agentd: stop was signalled while waiting for the sessionhost socket {socket_path}")
            return False
        sleep(0.5)
    return True
...
def start_agentd_thread(
    ...
    def body() -> None:
        ...
        run_loop(
            settings, dispatchers, stop, _stderr, holder, drain, cache_dispatchers,
