...
# ADR-DOE-AGENTS-012 R61: 起動できる agent の種類の観測と、host に届かない拍の申告の取り下げ
def observe_agent_kinds(settings: AgentdSettings, state: AgentdState) -> Program: ...
def withdraw_node_capabilities(settings: AgentdSettings, key: str) -> Program: ...
def write_node_observations(
    settings: AgentdSettings, key: str, sessions: list, transcripts: list, kinds: tuple
) -> Program: ...
...
# card acp:kanban-issue:ki-0bb4104cd8c2: 送信待ちの列 — 運搬郵便が運ぶ入力の行を取る / 読まれた印を書く。
def mail_id_of_row(message: AcpRow) -> Program: ...
def take_carried_input(job_id: str, message: AcpRow, now_ms: int) -> Program: ...
def take_carried_inputs(job_id: str, rows: tuple, now_ms: int) -> Program: ...
def mark_inputs_read(taken: tuple, handed: tuple, evidence: str, now_ms: int) -> Program: ...
...
def adopt_existing_record(settings: AgentdSettings, job: InFlightJob) -> Program: ...
def realign_record_spec(job: InFlightJob, row: AcpRow) -> Program: ...
...
def end_turn_record(
    job_id: str, usage: dict | None, entries: tuple, mark: tuple | None, conditions: tuple,
    cache_observation: dict | None = None, responses: tuple = (), job: InFlightJob | None = None
) -> Program: ...
def drain_stream(
...
# 拍の途中の受け(観測の合間に新しい結びを受け付けの係へ — 受けの腕は拍に 1 回のまま)
def receive_fresh_bindings(settings: AgentdSettings, state: AgentdState, now_ms: int) -> Program: ...
def receive_checkpoint(settings: AgentdSettings, state: AgentdState, look_ms: int) -> Program: ...
...
# agora-redesign #519(D-519-3): 配置が退役させた行の turn-record を閉じる書き(ADR-012 R49 の検が材料の役で読む)
def end_retired_records(
    settings: AgentdSettings, rows: tuple, node_row_id: str | None
) -> Program: ...
