"""Pytest wrapper for sessionhost contract-schema deftests (doeff#482).

Same dynamic exposure pattern as ``test_sessionhost_store.py``: every
``test_*`` deftest in ``sessionhost_schema_deftests.hy`` is surfaced
...
_deftests = importlib.import_module("sessionhost_schema_deftests")


def _deftest_interpreter(program: Any, *, env: dict[Any, Any] | None = None) -> Any:
    if env is not None:
        raise ValueError("sessionhost schema deftests do not use env overrides")
    return run(program)
...
assert _names, "sessionhost_schema_deftests exposes no test_* deftests"
for _name in _names:
