"""Pytest wrapper for the agentd conversation-handoff deftests (ADR-DOE-AGENTS-012 R20・段 8q・agora-redesign #51).

Same dynamic exposure pattern as ``test_sessionhost_turn.py``: every
``test_*`` deftest in ``sessionhost_acp_rehydrate_deftests.hy`` is surfaced
...
_deftests = importlib.import_module("sessionhost_acp_rehydrate_deftests")


def _deftest_interpreter(program: Any, *, env: dict[Any, Any] | None = None) -> Any:
    if env is not None:
        raise ValueError("sessionhost acp rehydrate deftests do not use env overrides")
    return run(program)
...
assert _names, "sessionhost_acp_rehydrate_deftests exposes no test_* deftests"
for _name in _names:
