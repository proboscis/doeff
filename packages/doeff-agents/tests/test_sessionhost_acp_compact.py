"""Pytest wrapper for the agentd self-compaction deftests (段 10f 便 2・agora-redesign #82 — compactAt と直前の手番の文脈の実測).

Same dynamic exposure pattern as ``test_sessionhost_turn.py``: every
``test_*`` deftest in ``sessionhost_acp_compact_deftests.hy`` is surfaced
...
_deftests = importlib.import_module("sessionhost_acp_compact_deftests")


def _deftest_interpreter(program: Any, *, env: dict[Any, Any] | None = None) -> Any:
    if env is not None:
        raise ValueError("sessionhost acp compact deftests do not use env overrides")
    return run(program)
...
assert _names, "sessionhost_acp_compact_deftests exposes no test_* deftests"
for _name in _names:
