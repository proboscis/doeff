"""Pytest wrapper for the auto-memory-home deftests (ADR-DOE-AGENTS-006 R11).

Same dynamic exposure pattern as ``test_sessionhost_turn.py``: every
``test_*`` deftest in ``sessionhost_acp_memory_home_deftests.hy`` is surfaced
...
_deftests = importlib.import_module("sessionhost_acp_memory_home_deftests")


def _deftest_interpreter(program: Any, *, env: dict[Any, Any] | None = None) -> Any:
    if env is not None:
        raise ValueError("sessionhost acp memory-home deftests do not use env overrides")
    return run(program)
...
assert _names, "sessionhost_acp_memory_home_deftests exposes no test_* deftests"
for _name in _names:
