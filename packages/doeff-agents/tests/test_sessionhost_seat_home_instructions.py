"""Pytest wrapper for the seat-home common-instruction deftests.

Same dynamic exposure pattern as ``test_sessionhost_launch.py``: every
``test_*`` deftest in ``sessionhost_seat_home_instructions_deftests.hy`` is
...
_deftests = importlib.import_module("sessionhost_seat_home_instructions_deftests")


def _deftest_interpreter(program: Any, *, env: dict[Any, Any] | None = None) -> Any:
    if env is not None:
        raise ValueError("seat-home instruction deftests do not use env overrides")
    return run(program)
...
assert _names, "sessionhost_seat_home_instructions_deftests exposes no test_* deftests"
for _name in _names:
