"""Pytest wrapper for the turn-event durability deftests (段 8 lane 4u・agora-redesign #49・ADR-DOE-AGENTS-012 R19).

Same dynamic exposure pattern as ``test_sessionhost_acp_profile.py``: every
``test_*`` deftest in ``sessionhost_acp_turn_events_deftests.hy`` is surfaced
...
_deftests = importlib.import_module("sessionhost_acp_turn_events_deftests")


def _deftest_interpreter(program: Any, *, env: dict[Any, Any] | None = None) -> Any:
    if env is not None:
        raise ValueError("sessionhost acp turn-events deftests do not use env overrides")
    return run(program)
...
assert _names, "sessionhost_acp_turn_events_deftests exposes no test_* deftests"
for _name in _names:
