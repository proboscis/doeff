...
(import doeff_agents.sessionhost.acp.reply_channel [turn-input-text-of])
...
(deftest test-a-continued-turn-rebuilt-on-tui-keeps-the-handed-mail-in-the-transcript-only
  ...
  (assert (= (str-at launch "prompt") "【前置き】\nstart") launch)
...
(deftest test-a-body-ref-is-read-by-its-stream-and-folded-where-the-body-was
  ...
  ;; 設計 direct-chat-2026-09-24 §3: 各項は出どころの判定つき(区切り + 見出し + 本文 — reply_channel.turn-input-text-of の 1 点)。
  (setv inline-item (run (turn-input-text-of "m-1" inline.spec "短い本文")))
  (assert (= (run (message-bodies-of #(inline ref-row) #("m-1" "m-b") {"m-b" "長い本文"} {}))
             #(#(inline-item (run (turn-input-text-of "m-b" ref-row.spec "長い本文"))) #(#() #()) #())))
