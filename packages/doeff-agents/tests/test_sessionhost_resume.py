"""Pytest wrapper for sessionhost resume/fork deftests (ADR-DOE-AGENTS-006).

Same dynamic exposure pattern as ``test_sessionhost_launch.py``: every
``test_*`` deftest in ``sessionhost_resume_deftests.hy`` is surfaced
...
_deftests = importlib.import_module("sessionhost_resume_deftests")


def _deftest_interpreter(program: Any, *, env: dict[Any, Any] | None = None) -> Any:
    if env is not None:
        raise ValueError("sessionhost resume deftests do not use env overrides")
    return run(program)
...
assert _names, "sessionhost_resume_deftests exposes no test_* deftests"
for _name in _names:
