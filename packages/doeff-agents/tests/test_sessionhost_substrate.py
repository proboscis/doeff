"""Pytest wrapper for sessionhost real substrate deftests (DOE-004 C2).

Same dynamic exposure pattern as ``test_sessionhost_policy.py``: every
``test_*`` deftest in ``sessionhost_substrate_deftests.hy`` is surfaced
...
_deftests = importlib.import_module("sessionhost_substrate_deftests")


def _deftest_interpreter(program: Any, *, env: dict[Any, Any] | None = None) -> Any:
    if env is not None:
        raise ValueError("sessionhost substrate deftests do not use env overrides")
    return run(program)
...
assert _names, "sessionhost_substrate_deftests exposes no test_* deftests"
for _name in _names:
