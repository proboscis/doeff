"""Pytest wrapper for the lender-refusal deftests (試作・card acp:kanban-issue:ki-fd0f3b234a38).

Same dynamic exposure pattern as ``test_sessionhost_acp_profile.py``: every
``test_*`` deftest in ``sessionhost_acp_lender_deftests.hy`` is surfaced
