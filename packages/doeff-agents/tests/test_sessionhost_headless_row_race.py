"""Pytest wrapper for the headless row read-modify-write race deftests.

Same dynamic exposure pattern as ``test_sessionhost_charter_reaches_the_seat.py``:
every ``test_*`` deftest in ``sessionhost_headless_row_race_deftests.hy`` is
...
_deftests = importlib.import_module("sessionhost_headless_row_race_deftests")


def _deftest_interpreter(program: Any, *, env: dict[Any, Any] | None = None) -> Any:
    if env is not None:
        raise ValueError("headless row race deftests do not use env overrides")
    return run(program)
...
assert _names, "sessionhost_headless_row_race_deftests exposes no test_* deftests"
for _name in _names:
