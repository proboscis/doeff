"""Pytest wrapper for the agentd verify-command deftests (ADR-DOE-AGENTS-012 R36・段 12 lane 12a・agora-redesign #230).

Same dynamic exposure pattern as ``test_sessionhost_acp_profile.py``: every
``test_*`` deftest in ``sessionhost_acp_verify_deftests.hy`` is surfaced
...
_deftests = importlib.import_module("sessionhost_acp_verify_deftests")


def _deftest_interpreter(program: Any, *, env: dict[Any, Any] | None = None) -> Any:
    if env is not None:
        raise ValueError("sessionhost acp verify deftests do not use env overrides")
    return run(program)
...
assert _names, "sessionhost_acp_verify_deftests exposes no test_* deftests"
for _name in _names:
