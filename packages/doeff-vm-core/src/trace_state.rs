...
pub(crate) struct LiveDispatchSnapshot {
    pub(crate) origin_dispatch_id: FiberId,
...
impl TraceState {
    ...
    pub(crate) fn has_dispatch(&self, origin_dispatch_id: FiberId) -> bool {
        self.completed_dispatches
            .iter()
            .any(|dispatch| dispatch.origin_dispatch_id == origin_dispatch_id)
    }
    ...
    pub(crate) fn dispatch_has_terminal_result(&self, origin_dispatch_id: FiberId) -> bool {
        self.completed_dispatches
            .iter()
            .find(|dispatch| dispatch.origin_dispatch_id == origin_dispatch_id)
    ...
    pub(crate) fn remember_completed_dispatch(&mut self, preserved: Option<LiveDispatchSnapshot>) {
        ...
        self.completed_dispatches.push(PreservedDispatchSnapshot {
            origin_dispatch_id: preserved.origin_dispatch_id,
    ...
    pub(crate) fn assemble_traceback_entries(
        ...
        for dispatch in &dispatches {
            ...
            entries.push(TraceEntry::Dispatch {
                dispatch_id: dispatch.origin_dispatch_id,
    ...
    fn collect_dispatches(
        ...
        for dispatch in live_dispatches {
            if !seen.insert(dispatch.origin_dispatch_id) {
                continue;
            }
        ...
        dispatches.sort_by_key(|dispatch| dispatch.origin_dispatch_id.index());
        dispatches
    }
    ...
    fn active_chain_entries(
        ...
        for (index, frame) in frames.iter().enumerate() {
            if let Some(dispatch) = Self::dispatch_for_frame(dispatches, frame.frame_id, false) {
                represented.insert(dispatch.origin_dispatch_id);
        ...
        for dispatch in dispatches
            .iter()
            .filter(|dispatch| !represented.contains(&dispatch.origin_dispatch_id))
        {
            if !Self::dispatch_is_visible(&dispatch.dispatch_display) {
                continue;
            }
