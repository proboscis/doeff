...
impl VM {
        ...
        if matches!(continuation, EvalReturnContinuation::TailResumeReturn) {
            return match mode {
                ...
                Mode::Throw(exception) => {
                    if let Some(origin_fiber_id) = self.current_origin_fiber_id() {
    ...
    fn apply_stream_step(
        ...
        match step {
            ...
            IRStreamStep::Throw(exc) => {
                ...
                let origin_fiber_id = self.current_active_handler_dispatch_id().or_else(|| {
                    let origin_fiber_id = self.current_segment_dispatch_id_any()?;
                    if self.current_segment_is_active_handler_for_dispatch(origin_fiber_id) {
                ...
                if let Some(origin_fiber_id) = origin_fiber_id.filter(|_| !propagated_throw) {
                    self.emit_handler_threw_for_dispatch(origin_fiber_id, &exc);
                }
            ...
            IRStreamStep::NeedsPython(call) => {
                ...
                let (marker, k) = if let Some(origin_fiber_id) = self.current_origin_fiber_id() {
                    let Some((_, k, marker)) = self
                        .active_handler_dispatch_for(origin_fiber_id)
                        .or_else(|| self.handler_dispatch_for_any(origin_fiber_id))
                    else {
                        return StepEvent::Error(VMError::internal(
    ...
    pub(super) fn handle_yield_eval_in_scope(
        ...
        let child_parent = if scope_parent_seg_id == current_seg_id {
            Some(scope_parent_seg_id)
        } else {
            let mut return_anchor = Segment::new(Some(scope_parent_seg_id));
            return_anchor.push_frame(Frame::EvalReturn(Box::new(
                EvalReturnContinuation::ReturnToContinuation {
                    continuation: Continuation::from_fiber(current_seg_id, captured_caller),
                },
            )));
            let anchor_seg_id = self.alloc_segment(return_anchor);
            self.inherit_interceptor_guard_state(Some(current_seg_id), anchor_seg_id);
            Some(anchor_seg_id)
        };
        let return_to = child_parent
            .map(|seg_id| Continuation::from_fiber(seg_id, self.parent_segment(seg_id)))
            .expect("EvalInScope child must have a parent segment");
        let mut child_seg = Segment::new(child_parent);
        ...
        match outcome {
            ...
            PyCallOutcome::GenError(exception) => {
                ...
                if self.should_throw_into_live_handler_frame(effective_handler_kind) {
                    ...
                    if !propagated_throw {
                        if let Some(origin_fiber_id) = self
                    ...
                    .or_else(|| self.current_segment_dispatch_id_any())
                {
                    ...
                    if effective_handler_kind.is_some()
                        && !self.dispatch_uses_user_continuation_stream(origin_fiber_id, &stream)
                        && !propagated_throw
                    {
                        if let Some(original) =
                            self.original_exception_for_dispatch(origin_fiber_id)
                ...
                if let Some(origin_fiber_id) = active_dispatch_id
                    ...
                    .or_else(fallback_active_handler_dispatch)
                {
                    let propagated_throw = incoming_throw
                        .as_ref()
                        .is_some_and(|original| Self::same_exception(original, &exception));
                    if self.dispatch_uses_user_continuation_stream(origin_fiber_id, &stream)
                        || propagated_throw
                    {
                        site = GenErrorSite::StepUserGeneratorConverted;
                    } else if self.current_segment_is_active_handler_for_dispatch(origin_fiber_id) {
                        if let Some(original) =
                            self.original_exception_for_dispatch(origin_fiber_id)
                        {
                            TraceState::set_exception_cause(&exception, &original);
                        ...
                        && self.current_segment_dispatch_id_any().is_some_and(
                            |current_origin_fiber_id| current_origin_fiber_id == origin_fiber_id,
                        )
                    {
                        if let Some(original) =
                            self.original_exception_for_dispatch(origin_fiber_id)
