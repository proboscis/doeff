...
impl VM {
    ...
    pub(super) fn dispatch_supports_error_context_conversion(
        ...
        let marker = self
            ...
            .filter(|(_, current_origin_fiber_id, ..)| *current_origin_fiber_id == origin_fiber_id)
            .map(|(_, _, _, marker, _)| marker)
            .or_else(|| self.active_handler_marker_for_dispatch(origin_fiber_id));
    ...
    fn is_execution_context_effect_for_dispatch(&self, origin_fiber_id: SegmentId) -> bool {
    ...
    pub(super) fn current_handler_identity_for_dispatch(
        ...
        let marker = active_dispatch
            ...
            .or_else(|| {
                self.current_segment
                    .filter(|_| self.current_segment_dispatch_id() == Some(origin_fiber_id))
        ...
        let origin_seg_id = active_dispatch
            ...
            .or_else(|| self.dispatch_origin_user_segment_id(origin_fiber_id))?;
    ...
    pub(super) fn current_segment_is_active_handler_for_dispatch(
        ...
        self.current_handler_dispatch()
            .is_some_and(|(seg_id, current_origin_fiber_id, _, _, _)| {
                Some(seg_id) == self.current_segment && current_origin_fiber_id == origin_fiber_id
            })
    }

    pub(super) fn current_active_handler_dispatch_id(&self) -> Option<SegmentId> {
        self.current_live_handler_dispatch()
            .map(|(_, origin_fiber_id, _, _, _)| origin_fiber_id)
    }
    ...
    pub(super) fn user_continuation_dispatch_for_stream(
        ...
        self.dispatch_origins().into_iter().find_map(|origin| {
            self.continuation_uses_stream(&origin.k_origin, stream)
                .then_some(origin.origin_fiber_id)
        })
    }
    ...
    pub(super) fn handler_stream_throw_continuation(
        ...
        if self.is_execution_context_effect_for_dispatch(origin_fiber_id) {
            return None;
        }
        let origin = self.dispatch_origin_for_origin_fiber_id(origin_fiber_id)?;
        let continuation = if self.dispatch_uses_user_continuation_stream(origin_fiber_id, stream) {
            origin.k_origin
    ...
    pub(super) fn original_exception_for_dispatch(
        ...
        if let Some((seg_id, current_origin_fiber_id, ..)) = self.current_handler_dispatch() {
            if current_origin_fiber_id == origin_fiber_id {
    ...
    pub(super) fn mode_after_generror(
        ...
        let current_origin_fiber_id = self.current_origin_fiber_id().or(active_dispatch_id);
        let active_handler_supports_conversion =
            active_dispatch_id.is_some_and(|origin_fiber_id| {
        ...
        let in_get_execution_context_dispatch =
            current_origin_fiber_id.is_some_and(|origin_fiber_id| {
                self.is_execution_context_effect_for_dispatch(origin_fiber_id)
            });
        ...
        if in_get_execution_context_dispatch {
            if let Some(origin_fiber_id) = current_origin_fiber_id {
    ...
    pub(super) fn emit_handler_threw_for_dispatch(
        ...
        let is_live_handler_throw = self.current_active_handler_dispatch_id()
            == Some(origin_fiber_id)
            || self.current_segment_is_active_handler_for_dispatch(origin_fiber_id);
        if self.dispatch_has_terminal_handler_action(origin_fiber_id) && !is_live_handler_throw {
            return;
        }
        let handler_identity = self
            ...
            .or_else(|| {
                ...
                if self.current_segment_dispatch_id() != Some(origin_fiber_id) {
                    return None;
                }
    ...
    pub(crate) fn assemble_active_chain_for_dispatch(
        ...
        let current_segment = self
            .dispatch_origin_user_segment_id(origin_fiber_id)
    ...
    fn should_attach_active_chain_for_dispatch(&self, origin_fiber_id: SegmentId) -> bool {
    ...
    pub(super) fn maybe_attach_active_chain_to_execution_context(
        ...
        let Some(origin_fiber_id) = origin_fiber_id else {
            return Ok(());
        };
        if !self.should_attach_active_chain_for_dispatch(origin_fiber_id) {
            return Ok(());
        }
        ...
        let mut active_chain = self.assemble_active_chain_for_dispatch(origin_fiber_id, None);
