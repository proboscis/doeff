...
const DISPATCH_LOOKUP_PROFILE_ENV: &str = "DOEFF_VM_DISPATCH_PROFILE";
...
pub struct DispatchLookupStats {
    pub dispatch_origin_in_segment_calls: u64,
    pub dispatch_origin_in_segment_for_dispatch_calls: u64,
    pub dispatch_origins_calls: u64,
...
struct DispatchLookupProfiler {
    timing_enabled: AtomicBool,
    dispatch_origin_in_segment_calls: AtomicU64,
    dispatch_origin_in_segment_for_dispatch_calls: AtomicU64,
    dispatch_origins_calls: AtomicU64,
...
impl DispatchLookupProfiler {
    ...
    fn record_dispatch_origin_in_segment(&self, scanned_frames: usize, started: Option<Instant>) {
        if !self.enabled() {
            return;
        }
        ...
        self.frames_scanned
            .fetch_add(scanned_frames as u64, Ordering::Relaxed);
        self.record_lookup_duration(started);
    }

    fn record_dispatch_origin_in_segment_for_dispatch(
        &self,
    ...
    fn record_dispatch_origins_call(&self) {
        if !self.enabled() {
            return;
        }
    ...
    fn record_step_duration(&self, started: Option<Instant>) {
        if !self.enabled() {
            return;
        }
    ...
    fn snapshot(&self) -> DispatchLookupStats {
        DispatchLookupStats {
            ...
            dispatch_origin_in_segment_for_dispatch_calls: self
                .dispatch_origin_in_segment_for_dispatch_calls
                .load(Ordering::Relaxed),
            dispatch_origins_calls: self.dispatch_origins_calls.load(Ordering::Relaxed),
    ...
    fn record_lookup_duration(&self, started: Option<Instant>) {
        if !self.enabled() {
            return;
        }
        let Some(started) = started else {
...
pub struct VM {
    ...
    dispatch_lookup_profiler: DispatchLookupProfiler,
}
...
impl VM {
    pub fn new() -> Self {
        VM {
            ...
            dispatch_lookup_profiler: DispatchLookupProfiler::default(),
        }
    }
    ...
    pub fn begin_run_session(&mut self) -> u64 {
        ...
        self.dispatch_lookup_profiler.reset_for_run();
        token
    }
    ...
    pub fn dispatch_lookup_stats(&self) -> DispatchLookupStats {
        self.dispatch_lookup_profiler.snapshot()
    }
