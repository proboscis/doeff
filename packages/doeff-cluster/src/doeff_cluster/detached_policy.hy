...
;;; renew-detached・absorb-detached-report)。ここは要求 1 件 → #(次の状態 status 本文) だけ。
(import dataclasses [replace])
(import .cluster_model [ClusterState TaskRecord])
...
(defn #^ tuple submit-detached [#^ ClusterState state #^ str key #^ dict body #^ int now]
  "PUT /detached/<key>: 同じ key の行が在ればそれを返す(created = false)。env・name・requires が違えば 409(同じ job id を別の
   仕事に使った呼び手の誤り)。無ければ待ちの行を作る。返り値 #(次の状態 status 本文)。"
  (setv lease (.get body "leaseSeconds" DETACHED-DEFAULT-LEASE-SECONDS)
        ...
        requires (tuple (sorted (.items (.get body "requires" {}))))
  ...
  (when (is-not existing None)
    (return
      (if (= #(existing.env existing.name existing.requires) #((get body "env") (.get body "name" "") requires))
          #(state 200 {"key" key "task" existing.id "created" False "phase" existing.phase})
          #(state 409 {"error" (.format "key {} は別の仕事(env {}・name {!r}・requires {})に使われている"
  ...
  (when (>= detached-count DETACHED-MAX-RECORDS)
    (return #(state 429 {"error" (.format "切り離した task の行(保持中を含む)が上限 {} 本に達している — 終わった物を解放する"
  ...
  #((replace state :tasks (| state.tasks {id task}) :next-task (+ state.next-task 1))
...
(defn #^ tuple cancel-detached [#^ ClusterState state #^ str key #^ int now]
  "POST /detached/<key>/cancel: 終わっていなければ cancelled にする(担い手は次の heartbeat の返事から外れた子 process を止める)。
   終わっていれば何もしない(結果は保持)。返り値 #(次の状態 本文)。"
  (setv task (task-by-key state key))
  (cond
    (is task None) #(state {"key" key "cancelled" False "phase" "unknown"})
    (in task.phase DETACHED-TERMINAL) #(state {"key" key "cancelled" False "phase" task.phase})
    True #((replace state :tasks (| state.tasks {task.id (end-detached task "cancelled" now "取り消された")}))
           {"key" key "cancelled" True "phase" "cancelled"})))


(defn #^ tuple release-detached [#^ ClusterState state #^ str key]
  "DELETE /detached/<key>: 終わった task の行を消す(以後その key は unknown・同じ key で送り直せる)。まだ終わっていなければ 409。
   返り値 #(次の状態 status 本文)。"
  (setv task (task-by-key state key))
  (cond
    (is task None) #(state 200 {"key" key "released" False})
    (not-in task.phase DETACHED-TERMINAL)
      #(state 409 {"error" (.format "key {} はまだ終わっていない({})— 先に取り消す" key task.phase)})
    True #((replace state :tasks (dfor #(k v) (.items state.tasks) :if (!= k task.id) k v)) 200 {"key" key "released" True})))
