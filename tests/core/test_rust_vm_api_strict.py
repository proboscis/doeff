...
@pytest.mark.asyncio
async def test_async_run_warns_on_early_termination(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    class ErrValueStub:
    ...
    result = await rust_vm_module.async_run(Program.pure(1), handlers=[])
...
@pytest.mark.asyncio
async def test_async_run_can_suppress_early_termination_warning(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    class RunResultStub:
    ...
    result = await rust_vm_module.async_run(
        Program.pure(1),
        handlers=[],
