...
def _capture_http_request_handler(captured: list[HttpRequest]):
    ...
    def handler(effect, k):
        if isinstance(effect, HttpRequest):
            ...
            return (yield Resume(k, effect))
        yield Pass(effect, k)

    return handler


def _run_wrapper(program: Any) -> HttpRequest:
...
def test_http_get_wrapper_builds_request() -> None:
    ...
    request = _run_wrapper(http_get("https://example.test/data", params={"a": "1"}))
...
def test_http_post_wrapper_builds_json_request() -> None:
    ...
    request = _run_wrapper(http_post("https://example.test/data", body={"a": 1}))
...
def test_http_put_delete_head_wrappers_build_methods() -> None:
    ...
    put_request = _run_wrapper(http_put("https://example.test/data", body="payload"))
    delete_request = _run_wrapper(http_delete("https://example.test/data"))
    head_request = _run_wrapper(http_head("https://example.test/data"))
