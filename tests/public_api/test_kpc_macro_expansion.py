...
from doeff_vm import Call, Perform, Pure
...
def test_do_call_returns_call_doctrl() -> None:
    result = takes_plain(1)
    assert isinstance(result, Call), f"expected Call DoCtrl, got {type(result).__name__}"
...
def test_kernel_is_exposed_from_call_f_as_pure() -> None:
    result = takes_plain(3)
    assert isinstance(result, Call), f"expected Call DoCtrl, got {type(result).__name__}"
    assert isinstance(result.f, Pure), (
        f"expected Pure kernel wrapper, got {type(result.f).__name__}"
    )
