...
class NonBoolEqType:
    """Object whose equality comparison raises ambiguous-truth errors."""
...
def test_wstep_eq_with_non_bool_comparison_in_meta():
    ...
    # Create steps with custom values whose __eq__ returns non-bool.
    value1 = NonBoolEqType([1, 2, 3])
    value2 = NonBoolEqType([4, 5, 6])

    step1 = WStep(inputs=(node1,), output=output_node, meta={"data": value1})
    step2 = WStep(inputs=(node1,), output=output_node, meta={"data": value2})
    ...
    # Create step3 with same _unique_id as step1 but different meta value.
    step3 = WStep(
        inputs=(node1,),
        output=output_node,
        meta={"data": value2},  # different object
        _unique_id=step1._unique_id,  # same unique_id
    )
...
def test_wstep_set_operations_with_non_bool_meta():
    ...
    # Simulate the scenario: same WStep with non-bool-eq object in meta
    original_value = NonBoolEqType([[1, 2], [3, 4]])
    original_step = WStep(inputs=(node1,), output=output_node, meta={"df": original_value})

    # Simulate serialize/deserialize by creating new instance with same _unique_id
    deserialized_step = WStep(
        inputs=(node1,),
        output=output_node,
        meta={"df": NonBoolEqType([[1, 2], [3, 4]])},  # equivalent but distinct object
