...
- `await` cannot be evaluated inside a normal sync function without handing control to an event
  loop.
- `run(scheduled(prog))` is the entry point that allows this handoff when async handlers are
...
Only Python `await` syntax forces an external boundary, so doeff keeps one VM and one
entry point: `run(doexpr)` which accepts a single program argument and returns the raw value.
