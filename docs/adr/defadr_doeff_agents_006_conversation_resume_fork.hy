...
(defadr ADR-DOE-AGENTS-006
  ...
  :problem
    [(fact
     ...
     (fact
       "【2026-08-18 改訂の駆動問題】本番の resume は 1 本も完走していなかった(agentd.sqlite: resumed_from_session_id 非 NULL 105 行・done 0・2026-08-12〜08-17)。うち 91 行の一次原因は宿り先の消失: ACP は invocation 終了後に workspace(.acp/workspaces/inv_wi_*)を削除するが、resume failover は蘇生元行の work_dir(source copy 固定)をそのまま使う。sessionhost は work_dir の実在を検査せず tmux に渡し、tmux は不在 start-directory を黙って $HOME に差し替える — pane の shell prompt が `CA-…:~` で立つ。claude の --resume は cwd 鍵(projects/<mangle(realpath cwd)>)で会話を解決するため、transplant が正しく敷設した transcript ごと見えず『No conversation found with session ID』rc=1 で即死。起動 gate はこの画面の分類名を持たず 120s の予算切れまで待って no-agent-frame / prompt_undelivered で終端していた(91/91 の証拠 frame で確認 — 全件に『HOME の shell prompt + No conversation found』)。R4 の『source copy 固定が transcript 発見可能性の保証』は work_dir が実在する間しか成立しない、という前提の穴。"
       :evidence "~/.local/state/doeff/agentd.sqlite(resumed_from 非 NULL 105 行の output_snippet 全件照合 2026-08-18); fleet-stall-steward v143 起票 route-de257d16f2; sessionhost-steward v215")]
  :context
    [(interpretation
  ...
  :decision
    [(rule R1 "conversation identity は launch の成果物である: 全 kind impl は起動時に会話 identity を確定し session 行へ書く。claude = host が UUID を鋳造し --session-id で注入(boot 前に確定)。codex = boot 後に rollout を発見(Fs / Clock effect 経由 — impls は substrate-clean のまま、work_dir の cwd-match で特定し、rollout ファイル名の UUID を identity とする)。捕獲は BuildLaunch / PreLaunchSetup の kind 所有物理であり、捕獲失敗を黙って無 identity のまま running にしない — typed の degraded 観測(identity-unknown)として行に現れ、その session への resume 要求は typed 失敗になる。")
     ...
     (rule R7 "transcript transplant 前処理(2026-08-11 追加): cross-binding resume(binding 指定 ∧ source 行の effective_identity と異なる home)は、launch より前に per-kind impl が source home → binding home へ transcript を symlink で敷設する。物理は resume-physics.md 2026-08-11 プローブが凍結する: claude = projects/<mangle(canonical work_dir)>/<conversation.session_id>.jsonl + 周辺 artifact 3 対(sessions-index.json / session-env/<sid> / file-history/<sid> — dotfiles agentcli share.py の 4 対と同型。transcript のみ必須、周辺は best-effort)。codex = conversation_json の rollout_path を <target sessions root>/<sessions/ 以下同相対 path> へ link(native 形 = codex_home/sessions、二軸形 = profile_dir/sessions)— codex の resume 発見は常に自 home の sessions/ 走査であり rollout_path 絶対 path を CLI は辿らない。source transcript が実体としても symlink 解決先としても不在なら typed reject(transcript_not_discoverable)— 実 CLI の loud 失敗を row 生成前に前倒しする。transplant の path 物理は sessionhost impls/ の kind モジュール単一所有(law resume-physics-has-one-home の延長)であり、substrate プリミティブは FsLinkArtifact(share.py link_session_artifact の意味移植: source-missing / same-entity no-op / target-conflict no-op / linked / refused-by-container)のみ。【2026-09-22 改訂(設計 docs/design/symlink-verbs-fail-vocabulary-ZCN5BD): 語彙へ refused-by-container を追加し、戻り値を state / errno / detail を持つ FsSymlinkOutcome にした。器(file system)が敷設を断る形 — 権限 / 容量 / 読み取り専用 / 親の位置の実体 / 同拍の衝突 — は 必ず起きるのに 4 値の語彙に無く、断りは素の OSError として effect の外へ抜けていた(実測: 同じ敷設先へ 2 process が同拍で降りると 200 ラウンド中 184〜196 = 92〜98 % で負けた側が FileExistsError で落ちる。器の断り 3 形はすべて素通しだった)。敷設の物理も「見てから張る」から「symlink(2) を撃って FileExistsError を合図に samefile を読み直す」へ変えた — lock は足さない(家は process をまたぐ)。綴りの唯一の定義点は effects.hy の FS-SYMLINK-* 定数で、関数ごとに返しうる集合も FS-ENSURE-SYMLINK-STATES / FS-LINK-ARTIFACT-STATES として宣言する。必須 artifact (claude の transcript / codex の rollout)の refused-by-container は typed reject transplant_refused(R9)— transcript は在るので transcript_not_discoverable を当てない。】所有 profile は source 行の記帳事実(effective_identity)で既知 — registry 走査は持ち込まない。transplant は会話(transcript)だけを運び、auth は運ばない(auth は binding の責務 — claude の credential は CLAUDE_CONFIG_DIR ごとに Keychain 分離、と実測済み)。なお transplant は蘇生可能性の判定ではない(判定は R1 のとおり stored fact のみ)— 宿しの物理前処理であり、law revivability-is-stored-fact の kill / reap 決定面とは別層。")
     ...
     (rule R9 "typed error_code 語彙(2026-08-11 追加): resume / fork の admission reject は RpcHostError 系に整列した安定 error_code を wire に載せる — one_live_incarnation / identity_unknown / transcript_not_discoverable / kind_not_supported(effects.hy RESUME-ERR-* が語彙の家。koine の typed 文字列 code — adopt_target_not_found — と同系)。機械消費者(ACP Haskell client)は message substring ではなく code を照合する。既存 message 文言は後方互換で不変。program 層は ResumeRejected(code, message)で raise し、host dispatch が封筒の error_code へ写す。【2026-08-18 追補: 語彙へ workdir_not_found(R10)を追加。】【2026-09-22 追補: 語彙へ transplant_refused(R7 改訂)を追加 — transcript は解決できているのに 器が敷設を断った拍。既存 5 語はどれも『見つからない』の意味で、断りにその語を当てると運用者が 居もしない transcript を探しに行く。message には FsSymlinkOutcome の detail(syscall と errno)を載せる。】")
     (rule R10 "発注の物理前提は admission 事実(2026-08-18 追加 — 本番 resume 全滅 91/91 の根治): resume / fork は宿りを作る前に、(a) 宿り先 work_dir(R4 の source copy 固定)がディレクトリとして実在すること、(b) 会話 transcript が実 CLI の発見物理で解決可能であること、を検査し、不成立は副作用ゼロ(行不生成・tmux 不接触・symlink 不敷設)の typed reject にする。(a) 不成立 = workdir_not_found(新 code)。tmux は不在 start-directory を黙って $HOME に差し替えるため、この検査なしでは claude の cwd 鍵会話解決・trust pre-seed 鍵・transplant 敷設鍵のすべてが実際の cwd と食い違い、120s の予算切れ(no-agent-frame)という無名の死に化ける。(b) は R7 の transplant 検査を same-home にも広げる形 = transcript_not_discoverable(既存 code)。per-kind 物理: claude = projects/<mangle(canonical work_dir)>/<conversation.session_id>.jsonl の実在(symlink は解決先で判定)。codex = conversation_json に rollout_path が記帳されていればその実在(記帳が無い行は CLI の自 home sessions/ 走査で解決し得るため検査せず通す — 発見物理は home 走査で cwd 非依存)。session.launch も同じ (a) を全副作用より前に loud に検査する(resume は launch program を再利用するため backstop になり、fresh launch の同型崩壊も塞ぐ)。第 2 層として起動 gate は CLI の会話解決失敗画面(claude『No conversation found with session ID』/ codex『no rollout found for thread id』)を conversation-not-found class で予算を待たず即終端する(ADR-DOE-AGENTS-011 の閉語彙に追加 — TOCTOU と未知経路の備えであり、admission 検査の代替ではない)。解決できない再開発注を新規会話へ silent に縮退させる形は禁止 — resume の意味論(同一会話の継続)に反し、継ぐか否かの政策判定は呼び手(ACP engine — 方向宣言 resume-on-failover-a64d9d)の家である。なお work_dir を検査することは既存法 revivability-is-stored-fact と矛盾しない: あちらは kill / reap の決定面(行だけを読む)、こちらは宿しの発注面(壊れた宿りを作らない — R7 の前倒しと同じ層)。")
     (rule R11 "会話に付く durable な状態の置き場は会話 id で決まる(2026-09-20 追加 — 自動記憶の割れの根治): transcript / sessions-index / session-env / file-history は R7 の敷設で家をまたいで会話に従うが、Claude Code の**自動記憶(auto-memory)**だけがこの隊列に入っていなかった。記憶の置き場は CLAUDE_CONFIG_DIR の副作用(`<config dir>/projects/<潰した cwd>/memory/`)でしかなく、config dir は doeff が預かり所の account から組む(charter-with-grant)。⇒ 実効の鍵は『account × 作業ディレクトリ』という**誰も宣言していない組**になり、(a) profile を替えるたびに 1 つの会話が別の置き場へ割れ、(b) 同じ account に載った別々の会話が 1 つの置き場で相席し、(c) 同じ会話が隔離作業ツリーと主 checkout を行き来すると 3 つ目に割れる。実測 2026-09-20(会社 Mac・読みだけ): 家 30・同じ会話の連続する 2 手番が交わり 0 件の 2 つの置き場を読み、15 分前に書いた記憶が次の手番から存在しなかった。⇒ 置き場を宣言の 1 点にする: 判断は judgment.memory-home-of の純関数 1 つ(`<記憶の根>/<会話 id>` — account も作業ディレクトリも**引数に無い**)、値の宣言は AgentdSettings.memory_root の 1 欄(既定は homes-root と同じ導き方で、資格の家の**外** — 家の中に置くと account が替わった拍に置き場も替わり、直している欠陥がそのまま戻る)、器への綴りは impls/claude_code.hy の 1 か所。席の process を起こす腕**すべて**で charter に据え、貸与の有無に依らない(記憶は資格ではない)。⚠ 腕は 3 つではなく **4 つ**(2026-09-21 の追補・card acp:kanban-issue:ki-a40292ed30d9): launch / resume / rehydrate に加えて、**降りた process の続き**(headless.continue-headless-process)が在る。claude の headless は手番の終わりに必ず降りる(段 12 lane 12e #517)ので、会話の 2 手番目から先はこの 4 つ目しか通らない — 3 つで数えた註のまま置き場と冊を charter に載せたら、**普段の手番が全部**既定の置き場へ戻っていた(pool の pod で実測: ACP の行 44 冊に対し置き場 0 file・索引も無し)。4 つ目は charter を持たないので、その手番の送り(session.send の turn_charter)が同じ欄を運び、器の側の書き出しは腕で分かれない 1 点(interface effect HydrateMemoryHome → impls/claude_code.hy claude-hydrate-memory-home)に閉じる。⚠ 起こす腕は 2026-09-23 に 4 つ目が足された(card acp:kanban-issue:ki-c3aace97d825): **rebuild**(会話の記録から組んだ transcript を家へ置いて `claude --resume` で続ける腕)。この腕も charter を組む 1 点(judgment.incarnation-charter-of)と冊の水入れ(agentd.memory-files-for-turn)を他の起こす腕と同じく通り、SessionLaunch の charter で置き場を運ぶ — 腕を足す人が置き場の口を名乗らないと緑にならない形は、sessionhost_acp_memory_home_deftests.hy の MEMORY-HOME-CARRIER-OF(effects.NextArm からの反射)が持つ。⚠ 直し方を「行へ写す」に倒さない(policy.LAUNCH-FLAG-KEYS へ移す・行の launch_overlay に積む): 正本は ACP の行(法 ACP 575b1e)で、行の写しは第 2 の正本になって腐る。semgrep doeff-agents-turn-carried-key-must-not-become-a-launch-flag がその形を禁じる。deftest: sessionhost_charter_reaches_the_seat_deftests.hy test-declared-charter-keys-reach-the-seat-on-the-continue-arm・test-the-seat-writes-one-file-per-book-and-the-index-on-the-continue-arm、sessionhost_acp_memory_rows_deftests.hy test-a-warm-turn-carries-the-home-and-the-books-to-the-seat-too。実射で凍結した CLI 物理(2026-09-20): 効く設定の綴りは `autoMemoryDirectory` で、`memoryDir` は CLI 内部の property 名に過ぎず、渡しても黙って無視され既定の置き場が作られる。`--settings` の inline JSON は flagSettings として userSettings より先に効く(env `CLAUDE_COWORK_MEMORY_PATH_OVERRIDE` はさらに先だが、説明文を持つ公開の鍵である設定の綴りの方を使う)。解決は**最初に見つかった 1 つが勝つ**で、順は policySettings → flagSettings →(信用された作業ツリーでだけ)localSettings / projectSettings → userSettings(2.1.278 の実体を読んで確認 — 2026-09-20)。⇒ 採った形の上に立てるのは**管理者設定(policySettings)ちょうど**で、会社が `/Library/Application Support/ClaudeCode/managed-settings.json`(Linux は `/etc/claude-code/managed-settings.json`)に `autoMemoryDirectory` を配ると、置き場の指定は黙って上書きされる。症状(会話の記憶がまた割れる)が出た時の最初の確認点はこの file 1 つの在否で、2026-09-20 の会社 Mac では両方とも不在。なお repo に commit する `.claude/settings.json`(projectSettings)へ置く形は CLI が意図的に無視する(設定の説明文の逐語 『Ignored if set in projectSettings (checked-in .claude/settings.json) for security』)— 将来この宣言を設定 file へ移す改修は、この道だけは使えない。既存の置き場は 1 件も消さず移さない: 機械的な統合は著者を決められない(記憶 file に著者の欄が無く、1 つの家を複数の会話が同時に使っていた)ので、新しい置き場は空から始め、当たらなかった記憶は元の場所に残って読まれなくなるだけとする。codex は対象外(codex の作業状態は profile dir の側の話で、claude の auto-memory に当たる置き場を持たない)。機体をまたぐ継続(記憶の根はローカルの file system なので、会話が機体を移ると別の複製になる)は本 R の範囲外 — 別に立てる。")]
  :laws
    [(law conversation-outlives-incarnation
     ...
     (law resume-rejects-are-machine-readable
       :statement "resume_admission_reject => stable_error_code_on_wire(one_live_incarnation | identity_unknown | transcript_not_discoverable | kind_not_supported | workdir_not_found); consumers_never_match_message_substrings"
       :counterexamples
     ...
     (law resume-premises-verified-at-admission
       :statement "resume_or_fork_order => workdir_exists(source.work_dir) and transcript_discoverable(kind_physics); premise_failure_is_typed_reject_with_zero_side_effects; never_silently_degrade_to_new_conversation"
       :counterexamples
         [(counterexample "2026-08-16〜17 の本番全滅: ACP が削除済み workspace へ resume failover を発注し、sessionhost が work_dir 無検査で tmux に渡す → tmux が $HOME に黙って差し替え → claude の cwd 鍵会話解決が全滅(『No conversation found』)→ 120s 予算切れ no-agent-frame。91 発注 / 完走 0・16 profile 横断・binding 20/22 が cooldown 汚染(fleet-stall-steward v143)")
          (counterexample "same-home の resume を『transplant が発火しないから』と FS 検査ゼロで通す — 起動段で死んだ行(会話 identity は鋳造済み・transcript 未実体化)の resume が実 CLI の loud 死に落ちる")
          (counterexample "解決できない UUID の再開発注を黙って新規会話(--session-id 新鋳造)へ縮退させる — 呼び手は『会話が継がれた』と誤認し、文脈喪失が silent になる")
          (counterexample "120s の予算切れを短くするだけの変更で症状を治す — 発注は依然通り、壊れた宿り(failed 行 + tmux 残骸)が作られ続ける")])
     (law auto-memory-home-is-keyed-by-the-conversation
       :statement "conversation_scoped_durable_state_location => derived_from_conversation_id_only(memory_root + conversation_id); never_from_custody_account_or_credential_home_or_working_directory; stamped_on_every_waking_arm(launch | resume | rehydrate | rebuild)"
       :counterexamples
         [(counterexample "自動記憶の置き場を資格の家(CLAUDE_CONFIG_DIR = <homes-root>/claude/<account>)の副作用に任せる — 預かり所が別の account を貸した拍に置き場が変わり、1 つの会話の記憶が複数の家へ割れる(実測 2026-09-20: 家 30・同じ会話の連続する 2 手番が読んだ 2 つの置き場の交わりが 0 件)")
          (counterexample "記憶の根を資格の家の**中**(<homes-root>/claude/<account>/…)に置く — 会話 id で鍵を作っても account が替われば置き場が替わり、同じ欠陥が戻る")
          (counterexample "作業ディレクトリを鍵に混ぜる — 同じ会話が隔離作業ツリー(~/.worktrees/…)と主 checkout を行き来するたびに別の置き場が生える")
          (counterexample "起こす腕の一部(例 launch だけ)で置き場を据える — 腕が resume の手番だけ記憶が既定の置き場へ行き、誤りも条件も出ないまま剥がれる(2026-09-15 の添付の落ち方と同型)")
          (counterexample "器への綴りを検で確かめずに組む — `memoryDir` は CLI 内部の property 名で、設定として渡しても黙って無視され既定の置き場が作られる(実射 2026-09-20)")])]
  :enforcement
    [(defsemgrep no-resume-fork-argv-outside-kind-impl
     ...
     (defsemgrep no-auto-memory-dir-spelling-outside-kind-impl
       :languages ["generic"]
       :pattern "autoMemoryDirectory"
       :message "自動記憶の置き場を器へ渡す設定の綴りは sessionhost impls/ の kind モジュール単一所有(ADR-DOE-AGENTS-006 R11 / law auto-memory-home-is-keyed-by-the-conversation)。置き場を決めるのは judgment.memory-home-of の 1 点、運ぶのは charter の `memory_dir` 欄 — 他所でこの綴りを組むと、宣言していない第 2 の置き場ができる。impls/ 内と、綴りを pin する検の正当な出現は installed rule の paths 除外で扱う。"
       :bad ["settings = {\"autoMemoryDirectory\": f\"{config_dir}/projects/{cwd}/memory\"}"]
       :good ["(setv memory-dir (.get params \"memory_dir\"))"])]
