...
(defadr ADR-DOE-AGENTS-004
  ...
  :decision
    [(rule R1 "effect 語彙が interface: BuildLaunch / PreLaunchSetup / ClassifyPane / DeliverMessage / WireResultChannel(+substrate: SessionStore / Tmux / Fs / Env / Clock / Proc)。共有 policy program(monitor・bounded solicitation・judge・taxonomy・result 検証)は 1 本で、impl は書かない。substrate の Fs / Env は C2 で追加 — per-kind trust 物理(S11/S12: claude .claude.json temp+rename・codex config.toml・process env fallback)は kind 所有だが、impls/ は substrate-clean で生 IO を持てないため、FS / env 読みも effect 境界を通る。")
     ...
     (rule R13 "無人席の hook の据え付け(card acp:kanban-issue:ki-7b52bb76aa6e・設計 ki-f10ffeb77cab・2026-09-21): `session_hooks = inherit`(2026-08-18 route-c03fe34745)の委ね先 = config-dir の持ち主 = **doeff 自身** — 資格の預かり(custody)の後、席の config-dir は doeff が借りの拍に作る家(agentd-homes/<uuid>・settings.json 無し・複数席が共有)なので、委ねるだけでは hook 層が空で、dotfiles hooks.d が無人へ配ると宣言する 14 本が pod でも会社 Mac でも 1 本も呼ばれていなかった(実測 2026-09-21・cwd=$HOME の席だけ ~/.claude/settings.json を project 層として偶然読んでいた)。だから doeff が運ぶ: 機体の参加の宣言 `[agentd].claude_settings_file` が名指した席の settings file(中身と意味論は dotfiles `claude-hooks/seat-settings.json` = hooks.d の proxy 登録 1 枚・宿ごとの差はその宿の参加宣言〔Mac の toml / pod の ConfigMap〕)を、`build-claude-argv` の `--settings` の**唯一の合流点**で `autoMemoryDirectory` と合わせて 1 つの JSON に載せる(tui と headless の両方 — headless_argv は build-claude-argv を借りる)。既知の形 = kubelet 型(ADR-012 R51 の seat_env と同じ: 宣言 → JoinSpec → env DOEFF_AGENTD_CLAUDE_SETTINGS_FILE → 起動の拍ごとに読む → argv)。参加の門は 4 つで fail-closed(join.claude-settings-declaration-of + runtime.join_plan): (a) file が読める (b) JSON の object (c) doeff が置く鍵(impls/claude_code.hy CLAUDE-SETTINGS-OWNED-KEYS = disableAllHooks / autoMemoryDirectory)を含まない (d) session_hooks が inherit — 外れた宣言の agentd は理由つきで参加しない(黙って hook 無しの席を起こさない = 49b3549b の「安全 hook 全滅」を無言で作らない)。鍵の衝突は argv の合流点で fail-loud。宣言の無い機体の argv は 1 byte も変わらない(S13 の pin)。家(config-dir)には何も書かない — 複数席が共有する家に外から書く係は起動と競合し据え付けの定義点が 2 つになる(退けた案 B)。戻す手 = 宣言の 1 行を消して agentd を 1 回 roll。この鍵を知らない agentd は「宣言に無い鍵」で参加を断る(旧い機体が名指しを黙って落とさない)ので、宣言の着地は doeff のこの版を持つ機体の後。deftest: sessionhost_impls_deftests.hy test-claude-argv-merges-the-declared-seat-settings / test-claude-argv-refuses-seat-settings-that-collide-with-doeff-keys / test-claude-argv-without-declared-seat-settings-is-byte-identical、tests/test_sessionhost_acp.py test_join_claude_settings_file_* 、sessionhost_launch_deftests.hy test-launch-claude-carries-the-declared-seat-settings-file / test-launch-claude-refuses-an-unreadable-seat-settings-file。")]
  :laws
