...
(defadr ADR-DOE-AGENTS-004
  ...
  :laws
    [(law store-transactions-surface-the-original-failure
       :statement "for_all explicit transaction t on the store connection c: t is opened only by store.db-immediate-transaction (db-acquire-lease, db-heartbeat-once and db-release-lease call it and spell no BEGIN / ROLLBACK of their own); for_all failure e raised inside t (statement or COMMIT): the exception that leaves t is e itself (same type and message), c.in_transaction = False afterwards, and ROLLBACK is issued iff c.in_transaction held at the time of the failure"
       :counterexamples
