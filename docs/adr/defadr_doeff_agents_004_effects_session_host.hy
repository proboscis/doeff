...
(defadr ADR-DOE-AGENTS-004
  ...
  :decision
    [(rule R1 "effect 語彙が interface: BuildLaunch / PreLaunchSetup / ClassifyPane / DeliverMessage / WireResultChannel(+substrate: SessionStore / Tmux / Fs / Env / Clock / Proc)。共有 policy program(monitor・bounded solicitation・judge・taxonomy・result 検証)は 1 本で、impl は書かない。substrate の Fs / Env は C2 で追加 — per-kind trust 物理(S11/S12: claude .claude.json temp+rename・codex config.toml・process env fallback)は kind 所有だが、impls/ は substrate-clean で生 IO を持てないため、FS / env 読みも effect 境界を通る。")
     ...
     (rule R12 "herdr substrate(第二 substrate)の同一性と protocol 追随(2026-08-17 改訂・herdr 0.7.5 / protocol 17 実測。herdr 0.8.2 / protocol 20 で 2026-09-15 に撃ち直して同じ結論 — agent.start の必須欄 {name, kind, pane_id} と workspace.create の欄 {label, cwd, env, focus} は不変で、増えた pane.read の strip_ansi〔既定 true〕は format=ansi の読みには掛からない): (a) doeff 所有 session の同一性アンカーは workspace label(doeff が workspace.create {label, cwd, env} で所有 — protocol 17 は cwd/env を直接受け root pane がそのまま session pane)。生死 = label holder 集合の非空、帰属 = 全 holder の pane 和、kill = 全 holder の閉鎖、重複拒否 = 事前照会 + 同時作成の合意(herdr-new-session-verdict)。holder 集合は順序を持たない(workspace_id は創出順に非単調 — 実測 w3NZ → w3N0)。(b) herdr の agent.start は protocol 17 で {name, kind, pane_id} 必須の『既存 pane で herdr 自身が claude/codex 等を起動して検出を待つ』API に改形された — kind の語彙は herdr が起動する agent 種別で、TmuxNewSession(名前付き shell pane を作り、起動コマンドは doeff が後から送る)には意味論が合わない。よって『kind を送る』のではなく agent.start を呼ばない形が正しい追随(旧 payload + kind でも missing field pane_id — 実測)。(c) herdr の agent 名簿は session 同一性を担えない: pane 内で実 agent が起動すると ~2 秒で herdr の検出が外部の付けた名札を消す(実測 n=4)。名簿を読んでよいのは**外部命名席の実在確認**のただ 1 点 — `ai` 等が名簿に付けた席名(koine session.adopt の session_name)は doeff 所有 workspace を持たず label は repo 名(実測: 自席 = agent 名 s-7bfc5d5028 / label 'doeff')ため、label holder が無い名前に限り agent.get {target: name}(応答の name 完全一致)で has-session / session-pane-ids を答える(herdr-external-agent-pane-id-io — semgrep waiver marker 直下の 1 呼び出し)。kill・重複拒否・doeff 所有の同一性には決して使わず、外部命名席は名前で閉じない(doeff が作っていない席は観測のみ)。実測記録の正本は conformance/herdr-physics.md。")
     (rule R13 "無人席の hook の据え付け(card acp:kanban-issue:ki-7b52bb76aa6e・設計 ki-f10ffeb77cab・2026-09-21): `session_hooks = inherit`(2026-08-18 route-c03fe34745)の委ね先 = config-dir の持ち主 = **doeff 自身** — 資格の預かり(custody)の後、席の config-dir は doeff が借りの拍に作る家(agentd-homes/<uuid>・settings.json 無し・複数席が共有)なので、委ねるだけでは hook 層が空で、dotfiles hooks.d が無人へ配ると宣言する 14 本が pod でも会社 Mac でも 1 本も呼ばれていなかった(実測 2026-09-21・cwd=$HOME の席だけ ~/.claude/settings.json を project 層として偶然読んでいた)。だから doeff が運ぶ: 機体の参加の宣言 `[agentd].claude_settings_file` が名指した席の settings file(中身と意味論は dotfiles `claude-hooks/seat-settings.json` = hooks.d の proxy 登録 1 枚・宿ごとの差はその宿の参加宣言〔Mac の toml / pod の ConfigMap〕)を、`build-claude-argv` の `--settings` の**唯一の合流点**で `autoMemoryDirectory` と合わせて 1 つの JSON に載せる(tui と headless の両方 — headless_argv は build-claude-argv を借りる)。既知の形 = kubelet 型(ADR-012 R51 の seat_env と同じ: 宣言 → JoinSpec → env DOEFF_AGENTD_CLAUDE_SETTINGS_FILE → 起動の拍ごとに読む → argv)。参加の門は「宣言そのものの誤りだけを断る」(join.claude-settings-declaration-of + runtime.join_plan): (d) session_hooks が inherit(**file の在否に依らない** — 宣言 file の 2 行の食い違い)(b) 在るなら JSON の object (c) 在るなら doeff が置く鍵(impls/claude_code.hy CLAUDE-SETTINGS-OWNED-KEYS = disableAllHooks / autoMemoryDirectory)を含まない — 外れた宣言の agentd は理由つきで参加しない。⚠ **file が無いのは断る理由にしない**(2026-09-21 の訂正 — 依頼書 §10-2・受入 8): 宿の入口は「先端で揃えられない日は image の下限へ戻して立つ」正規の degrade を持ち(ACP acpcluster.yaml の init の註・実弾 2026-09-18)、その日の checkout に seat-settings.json は無い。そこで参加を断つと、その degrade が **pool 全体の capacity 0** に化ける(今日の欠陥より悪い)。だから不在は参加して**名乗る**: join の拍に 1 行(seat-settings-file-absent)・**席の起動ごとに** 1 行(launch.claude-settings-declaration — argv は名指しの無い日と byte 同一)・node の行の labels.seat-settings = present / missing(judgment.node-labels-of・契約の観測の欄を増やさないのは、ACP の kind の登録が古い間の書きが 400 で断られて観測ごと固まるため)。「黙って hook 無しの席を起こさない = 49b3549b の『安全 hook 全滅』を無言で作らない」は、断りではなくこの 3 つの名乗りが担保する。不在でも env には path を載せる(起動の拍ごとに読むので、宿の checkout が追いついた拍から hook が届く)。鍵の衝突は argv の合流点で fail-loud。宣言の無い機体の argv は 1 byte も変わらない(S13 の pin)。家(config-dir)には何も書かない — 複数席が共有する家に外から書く係は起動と競合し据え付けの定義点が 2 つになる(退けた案 B)。戻す手 = 宣言の 1 行を消して agentd を 1 回 roll。この鍵を知らない agentd は「宣言に無い鍵」で参加を断る(旧い機体が名指しを黙って落とさない)ので、宣言の着地は doeff のこの版を持つ機体の後。deftest: sessionhost_impls_deftests.hy test-claude-argv-merges-the-declared-seat-settings / test-claude-argv-refuses-seat-settings-that-collide-with-doeff-keys / test-claude-argv-without-declared-seat-settings-is-byte-identical、tests/test_sessionhost_acp.py test_join_claude_settings_file_* 、sessionhost_launch_deftests.hy test-launch-claude-carries-the-declared-seat-settings-file / test-launch-claude-refuses-a-broken-seat-settings-file / test-launch-claude-names-an-absent-seat-settings-file-and-still-opens-the-seat、tests/test_sessionhost_acp.py test_join_plan_admits_the_seat_settings_file_and_joins_when_it_is_absent / test_node_row_names_whether_the_seat_settings_file_was_there。")
     (rule R14 "器の host は**仕組みだけを持ち、方策を持たない** — wire に載せるのは観測した事実で、『いつまで / いつ』の判断は制御面が持つ(card acp:kanban-issue:ki-567f2dd6140f §3.1e・依頼 lt-ENGDDYZJPYG689RDH9133RN2FN・2026-09-22): 専用操作(cache ping)の送信先を温かい session として**いつまで保つか**の期限は、host の `cache-resident-retention` から制御面の `judgment.cache-resident-retention-of` の 1 点へ移した。host が名乗るのは wire の `cache_last_success_at_ms`(その session で最後に**成功した**専用操作の完了時刻・読みは cache_host_store.cache_last_success_at = 成功 receipt の index 読み 1 点・無ければ None)ちょうどで、適格の篩(claude / headless / multi_turn / running / 会話あり)も保持の予算(CACHE_RESIDENT_IDLE_MS)も持たない。⚠ 専用操作の**実行**(process を起こす・events を読む・receipt を永続する)は host の仕組みなので host に残る。根 = ACP の腕と器の所有を別 process に割る(ADR-DOE-AGENTS-012 R57)前提が『host は自分の外の方策を持たない』で、方策が host 側に残ると、その方策を変えるたびに器の process を落とすことになる。")
     (rule R15 "store の書き込みの失敗は**元の姿のまま**名乗り、続けば host は readiness を落とす(実弾 2026-09-25: k3s の agentd-pool-0 / -1 の状態の volume agentd-state(2Gi)が 100% になり、sqlite の COMMIT が `database or disk is full` で落ちた。lease の 3 関数は except で無条件に ROLLBACK したので、SQLite が自分で巻き戻した後の `cannot rollback - no transaction is active` が元の失敗を隠した。pool-1 は host が再起動を繰り返し、pool-0 は Running のまま書き込みが全部失敗し、readiness(socket の connect)は緑のまま手番を受け続けた)。(a) 明示の transaction は store.hy `db-immediate-transaction` の 1 点だけが張る — BEGIN IMMEDIATE → body → COMMIT、失敗した時は `conn.in_transaction` が真の時だけ ROLLBACK し、元の例外をそのまま出す(巻き戻しの失敗は note に添えて上書きしない)。(b) 保管の書き込みの失敗(SQLITE_FULL / IOERR / READONLY / CANTOPEN / CORRUPT / NOTADB — store_health.storage_failure_name)は store の actor の 1 点で数え、書き込みが 1 度通れば数え直す(level-triggered)。続けた回数が DOEFF_AGENTD_STORE_WRITE_FAILURE_LIMIT(既定 3)に届いたら `daemon.status` は ready = false と理由を名乗り、`doeff-sessionhost ready --socket <path>` は 1 で終わる。⚠ k8s の readinessProbe をこの口へ替えるのは配備の変更(agent-control-plane の deploy/acp-control/acpcluster.yaml)で、この版には含まない。")]
  :laws
    [(law store-transactions-surface-the-original-failure
       :statement "for_all explicit transaction t on the store connection c: t is opened and closed only by store.db-immediate-transaction (db-acquire-lease, db-heartbeat-once and db-release-lease call it and spell no BEGIN / ROLLBACK of their own; no other sessionhost code spells BEGIN / COMMIT / END / ROLLBACK / SAVEPOINT / RELEASE, commit, rollback or executescript on c — db-migrate and db-seed-from are the named exceptions; a body that closes t anyway is named: raised on success, noted on the failure it raises); t runs on the store actor's thread (an actor op that returns an unexecuted Program / effect is refused with TypeError and writes nothing); for_all failure e raised inside t (statement or COMMIT): the exception that leaves t is e itself (same type and message), c.in_transaction = False afterwards, and ROLLBACK is issued iff c.in_transaction held at the time of the failure"
       :counterexamples
         [(counterexample "except で無条件に ROLLBACK する(2026-09-25 まで): SQLITE_FULL で SQLite が自分で巻き戻した後の ROLLBACK が `cannot rollback - no transaction is active` を出し、log には rollback の文だけが残って disk full が読めない")
          (counterexample "ROLLBACK の失敗を握りつぶさずに raise する形 — 第 2 の例外が第 1 の例外を上書きする。巻き戻しの失敗は元の例外の note に添える")
          (counterexample "lease の関数ごとに try / ROLLBACK を書く形 — 3 か所の片方だけ直った日に同じ病が残る。transaction の型は 1 点")
          (counterexample "transaction の本体から、自分で commit する helper を呼ぶ形(2026-09-26 まで cache_receipt_put が conn.commit() を撃っていた)— 本体の途中で transaction が終わり、それまでの書き込みが確定する。成功の路は `cannot commit - no transaction is active` に化け、失敗の路は確定した書き込みが黙って残る(依頼 lt-A5KGD83R9HQJ2K172V61A6VMG9 の盲検 A)")
          (counterexample "actor の op が transaction の Program を実行せずに返す形 — Program は actor の thread の外で actor の connection を使って走り、直列化・書き込みの健康の数え・journal の合図を素通りする(同 盲検 B)")]
       :enforcement ["packages/doeff-agents/tests/test_sessionhost_store.py::test_lease_transaction_surfaces_disk_full_not_the_rollback"
                     "packages/doeff-agents/tests/test_sessionhost_store.py::test_lease_acquire_and_heartbeat"
                     "packages/doeff-agents/tests/test_sessionhost_store.py::test_lease_release_owner_idempotent_and_successor"
                     "packages/doeff-agents/tests/test_sessionhost_store.py::test_transaction_refuses_a_body_that_closes_the_transaction"
                     "packages/doeff-agents/tests/test_sessionhost_store.py::test_store_actor_refuses_an_op_that_returns_a_program"
                     "packages/doeff-agents/tests/test_sessionhost_transaction_owner.py::test_only_the_transaction_type_opens_and_closes_transactions"])
     (law host-readiness-falls-when-store-writes-keep-failing
       :statement "for_all host h with store actor a and limit L = DOEFF_AGENTD_STORE_WRITE_FAILURE_LIMIT (default 3): for_all op o run by a: storage_failure_name(error(o)) ≠ None ⇒ consecutive_failures += 1; o changed the store (total_changes advanced) without such a failure ⇒ consecutive_failures = 0; any other op leaves it unchanged; daemon.status(h).ready = (consecutive_failures < L) and, when false, not_ready_reason names the count, the first failing time and the last SQLite error name; `doeff-sessionhost ready --socket <path>` exits 0 iff the answer says ready = true (no answer, an error or a missing field ⇒ 1); constraint violations, program errors and lock waits are never counted"
       :counterexamples
         [(counterexample "readiness を socket の connect だけで測る(2026-09-25 の pool-0): 書けない host が答え続けて緑のまま手番を受け、全部が書き込みの段で落ちる")
          (counterexample "失敗の数を呼び手ごと(heartbeat・monitor・RPC)に数える — 数える点が割れ、どれか 1 つが通ると他の失敗が見えない。数える点は全部の op が通る actor の 1 点")
          (counterexample "読みの成功で数え直す — 容量が尽きても読みは通るので、書けないまま ready に戻る")
          (counterexample "IntegrityError(呼び手の不具合)でも readiness を落とす — 配置から外しても直らず、1 本の誤った呼びで pool の容量が減る")
          (counterexample "一度落ちたら再起動まで戻らない — 容量が戻っても手番を受けない。書き込みが 1 度通れば戻す(level-triggered)")]
       :enforcement ["packages/doeff-agents/tests/test_sessionhost_host.py::test_daemon_status_drops_readiness_when_store_writes_keep_failing"
                     "packages/doeff-agents/tests/test_sessionhost_host.py::test_store_health_counts_only_storage_failures"])
     ...
     (law the-host-reports-observations-and-the-control-plane-decides-the-deadlines
       :statement "for_all wire snapshot w of session s produced by the host: w.cache_last_success_at_ms = the completed_at of the newest succeeded cache-maintenance receipt of s whose reply used or wrote cache (None when there is none), computed by cache_host_store.cache_last_success_at alone and carrying no eligibility filter and no retention budget; the deadline for keeping s as a cache-ping target is judgment.cache-resident-retention-of(view) alone on the control-plane side (eligible iff agent_type = claude AND backend_kind = headless AND lifecycle = multi_turn AND status = running AND a conversation is present AND turn_ended_at is stamped; deadline = max(turn_ended_at, cache_last_success_at) + CACHE_RESIDENT_IDLE_MS declared once in acp.cache_operation), and sessions_to_retire reads that one point; the host keeps the mechanism (spawning the ping process, reading its events, persisting the receipt) and the control plane keeps every when/until"
       :counterexamples
         [(counterexample "host が `cache_retained_until_ms`(= 期限)を wire に載せる形(2026-09-22 より前): 保持の予算と適格の篩が host 側に住み、方策を変えるたびに器の process を落とすことになる — ACP の腕と器を別 process に割る前提(012 R57)が成り立たない")
          (counterexample "期限の計算を host と制御面の両方に置く形(片方を『念のため』残す): 2 つの座が予算の値で分岐し、idle の回収が機体ごとに違う時刻で起きる")
          (counterexample "適格の篩だけを host に残す形 — 篩は『どの session を保つか』の方策で、判断の座が 2 つに割れる(host は status の語を方策として読み始める)")
          (counterexample "host が方策を持たない代わりに制御面が cache_maintenance / cache_live ごと host へ降りる形: あちらは ACP の行(AcpGetRow / AcpPutStatus)と custody の借用に直結しているので、host に ACP の client と資格が要る = 分離の目的と正反対")
          (counterexample "cancel された器の receipt を host が消して『保持しない』を表す形: 観測を偽って方策を表している(receipt は起きた事実)。保たないのは制御面の判断で、事実はそのまま残る")]
       :enforcement ["docs/adr/defadr_doeff_agents_004_effects_session_host.hy::test-adr-doe-agents-004-the-host-reports-observations-and-the-plane-decides"
                     "packages/doeff-agents/tests/test_sessionhost_cache_residency.py::test_successful_ping_keeps_second_cycle_without_forging_turn_end"
                     "packages/doeff-agents/tests/test_sessionhost_cache_residency.py::test_cache_residency_reopens_existing_receipts_and_ignores_unknown"])
    ...
    [(defsemgrep no-new-agent-physics-in-rust-agentd
     ...
     (defsemgrep herdr-session-identity-not-agent-name
       ...
       [{"relative-path" "packages/doeff-agents/src/doeff_agents/sessionhost/substrate_herdr_label_ok.hy"
         "source" ";; ⚠ 語彙（deff / defk）はこの検体の争点ではない — rule の pattern は agent.get の regex で、定義の綴りに依存しない。実 code には deff のまま残る関数も在るが、ADR-DOE-HY-004 の台帳は文字列の中の字面も数えるので検体は defk で書く。\n;; 同一性は workspace label(集合)— kill は全 holder の閉鎖\n(defk herdr-kill-session-io [socket-path session-name]\n  (for [ws-id (herdr-label-workspace-ids-io socket-path session-name)]\n    (herdr-call socket-path \"workspace.close\" {\"workspace_id\" ws-id})))\n;; 唯一の実在確認 probe(waiver marker 直下)\n(defk herdr-external-agent-pane-id-io [socket-path name]\n  (try\n    ;; registry-existence-probe: 外部命名席の実在確認だけに許す(同一性・帰属・kill は label)\n    (setv result (herdr-call socket-path \"agent.get\" {\"target\" name}))\n    (except [e HerdrApiError]\n      (when (= e.code \"agent_not_found\") (return None))\n      (raise)))\n  (herdr-registry-agent-pane-id result name))\n"}])
     (deftest test-adr-doe-agents-004-the-host-reports-observations-and-the-plane-decides
       ;; law the-host-reports-observations-and-the-control-plane-decides-the-deadlines(R14)の
       ;; 構造の針: (a) 保持の予算の綴りは制御面の 1 点(acp.cache_operation)で、host 側の
       ;; module に 1 つも残らない。(b) 期限の判断は制御面の judgment.cache-resident-retention-of
       ;; の 1 点で、host 側に同名の座が無い。(c) host が載せる wire の欄は観測(…_last_success_at_ms)
       ;; ちょうどで、期限(…_retained_until_ms)を載せる行が残っていない。
       (import pathlib [Path])
       (setv root (. (Path __file__) parent parent parent))
       (setv host-dir (/ root "packages" "doeff-agents" "src" "doeff_agents" "sessionhost"))
       (defn text-of [path] (.read-text path :encoding "utf-8"))
       ;; (a) 予算の定義点は制御面に 1 つだけ。
       (setv budget "CACHE_RESIDENT_IDLE_MS = ")
       (setv plane (text-of (/ host-dir "acp" "cache_operation.py")))
       (assert (= (.count plane budget) 1) "保持の予算は制御面の 1 点(004 R14)")
       (for [leaf ["cache_host.hy" "cache_host_model.py" "cache_host_store.py" "host.hy" "store.hy"]]
         (setv body (text-of (/ host-dir leaf)))
         (assert (not-in budget body) f"host 側に保持の予算の第 2 の座が在る(004 R14): {leaf}"))
       ;; (b) 期限の判断は制御面の 1 点・host 側に同名の座が無い。
       (setv judgment (text-of (/ host-dir "acp" "judgment.hy")))
       (assert (= (.count judgment "(defk cache-resident-retention-of ") 1)
               "期限の判断は judgment の 1 点(004 R14)")
       (assert (not-in "(defk cache-resident-retention " (text-of (/ host-dir "cache_host.hy")))
               "host 側に期限の判断が残っている(004 R14)")
       ;; (c) host が載せるのは観測の欄ちょうど。
       (setv host-body (text-of (/ host-dir "host.hy")))
       (assert (in "(get wire \"cache_last_success_at_ms\")" host-body)
               "host が観測の欄を載せていない(004 R14)")
       (assert (not-in "\"cache_retained_until_ms\"" host-body)
               "host が期限の欄を載せている(004 R14)")
       ;; 読みの 1 点(成功 receipt の index 読み)も host 側の 1 つだけ。
       (setv store (text-of (/ host-dir "cache_host_store.py")))
       (assert (= (.count store "def cache_last_success_at(") 1) "観測の読みは 1 点(004 R14)")
       (assert (not-in "def cache_retained_until(" store) "旧い期限の読みが残っている(004 R14)"))
