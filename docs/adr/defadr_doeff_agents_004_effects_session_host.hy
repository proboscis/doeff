  ...
  :laws
    [(law protocol-physics-has-one-home
     ...
     (law the-host-reports-observations-and-the-control-plane-decides-the-deadlines
       ...
       :counterexamples
         [(counterexample "host が `cache_retained_until_ms`(= 期限)を wire に載せる形(2026-09-22 より前): 保持の予算と適格の篩が host 側に住み、方策を変えるたびに器の process を落とすことになる — ACP の腕と器を別 process に割る前提(012 R56)が成り立たない")
