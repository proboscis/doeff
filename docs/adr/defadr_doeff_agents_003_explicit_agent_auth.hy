...
(defadr ADR-DOE-AGENTS-003
  ...
  :decision
    [(rule R1 "codex を起動する session.launch(agent_type=codex または command が codex を含む)は、session_env の CODEX_HOME か command 内の CODEX_HOME= を必須とする。無ければ tmux 作業前に明確なエラーで拒否する。")
     ...
     (rule R4 "agentd は profile の中身(どのアカウントか)を検証しない — それは呼び手(project/namespace の宣言、ACP では ADR 0039)の責務。agentd が強制するのは『明示されていること』のみ。【2026-09 改訂(従量課金の便 lane B): **account は検めないが、課金の階級の宣言と binding kind の一致は検める。** 旧条項のままでは、家の中に従量課金の資格を入れれば黙って通る道が開いている(検査が env の名しか見ないため — 2026-09-15 の設計の実測)。黙って通る道は、運用主の不変条件『従量課金を系に入れない』を人の注意力だけで守る形で、課金の階級が型に現れないので監査点も消える。よって定額の kind(claude-code / codex)の家に従量課金の宣言(claude = settings.json の apiKeyHelper / env.CLAUDE_CODE_USE_VERTEX / env.ANTHROPIC_API_KEY / env.ANTHROPIC_AUTH_TOKEN・codex = auth.json の **非空の** OPENAI_API_KEY)が在れば PreLaunchSetup で typed reject(直し方 2 択 = kind を *-metered にするか、家から宣言を外す)。⚠ 判定は『欄が在り、かつ中身が非空』— codex の CLI は定額の login でも OPENAI_API_KEY を null で書き出すので、欄の有無で判じると定額の家を全部断る(実測 2026-09-15: 運用主の家 3 つとも欄は在る・中身は空。着地前にこの実測で運用主の配備に宣言 0 件を確かめてから出した)。settings.json / auth.json の不在・破損は **通す**(今日と同じ。破損は CLI 自身が loud に落ちる)— 破損は warning に 1 行。検めるのは階級の宣言ちょうどで、どの account の login かは依然として見ない。law no-default-auth-profile は不変。従量課金の側の契約は ADR-DOE-AGENTS-004 R9 と law metered-billing-is-declared-by-kind-and-allowed-by-host-policy。deftest: sessionhost_impls_deftests.hy test-claude-pre-launch-rejects-metered-declaration-in-subscription-home・test-codex-pre-launch-rejects-api-key-in-subscription-auth-file。】")]
  :laws
