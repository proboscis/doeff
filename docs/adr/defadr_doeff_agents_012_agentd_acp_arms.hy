  ...
  :enforcement
    [(deftest test-adr-doe-agents-012-history-thins-tool-items-before-dropping-turns
     ...
     (deftest test-adr-doe-agents-012-provider-limit-is-the-containers-cause-read-as-a-field
       ...
       (assert (is-not (re.search r"\(import doeff_agents\.sessionhost\.impls\.markers \[is-api-limit-refusal api-limit-scope-of api-limit-resets-at\b" headless-text) None)
               "限度の判定・範囲と理由・戻りの時刻は markers の 1 点を呼ぶ(R33・2026-09-23 / 09-24 追補)")
       ;; R33 追補 2026-09-23: 範囲(limit_scope)と戻りの時刻(limit_resets_at_ms)も器の側が当てて cause の欄に載せ、
       ;; 制御面はその欄を読む(文を読まない)。
       ;; R33 追補 2026-09-24: 分類は markers の表の 1 点 api-limit-scope-of(文 → #(範囲 理由))で、器の側の 1 点がその
       ;; 答えを cause の limit_scope / limit_reason に載せる。group の上限 $N の族は範囲 unknown(器は決めない)。
       (assert (in "(api-limit-scope-of verdict.detail)" headless-text) "範囲と理由を器の側で当てていない(R33 追補 2026-09-24)")
       (assert (in ":limit-reason" headless-text) "理由を cause の欄に載せていない(R33 追補 2026-09-24)")
       ...
       ;; R33 追補 2026-09-24: 範囲と理由の分類は markers の 1 点(定義はちょうど 1 つ)で、group の上限の族の行は
       ;; 範囲 unknown を返す(文が窓を名乗らない — 器は口座全体と決めない)。
       (assert (= (len (re.findall r"\(def[fk] api-limit-scope-of \[detail\]" markers-text)) 1)
               "範囲と理由の分類の 1 点が markers に無い / 2 つある(R33 追補 2026-09-24)")
       (assert (in "(setv API-LIMIT-SCOPE-UNKNOWN \"unknown\")" markers-text) "範囲の語 unknown が markers に無い(R33 追補 2026-09-24)")
       ;; 表の行の中身(group の上限 → unknown・session → account・Fable → model)と器 → 条件の一周の欄の形は反例の検が撃つ
       ;; (行の字面は焼かない — law spelling-pins-proxy-for-shape)。
       (setv impls-tests (.read-text (/ (. (Path __file__) parent parent parent) "packages" "doeff-agents" "tests" "sessionhost_impls_deftests.hy") :encoding "utf-8"))
       (for [name ["test-api-limit-scope-of-the-org-cap-family-is-unknown-because-the-text-names-no-window"
                   "test-the-containers-limit-words-stay-inside-the-condition-vocabulary"
                   "test-the-containers-limit-cause-becomes-a-condition-that-names-scope-reason-and-model"]]
         (assert (in (+ "(deftest " name) impls-tests) f"R33 追補 2026-09-24 の反例の検が無い: {name}"))
