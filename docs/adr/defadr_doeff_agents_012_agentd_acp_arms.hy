...
(import pathlib [Path])
(import hy)
(import hy.models [Expression List Object Sequence Symbol])
(import doeff [run])
...
(defn #^ str form-use-head [#^ Object node]
...
(defn #^ str top-form-name-of [#^ Object form]
...
(defn #^ list child-parent-pairs [#^ Object node]
...
(defn #^ bool assignment-target? [#^ Object parent #^ Object child]
...
(defn #^ set carriers-of [#^ list pairs #^ str bound #^ Expression introducer]
  ...
  :enforcement
    [(deftest test-adr-doe-agents-012-history-thins-tool-items-before-dropping-turns
     ...
     (deftest test-adr-doe-agents-012-headless-first-turn-carries-the-mail
       ...
       (for [#(name path) [#("judgment.hy" (/ ACP-DIR "judgment.hy")) #("agentd.hy" (/ ACP-DIR "agentd.hy"))]]
