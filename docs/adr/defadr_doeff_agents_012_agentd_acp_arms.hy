  ...
  :enforcement
    [(deftest test-adr-doe-agents-012-history-thins-tool-items-before-dropping-turns
     ...
     (deftest test-adr-doe-agents-012-provider-limit-is-the-containers-cause-read-as-a-field
       ...
       (assert (is-not (re.search r"\(import doeff_agents\.sessionhost\.impls\.markers \[is-api-limit-refusal api-limit-names-a-model api-limit-resets-at\b" headless-text) None)
               "限度の判定・範囲・戻りの時刻は markers の 1 点を呼ぶ(R33・2026-09-23 追補)")
       ;; R33 追補 2026-09-23: 範囲(limit_scope)と戻りの時刻(limit_resets_at_ms)も器の側が当てて cause の欄に載せ、
       ;; 制御面はその欄を読む(文を読まない)。
       (assert (in "(api-limit-names-a-model verdict.detail)" headless-text) "範囲を器の側で当てていない(R33 追補 2026-09-23)")
