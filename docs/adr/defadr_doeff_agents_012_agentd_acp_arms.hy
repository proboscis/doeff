...
;;; 突合、不一致は参加しない(会社 profile の API 呼び出しは会社所有の機体だけ)。
...
(setv JOB-STEP-TURN-END-READERS
      {"job-step-of" "温かい手番は終わったか — turn-end を**返す**唯一の点"
       ...
       "turn-produced-nothing-condition-of" "終わった温かい手番が本文のための出力を 1 本も出さなかったか(返さず**読む**だけ — turn-end でなければ何も言わない・依頼 lt-R79KYTYMJH4ZT9X4KHWKCD23KB D1)"})
...
(defadr ADR-DOE-AGENTS-012
  ...
  :decision
    [(rule R1 "agentd の出口は ACP(GET /api/resources・POST /api/events・GET /api/watch/stream・POST /api/streams)と custody(POST /lease/*)だけ。agora の台帳 API(/api/state・turn-jobs・seat-*・headless・agmsg)の語を sessionhost の source に置かない。")
     ...
     (rule R51 "席へ運ぶ env は機体の**参加の宣言**が名乗り、charter の 1 点で重ねる(段 12・agora-redesign #520・既知の形 = **kubelet 型**: runner が node 局所の宣言の表を workload の env へ具現化し、workload は runner 自身の env を 1 bit も継がない〔k3s の config.yaml → kubelet → 容器の env / systemd の EnvironmentFile=〕): 宣言 file の [agentd].seat_env(改行区切りの `NAME=value` の 1 つの文字列 — 表〔dict〕にしないのは `declared-values-of` の不変条件『宣言 file の値は全部文字列』を割らないため・旧い agentd はこの鍵を『宣言に無い鍵』で断って**参加しない** = fail-closed)を join.seat-env-of が解き(空行と `#` の行は飛ばす・`=` の無い行と名の形〔`[A-Za-z_][A-Za-z0-9_]*`〕の外と同じ名の 2 度は ValueError・値は最初の `=` の後を**逐語**で引用の剥がしも `${}` の展開もしない = 第 2 の置換の言語を作らない)、JoinSpec.seat_env → env DOEFF_AGENTD_SEAT_ENV(同じ綴りで運び読み直しも同じ 1 点)→ AgentdSettings.seat_env → 起こす手番の charter.session_env(judgment.charter-with-seat-env の 1 点・incarnation-charter-of が呼ぶ)。(1) **参加の門は家の関所を 1 点のまま再利用する**: 解いた表は launch の口 / session.send の口と**同じ** policy.session-env-admission-error(verb `join.seat_env`)を通し、断りの文が返れば ValueError で参加しない —— その docstring の逐語「運ぶ口が増えても判定を並行実装しない」がこの選択の根拠で、`seat_env` は文字どおり『運ぶ口が増えた』場合。⚠ **第 2 の関所を join.hy へ写さない**(写しても既存の針は 2 つの口しか数えていないので**赤くならない** —— 緑を根拠にしない。針は「呼びが 3 か所ちょうど」へ育てる)。関所の線は**名の形ちょうど**で、値は 1 byte も見ない: binding 所有(CLAUDE_CONFIG_DIR / CODEX_HOME)と従量課金(正規化して `_API_KEY` で終わる名 + 既知の別名)。⇒ **贈り物が 1 つ**: CLAUDE_CONFIG_DIR が断られるので、「会話の身元が必ず勝つ」が charter の順序だけでなく**門の側からも**守られる(node 全体の宣言で会話ごとの家を上書きする道が構造で塞がる)。加えて会話の身元の名(effects.CONVERSATION-ID-ENV / SEAT-OPENER-ENV)は join.seat-env-of が断る。⚠ 札は家の既定の置き場への **file の mount** が今日の形で、この口が運ぶのは宛先ちょうど —— ただし**札の path の綴り**(`*_TOKEN_FILE` / `AGORA_BORROWER_KEY_PATH`)を関所が通すか塞ぐかは**この便の外**(依頼者の裁定待ち・出荷済みの線を広げるかどうかは session_env の口の判定も同時に変える話なので、別便で決める)。(2) **順序は契約**: incarnation-charter-of は charter-with-seat-env を charter-with-conversation-env の**直前**に 1 度呼ぶ — 会話の身元が必ず後に勝つので、機体の宣言は会話の名乗りを偽れない((1) の門と二重の守り)。(3) **継承の名簿は 1 語も開けない**(R30 (4) の**継承**の禁はそのまま): policy.SPAWN-INHERITED-ENV-KEYS / substrate_headless の headless-spawn-env は触らない — 席が機体の env を丸ごと継ぐ形(実弾 #95)は締めたまま、**宣言された対だけ**が charter を通って届く。⚠ ただし R30 (4) の**条文**はこの便で改める: 末尾の「ACP_* / … は届かない」はこの口が開いた日に偽になるので、「**継承では**届かない」と「**宣言された名は charter を通って**届く」の 2 文へ分けた(改めないと、この便が直しに来た『宣言が偽になる』病を doeff 側の条文で再発させる)。(4) **doeff は語彙を知らない**: 宛先の綴りの定義点は宣言の側(k8s の Service と読み手 dotfiles)で、doeff-agents の src は席向けの名を 1 つも綴らない(semgrep doeff-agents-does-not-spell-seat-facing-env)— 写しが第 2 の既定へ育たない。(5) **宣言は process の起動時に読む**: 走っている agentd は新しい表を拾わないので、宣言を変えたら pool を roll する(宣言 file の指紋 declaration-sha256 が変わるので node の行から読める)。走っている席の env は差し替えられない(charter は incarnation ごとに組む)ので、受入の実射は roll の**後に新しく起こした手番**で測る。")]
  :laws
  ...
  :enforcement
    [(deftest test-adr-doe-agents-012-history-thins-tool-items-before-dropping-turns
     ...
     (deftest test-adr-doe-agents-012-live-events-are-polled-within-50ms-while-watched
       ...
       (for [name ["observe-job-fast" "stream-job-fast" "stream-push"]]
       ...
       (assert (any (gfor line (defk-body agentd-lines "stream-job-fast") (in "(ClockNowMs)" line)))
     ...
     (deftest test-adr-doe-agents-012-join-is-one-command-and-one-decision-point
       ...
       (assert (= (run (install (ownership-preflight (Ownership :grade "company" :proof file-proof))
                                [local.dispatch]))
                  (Ownership :grade "company" :proof file-proof)))
       (setv (get local.probe-answers file-proof) "proboscis-mbp")
       (setv moved False)
       (try
         (run (install (ownership-preflight (Ownership :grade "company" :proof file-proof))
                       [local.dispatch]))
         (except [ValueError]
           (setv moved True)))
     ...
     (deftest test-adr-doe-agents-012-turn-credential-rides-the-turn
       ...
       (setv admission-callers
             (+ (lfor line host-lines :if (in "(session-env-admission-error " line) line)
                (lfor line launch-lines :if (in "(session-env-admission-error " line) line)
                (lfor line join-lines :if (in "(session-env-admission-error " line) line)))
       (assert (= (len admission-callers) 3)
               (+ f"session_env の関所の呼びが 3 か所ちょうどでない(実測 {(len admission-callers)}: {admission-callers !r})"
                  " —— 運ぶ口が増えても判定を並行実装しない(policy.hy の docstring・R30 (3)・R51 (1))"))
       (setv headless-lines (code-lines (/ SESSIONHOST-DIR "headless.hy")))
     ...
     (deftest test-adr-doe-agents-012-seat-facing-env-is-declared-once-and-loses-to-the-conversation
       ...
       (assert (= (len (lfor line join-lines :if (in "(session-env-admission-error (dict pairs) \"join.seat_env\")" line) line)) 1)
               "宣言の口は家の関所を通す(R51 (1))")
       (setv policy-lines (code-lines (/ SESSIONHOST-DIR "policy.hy")))
       (for [word ["seat-env-credential-shaped-offenders" "SEAT-ENV-CREDENTIAL-SHAPED"]]
         (for [line policy-lines]
           (assert (not-in word line)
                   (+ f"seat_env 専用の資格の判定が policy に生えた({word})—— 関所は 1 点の再利用で、"
                      f"線を広げるなら session_env の口の判定も同時に変わる(R51 (1)): {line}"))))
       ;; ⚠ join.hy が docstring で線の中身(CLAUDE_CONFIG_DIR / `_API_KEY`)を読み手へ書くのは可 ——
       ;; 禁じているのは**判定を写すこと**で、それは上の「呼びが 3 か所ちょうど」と
       ;; 「policy に seat_env 専用の判定が無い」の 2 本が撃つ。
