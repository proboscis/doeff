  ...
  :enforcement
    [(deftest test-adr-doe-agents-012-history-thins-tool-items-before-dropping-turns
     ...
     (deftest test-adr-doe-agents-012-dropped-history-turns-leave-a-headline
       ...
       (assert (any (gfor line fold-body (in "(+ [header headline] (cut blocks dropped-turns None))" line))) "見出しは残した手番の前(R34)")
     ...
     (deftest test-adr-doe-agents-012-summaries-are-triggered-at-turn-end-and-folded-first-on-rehydrate
       ...
       (assert (any (gfor line judgment-lines (.startswith line "(defk rehydrate-history-of [conversation-id messages source exclude budget fetched summaries]")))
       ...
       (assert (any (gfor line fold-body (in ":summary-regions (len summaries)" line))) "HistoryFold が要約の数を運んでいない(R38)")
       (setv agentd-lines (code-lines (/ ACP-DIR "agentd.hy")))
       ...
       (for [needle ["(summaries-for settings subject)" "(summary-floor-of summaries)" "(record-turns-for settings subject floor)"]]
         (assert (any (gfor line history-body (in needle line))) f"再開の読みが要約を先に読んでいない(R38): {needle}"))
       ...
       (for [needle ["METRIC_SUMMARIZE_TRIGGERS_TOTAL = \"agentd_summarize_triggers_total\"" "HISTORY_SUMMARY_KIND = \"要約\""
                     "SUMMARIZE_JOB_ID_PREFIX = \"aj-summary-\"" "    summary_regions: int = 0"]]
       ...
       (for [name ["test-summaries-fold-first-in-region-order-and-drop-before-raw-under-the-budget"
