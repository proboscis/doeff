  ...
  :enforcement
    [(deftest test-adr-doe-agents-012-history-thins-tool-items-before-dropping-turns
     ...
     (deftest test-adr-doe-agents-012-capture-gone-is-terminal-and-ticks-do-not-share-failure
       ...
       (assert (= (len (lfor line agentd-lines :if (in "(except [e IO-FAILURES]" line) line)) 8)
               "縁は heartbeat・profile の観測・受け・割り込みの配達・job ごと・verify の命令ごと(段 12 lane 12a R36)・spool の送り・spool の書きの 8 つ(ADR-DOE-AGENTS-012 R9・R18・R21・R36・段 9f lane 9f-2)")
