...
(import doeff_agents.sessionhost.acp.effects
        [AGENT-JOB-KIND AGENT-JOB-NAMESPACE AGORA-KINDS-NAMESPACE AcpGetRow AcpPutStatus AcpRow AgentdSettings
         ...
         RECORD-BODY-FIELDS
         ProbeAnswer ProfileHome ProfileUsage ProfileUsageUnavailable TURN-RECORD-END-ONLY-MATERIALS TURN-RECORD-KIND
         TURN-RECORD-ROW-MATERIALS TurnEndOnlyMaterial UsageWindow])
...
(import doeff_agents.sessionhost.acp.judgment [capture-verdict job-step-of mail-turn-text-of node-seat-env-of record-due resume-params-of
                                               status-object-of stream-capability-of-backend turn-record-ended-status
                                               turn-record-sweep-verdict wait-seconds-for])
...
;;
;; 盲検の反例(依頼 lt-N23MQ5ZMSM6KCDKCB0G2RTFCAH・docs/design-checks/ の同名の dir)を受けた形:
;; (A) どの引数が「手番の終わりにしか数えられない材料」かの分類を検の中の表に持つと、呼び先が材料を足すたびに
;;     検も直す必要があり、表に無い材料(cache-observation)は黙って緑だった ⇒ 分類は呼び先の側の宣言
;;     (effects.TURN-RECORD-END-ONLY-MATERIALS / TURN-RECORD-ROW-MATERIALS)に置き、検は宣言を読んで
;;     全ての引数が分類されていることを確かめる(分類の無い引数は赤)。
;; (B) 「書く行が AcpGetRow の答え」だけを見ると、一覧の行で判断して status を組み、読み直しは書く直前の
;;     generation のためだけ、という形が緑だった ⇒ 判断(turn-record-sweep-verdict の record)・status の材料
;;     (turn-record-ended-status の status = status-object-of の row)・書く status(turn-record-ended-status の答え
;;     ちょうど・書く前に他の式へ渡さない)の出所を、書く行と同じ読み直した行まで辿る。
...
(defclass [(dataclasses.dataclass :frozen True)] SweepCallees []
  "巡回の検が読む呼び先の役の組: 閉じる status を組む judgment.turn-record-ended-status・行を鍵で読む effect AcpGetRow・
   status を書く effect AcpPutStatus・行の status を読む judgment.status-object-of・閉じるかを判じる judgment.turn-record-sweep-verdict。
   反例の検が作り物の呼び先を渡せるように、本物の定義から導いた値も作り物の値も同じ形で運ぶ。"
  #^ CalleeRoles ended
  #^ CalleeRoles get-row
  #^ CalleeRoles put-status
  #^ CalleeRoles status-object
  #^ CalleeRoles verdict)


(defclass [(dataclasses.dataclass :frozen True)] MaterialClasses []
  "turn-record-ended-status の引数の材料の分類(本物 = effects の宣言 2 つ): end-only = 手番の終わりの書きだけが中身を渡す
   材料の役 → TurnEndOnlyMaterial(材料を持たない書き手が渡す空の値と理由)・row = 行から読める材料の役。"
  #^ dict end-only
  #^ frozenset row)
...
(defk real-sweep-callees []
  {:pre []
   :post [(: % SweepCallees)]}
  "本物の巡回を読むため、呼び先の役を本物の定義(defk の引数の並び・effect の dataclass の欄)から導いて組む。"
  (<- ended CalleeRoles (defk-roles turn-record-ended-status))
  (<- get-row CalleeRoles (effect-roles AcpGetRow))
  (<- put-status CalleeRoles (effect-roles AcpPutStatus))
  (<- status-object CalleeRoles (defk-roles status-object-of))
  (<- verdict CalleeRoles (defk-roles turn-record-sweep-verdict))
  (SweepCallees :ended ended :get-row get-row :put-status put-status :status-object status-object :verdict verdict))


(defk module-globals-of [kleisli]
  {:pre [(: kleisli types.FunctionType)]
   :post [(: % dict)]}
  "defk の本体の呼び先の名を、その defk を定義した module の名で引くため(巡回が呼ぶ補助の defk を辿る)。"
  (vars (get sys.modules kleisli.__module__)))
...
(defk head-names-in [forms]
  {:pre [(: forms list)]
   :post [(: % list)]}
  "form の列の中の全ての呼びの頭の名(書いた順・重複なし)を拾うため(巡回が呼ぶ補助の defk を辿る入口)。"
  (setv out [])
  (setv pending (list (reversed forms)))
  (while pending
    (setv node (.pop pending))
    (when (and (isinstance node hy.models.Expression) node (isinstance (get node 0) hy.models.Symbol)
               (not-in (str (get node 0)) out))
      (.append out (str (get node 0))))
    (when (isinstance node hy.models.Sequence)
      (.extend pending (reversed (list node)))))
  out)


(defk symbol-in? [form name]
  {:pre [(: form hy.models.Object) (: name hy.models.Symbol)]
   :post [(: % bool)]}
  "名 name が form の中(入れ子を含む)に在るかを判じるため(分割の束ねの的・関数の引数の中の名を見つける)。"
  (setv found False)
  (setv pending [form])
  (while (and pending (not found))
    (setv node (.pop pending))
    (cond
      (and (isinstance node hy.models.Symbol) (= node name)) (setv found True)
      (isinstance node hy.models.Sequence) (.extend pending (list node))))
  found)


;; 束ねの形: 値を 1 つ束ねる頭(<- / setv — 的が名ちょうどなら値の式が出所)・束ねの列を持つ頭(let は値の式が出所・
;; for / with は繰り返しや文脈の答えなので形そのものが出所)・中に名の在る列で束ねる内包の頭・引数の並びを持つ関数の頭。
(setv VALUE-BINDING-HEADS #("<-" "setv"))
(setv VECTOR-BINDING-HEADS {"let" True "for" False "with" False})
(setv COMPREHENSION-BODY-SIZES {"lfor" 1 "gfor" 1 "sfor" 1 "dfor" 2})
(setv FUNCTION-PARAM-INDEX {"fn" 1 "defn" 2 "defk" 2})


(defk bindings-of [forms name]
  {:pre [(: forms list) (: name hy.models.Symbol)]
   :post [(: % list)]}
  "名 name の値がどの式から来たかを確かめるため、form の列の中で name を束ねる所を全部、出所の form で返す。
   (<- name expr) / (<- name T expr) / (setv name expr …) / (let [name expr] …) は expr。分割の的(#(name …))・
   for / with / 内包の繰り返しの名・関数の引数は、束ねた形そのもの(1 つの呼びの答えではない — 出所を確かめる検は赤)。"
  (setv out [])
  (for [head VALUE-BINDING-HEADS]
    (<- binds list (calls-in forms head))
    (for [bind binds]
      (setv pairs (if (= head "<-")
                      (if (in (len bind) #(3 4)) [#((get bind 1) (get bind -1))] [])
                      (lfor i (range 1 (- (len bind) 1) 2) #((get bind i) (get bind (+ i 1))))))
      (for [[target value] pairs]
        (<- nested bool (symbol-in? target name))
        (cond
          (= target name) (.append out value)
          (and nested (isinstance target #(hy.models.Tuple hy.models.List))) (.append out bind)))))
  (for [[head value-is-source] (.items VECTOR-BINDING-HEADS)]
    (<- binds list (calls-in forms head))
    (for [bind binds]
      (when (and (> (len bind) 1) (isinstance (get bind 1) hy.models.List))
        (setv vector (list (get bind 1)))
        (setv i 0)
        (while (< (+ i 1) (len vector))
          (setv target (get vector i))
          (<- nested bool (symbol-in? target name))
          (when (or (= target name) (and nested (isinstance target #(hy.models.Tuple hy.models.List))))
            (.append out (if (and value-is-source (= target name)) (get vector (+ i 1)) bind)))
          (setv i (+ i 2))))))
  (for [[head body-size] (.items COMPREHENSION-BODY-SIZES)]
    (<- binds list (calls-in forms head))
    (for [bind binds]
      (setv i 1)
      (while (< (+ i 1) (- (len bind) body-size))
        (setv target (get bind i))
        (<- nested bool (symbol-in? target name))
        (when (or (= target name) (and nested (isinstance target #(hy.models.Tuple hy.models.List))))
          (.append out bind))
        (setv i (+ i 2)))))
  (for [[head index] (.items FUNCTION-PARAM-INDEX)]
    (<- fns list (calls-in forms head))
    (for [fn-form fns]
      (when (> (len fn-form) index)
        (<- nested bool (symbol-in? (get fn-form index) name))
        (when nested
          (.append out fn-form)))))
  out)
...
(defk sources-of [forms form]
  {:pre [(: forms list) (: form hy.models.Object)]
   :post [(: % list)]}
  "引数に渡した form の出所を得るため: 名なら form の列の中で束ねる所の全部、式ならその式ちょうど。"
  (if (isinstance form hy.models.Symbol)
      (do (<- found list (bindings-of forms form))
          found)
      [form]))
...
;; 巡回の検が役の名で読む呼び先の役(呼び先の契約の名)。改名は「役が無い」の赤で名乗る。
(setv SWEEP-READ-ROLES
      [#("turn-record-ended-status" "ended" "status")
       #("AcpGetRow" "get-row" "key")
       #("AcpPutStatus" "put-status" "row")
       #("AcpPutStatus" "put-status" "status")
       #("status-object-of" "status-object" "row")
       #("turn-record-sweep-verdict" "verdict" "record")])


(defk material-class-violations [ended-roles materials]
  {:pre [(: ended-roles CalleeRoles) (: materials MaterialClasses)]
   :post [(: % list)]}
  "turn-record-ended-status の引数が全て材料の分類(effects の宣言)に 1 度ずつ在り、分類に在る役が全て引数に在るかを
   確かめるため(分類は呼び先の側の知識 — 検は分類を持たず宣言を読む)。"
  (setv out [])
  (setv declared (| (frozenset materials.end-only) materials.row))
  (for [role (sorted declared)]
    (when (not-in role ended-roles.names)
      (.append out f"材料の宣言の役 {role} が呼び先 turn-record-ended-status の定義に無い(契約の変更 — 宣言と検査も直す)")))
  (for [role (sorted (& (frozenset materials.end-only) materials.row))]
    (.append out f"材料の宣言の役 {role} が 2 つの分類の両方に在る(effects の宣言を 1 つにする)"))
  (for [role ended-roles.positional]
    (when (not-in role declared)
      (.append out (+ f"呼び先 turn-record-ended-status の役 {role} に材料の分類が無い"
                      "(effects.TURN_RECORD_END_ONLY_MATERIALS か TURN_RECORD_ROW_MATERIALS に宣言する)"))))
  (for [role (sorted (- ended-roles.names (frozenset ended-roles.positional)))]
    (when (not-in role declared)
      (.append out (+ f"呼び先 turn-record-ended-status の役 {role} に材料の分類が無い"
                      "(effects.TURN_RECORD_END_ONLY_MATERIALS か TURN_RECORD_ROW_MATERIALS に宣言する)"))))
  out)
...
(defk other-uses-of [forms name put-roles]
  {:pre [(: forms list) (: name hy.models.Symbol) (: put-roles CalleeRoles)]
   :post [(: % list)]}
  "巡回が組んだ status の名 name が、束ねる所と AcpPutStatus の status の役の外で使われている form を拾うため
   (書く前に足す・書き換える・他の式へ渡すと、書くのは turn-record-ended-status の答えちょうどでなくなる)。"
  (setv out [])
  (setv pending (list (reversed forms)))
  (while pending
    (setv node (.pop pending))
    (when (isinstance node hy.models.Sequence)
      (<- is-put bool (call-of? node "AcpPutStatus"))
      (<- is-bind bool (call-of? node "<-"))
      (<- is-setv bool (call-of? node "setv"))
      (setv passed {})
      (when is-put
        (<- passed dict (call-roles put-roles node)))
      (setv items (list node))
      (for [[i child] (enumerate items)]
        (when (= child name)
          (setv allowed (or (and is-put (is (.get passed "status") child))
                            (and is-bind (= i 1))
                            (and is-setv (= (% i 2) 1))))
          (when (not allowed)
            (.append out node))))
      (.extend pending (reversed items))))
  out)


(defk helper-writes-in [forms resolve visited]
  {:pre [(: forms list) (: resolve dict) (: visited set)]
   :post [(: % list)]}
  "巡回の本体が呼ぶ補助の defk(入れ子も)の中に AcpPutStatus の呼びが在れば、その defk の名を返すため
   (検が読めるのは巡回の本体だけ — 記録を閉じる書きを補助へ移すと検の外になる)。resolve = 呼び先の名 → 値。"
  (setv out [])
  (<- heads list (head-names-in forms))
  (for [head heads]
    (setv target (.get resolve (hy.mangle head)))
    (when (and (isinstance target types.FunctionType) (is-not (body-of target) None) (not-in (id target) visited))
      (.add visited (id target))
      (<- inner list (defk-forms target))
      (<- writes list (calls-in inner "AcpPutStatus"))
      (when writes
        (.append out head))
      (<- inner-globals dict (module-globals-of target))
      (<- deeper list (helper-writes-in inner inner-globals visited))
      (.extend out (gfor name deeper f"{head} → {name}"))))
  out)
...
(defk sweep-violations [callees materials sweep-body resolve]
  ...
  "R49 の巡回の本体(form の列)を呼び先と引数の役で読み、違反を失敗文の列で返す(空 = 緑)。確かめること:
   ...
   (6) turn-record-sweep-verdict の record は書く行(読み直した行で判断する)
   (7) 巡回が呼ぶ補助の defk の中で AcpPutStatus を撃たない。resolve = 巡回の本体の呼び先の名 → 値(本物 = agentd の module)。"
  (setv out [])
  (<- class-wrongs list (material-class-violations callees.ended materials))
  (.extend out class-wrongs)
  (setv by-field {"ended" callees.ended "get-row" callees.get-row "put-status" callees.put-status
                  "status-object" callees.status-object "verdict" callees.verdict})
  (setv missing-roles [])
  (for [[callee field role] SWEEP-READ-ROLES]
    (when (not-in role (. (get by-field field) names))
      (.append missing-roles role)
      (.append out f"呼び先 {callee} の定義に役 {role} が無い(契約の変更 — 検査も直す)")))
  (when missing-roles
    (return out))
  ;; (2) 手番の終わりだけの材料。
  ...
  (setv written-rows [])
  (for [call put-calls]
    (<- put-passed dict (call-roles callees.put-status call))
    ...
    (if (not row-is-read)
        ...
        (do (.append written-rows row)
            (for [read row-sources]
              (<- get-passed dict (call-roles callees.get-row read))
              (setv key (.get get-passed "key"))
              (<- key-shown str (form-text key))
              (<- key-sources list (sources-of sweep-body key))
    ...
    (setv status (.get put-passed "status"))
    (<- status-shown str (form-text status))
    (setv status-sources [])
    (when (isinstance status hy.models.Symbol)
      (<- status-sources list (bindings-of sweep-body status)))
    (<- status-is-ended bool (all-calls-of? status-sources "turn-record-ended-status"))
    (if (not status-is-ended)
        (.append out f"巡回が書く status に {status-shown} を渡している(R49 — turn-record-ended-status の答えちょうどではない)")
        (do (<- uses list (other-uses-of sweep-body status callees.put-status))
            (for [use uses]
              (<- use-shown str (form-text use))
              (.append out (+ f"巡回が組んだ status {status-shown} を書く前に {use-shown} で使っている"
                              "(R49 — 書くのは turn-record-ended-status の答えちょうど)"))))))
  ;; (5) 閉じる status の材料は書く行から読む。
  (for [call ended-calls]
    (<- passed dict (call-roles callees.ended call))
    (setv material (.get passed "status"))
    (<- material-shown str (form-text material))
    (<- material-sources list (sources-of sweep-body material))
    (<- from-status-object bool (all-calls-of? material-sources "status-object-of"))
    (setv from-written False)
    (when from-status-object
      (setv from-written True)
      (for [source material-sources]
        (<- object-passed dict (call-roles callees.status-object source))
        (setv from-written (and from-written (in (.get object-passed "row") written-rows)))))
    (when (not from-written)
      (.append out (+ f"巡回が閉じる status を読み直した行から組んでいない(R49 — turn-record-ended-status の status に {material-shown}"
                      " を渡している — 書く行の status-object-of の答えではない)"))))
  ;; (6) 閉じるかの判断は読み直した行で。
  (<- verdict-calls list (calls-in sweep-body "turn-record-sweep-verdict"))
  (when (not verdict-calls)
    (.append out "巡回が turn-record-sweep-verdict を呼んでいない(R49 — 閉じるかの判断は judgment の 1 点)"))
  (for [call verdict-calls]
    (<- verdict-passed dict (call-roles callees.verdict call))
    (setv judged (.get verdict-passed "record"))
    (<- judged-shown str (form-text judged))
    (when (not-in judged written-rows)
      (.append out (+ f"巡回が読み直した行で判断していない(R49 — turn-record-sweep-verdict の record に {judged-shown}"
                      " を渡している — 書く行ではない)"))))
  ;; (7) 補助の defk の中の書き。
  (<- helpers list (helper-writes-in sweep-body resolve (set)))
  (for [helper helpers]
    (.append out (+ f"巡回が呼ぶ {helper} の中で AcpPutStatus を撃っている(R49 — 記録を閉じる書きは巡回の本体に置く"
                    " — 検が読めるのは巡回の本体だけ)")))
  out)


;; 反例の検(test-adr-doe-agents-012-sweep-roles-are-read-from-the-callee-not-the-spelling)の材料: 作り物の呼び先の
;; 引数の並び・作り物の材料の分類・作り物の巡回の本体。本物の綴りも本物の宣言も読まない — 本物が変わっても反例の検が
;; 黙って別の物を撃たないように、形は全部ここに書く。
(setv SWEEP-CASE-ENDED-PARAMS "[status usage entries conditions [cache-observation None] [responses None]]")
(setv SWEEP-CASE-CALLEE-PARAMS
      {"get-row" "[key]"
       "put-status" "[row status]"
       "status-object" "[row]"
       "verdict" "[record pair node-name live-nodes in-flight-ids]"})
(setv SWEEP-CASE-END-ONLY
      {"usage" (TurnEndOnlyMaterial :empty None :reason "消費の和は手番の終わりの 1 回")
       "entries" (TurnEndOnlyMaterial :empty #() :reason "手番の出来事の見出しは手番の中の追記と終わりの書きだけが持つ")
       "cache_observation" (TurnEndOnlyMaterial :empty None :reason "cache の観測は手番の終わりの 1 回")
       "responses" (TurnEndOnlyMaterial :empty None :reason "応答ごとの消費は手番の終わりの 1 回")
       "model_usage" (TurnEndOnlyMaterial :empty None :reason "model ごとの消費は手番の終わりの 1 回")})
(setv SWEEP-CASE-TODAY-ENDED "(<- ended dict (turn-record-ended-status record-status None #() pair-conditions))")

;; 今日の巡回と同じ骨組み(一覧 → 記録の鍵で読み直す → 読み直した行で判断する → 読み直した行から閉じる status を組む → 書く)。
;; %名% の所を例ごとに差し替える。
(setv SWEEP-CASE-TEMPLATE "(<- listed tuple (AcpRunningTurnRecords))
(for [row listed]
  ...
  (when (isinstance %RECORD% AcpRow)
    (<- verdict str (turn-record-sweep-verdict %VERDICT-ROW% pair settings.node-name live-nodes mine))
    (when (= verdict TURN-RECORD-SWEEP-END)
      (<- record-status dict (status-object-of %STATUS-ROW%))
      %ENDED-CALL%
      %AFTER-ENDED%
      (<- wrote (| Written Conflict Refused) (AcpPutStatus :row %ROW% :status %PUT-STATUS%)))))")


(defclass [(dataclasses.dataclass :frozen True)] SweepRoleCase []
  "反例の検の 1 例: 名・期待する失敗文の列(空 = 緑)と、作り物の呼び先の引数の並び・材料の分類・巡回の本体の差し替え。
   役の読みが正常例を緑に、違反例を意図した文の赤にすることを 1 例ずつ確かめるため。record = 読み直した行の名
   (書く行・判断する行・status を読む行の既定)・helpers = 巡回が呼ぶ作り物の補助の defk の名。"
  #^ str label
  #^ tuple expected
  (setv #^ str params SWEEP-CASE-ENDED-PARAMS)
  (setv #^ tuple end-only #("usage" "entries" "cache_observation" "responses"))
  (setv #^ tuple row-roles #("status" "conditions"))
  (setv #^ str ended-call SWEEP-CASE-TODAY-ENDED)
  (setv #^ str record "record")
  (setv #^ (| str None) row None)
  (setv #^ str key "key")
  (setv #^ (| str None) verdict-row None)
  (setv #^ (| str None) status-row None)
  (setv #^ str put-status "ended")
  (setv #^ str extra "")
  (setv #^ str after-ended "")
  (setv #^ tuple helpers #()))


(defk sweep-case-close [record ended]
  {:pre [(: record AcpRow) (: ended dict)]
   :post [(: % str)]}
  "反例の検の作り物の補助: 巡回の本体から書きを移した形(巡回が呼ぶ defk の中で AcpPutStatus を撃つ)を作るため。
   反例の検は本体の form を読むだけで、この defk を走らせない。"
  (<- (AcpPutStatus :row record :status ended))
  "closed")


(defk synthetic-sweep-forms [case]
  {:pre [(: case SweepRoleCase)]
   :post [(: % list)]}
  "反例の検が役の読みを試すため、今日の巡回と同じ骨組みの本体を、例の差し替えを当てて form の列で作る。"
  (setv text SWEEP-CASE-TEMPLATE)
  (for [[slot value] [#("%EXTRA%" case.extra)
                      #("%RECORD%" case.record)
                      #("%KEY%" case.key)
                      #("%VERDICT-ROW%" (or case.verdict-row case.record))
                      #("%STATUS-ROW%" (or case.status-row case.record))
                      #("%ENDED-CALL%" case.ended-call)
                      #("%AFTER-ENDED%" case.after-ended)
                      #("%ROW%" (or case.row case.record))
                      #("%PUT-STATUS%" case.put-status)]]
    (setv text (.replace text slot value)))
  (list (hy.read-many text)))


(defk synthetic-case-inputs [case]
  {:pre [(: case SweepRoleCase)]
   :post [(: % tuple)]}
  "反例の検の 1 例の入力(作り物の呼び先の役・材料の分類・巡回の本体・補助の defk の引き先)を組むため。"
  (<- ended CalleeRoles (roles-of-params (hy.read case.params)))
  (setv others {})
  (for [[field params] (.items SWEEP-CASE-CALLEE-PARAMS)]
    (<- roles CalleeRoles (roles-of-params (hy.read params)))
    (setv (get others field) roles))
  (setv callees (SweepCallees :ended ended :get-row (get others "get-row") :put-status (get others "put-status")
                              :status-object (get others "status-object") :verdict (get others "verdict")))
  (setv materials (MaterialClasses :end-only (dfor role case.end-only role (get SWEEP-CASE-END-ONLY role))
                                   :row (frozenset case.row-roles)))
  (<- forms list (synthetic-sweep-forms case))
  (setv resolve (dfor name case.helpers (hy.mangle name) (get (globals) (hy.mangle name))))
  #(callees materials forms resolve))


(setv SWEEP-ROLE-CASES
      [(SweepRoleCase :label "正常例 今日の巡回" :expected #())
       (SweepRoleCase :label "正常例 呼び先が行から読める役を足し(分類を宣言し)、呼びが渡す"
                      :params "[status usage entries conditions [cache-observation None] [responses None] [trace None]]"
                      :row-roles #("status" "conditions" "trace")
       ...
       (SweepRoleCase :label "正常例 呼び先が usage の前に役を足す(役の位置は呼び先の定義から導く)"
                      ...
                      :row-roles #("status" "conditions" "node")
                      :ended-call "(<- ended dict (turn-record-ended-status record-status record-node None #() pair-conditions))"
                      :expected #())
       (SweepRoleCase :label "正常例 呼び先が手番の終わりだけの材料を足し(分類を宣言し)、巡回は渡さない"
                      :params "[status usage entries conditions [cache-observation None] [responses None] [model-usage None]]"
                      :end-only #("usage" "entries" "cache_observation" "responses" "model_usage")
                      :expected #())
       (SweepRoleCase :label "正常例 呼びの中の註と折れ方"
                      :ended-call "(<- ended dict (turn-record-ended-status record-status
                                     None   ; usage は書かない
                                     #()    ; entries は無い
                                     pair-conditions))"
                      :expected #())
       (SweepRoleCase :label "正常例 keyword の既定値つき引数"
                      ...
                      :expected #())
       (SweepRoleCase :label "正常例 読み直した行の局所変数の改名" :record "fresh-record" :expected #())
       (SweepRoleCase :label "違反例 usage に値"
       ...
       (SweepRoleCase :label "違反例 keyword で responses"
       ...
       (SweepRoleCase :label "違反例 entries に値"
                      :ended-call "(<- ended dict (turn-record-ended-status record-status None entries pair-conditions))"
                      :expected #("巡回が entries に entries を渡している(R49 — 手番の出来事の見出しは手番の中の追記と終わりの書きだけが持つ)"))
       (SweepRoleCase :label "違反例 keyword で cache-observation"
                      :ended-call (+ "(<- ended dict (turn-record-ended-status record-status None #() pair-conditions"
                                     " :cache-observation swept-cache))")
                      :expected #("巡回が cache_observation に swept-cache を渡している(R49 — cache の観測は手番の終わりの 1 回)"))
       (SweepRoleCase :label "違反例 呼び先が手番の終わりだけの材料を足し(分類を宣言し)、巡回が渡す"
                      :params "[status usage entries conditions [cache-observation None] [responses None] [model-usage None]]"
                      :end-only #("usage" "entries" "cache_observation" "responses" "model_usage")
                      :ended-call (+ "(<- ended dict (turn-record-ended-status record-status None #() pair-conditions"
                                     " :model-usage swept-model-usage))")
                      :expected #("巡回が model_usage に swept-model-usage を渡している(R49 — model ごとの消費は手番の終わりの 1 回)"))
       (SweepRoleCase :label "違反例 呼び先に分類の無い役を足す"
                      :params "[status usage entries conditions [cache-observation None] [responses None] [model-usage None]]"
                      :expected #(#[[呼び先 turn-record-ended-status の役 model_usage に材料の分類が無い(effects.TURN_RECORD_END_ONLY_MATERIALS か TURN_RECORD_ROW_MATERIALS に宣言する)]]))
       (SweepRoleCase :label "違反例 呼び先の役の改名(usage → consumption)"
                      ...
                      :expected #("材料の宣言の役 usage が呼び先 turn-record-ended-status の定義に無い(契約の変更 — 宣言と検査も直す)"
                                  #[[呼び先 turn-record-ended-status の役 consumption に材料の分類が無い(effects.TURN_RECORD_END_ONLY_MATERIALS か TURN_RECORD_ROW_MATERIALS に宣言する)]]))
       (SweepRoleCase :label "違反例 呼びを消す" :ended-call "(setv ended {})"
                      :expected #("巡回が turn-record-ended-status を呼んでいない(R49 — 閉じる記録の status を組まない)"
                                  "巡回が書く status に ended を渡している(R49 — turn-record-ended-status の答えちょうどではない)"))
       (SweepRoleCase :label "違反例 一覧の行で書く" :row "row"
                      :expected #("巡回が読み直した image で書いていない(R49 — AcpPutStatus の row に row を渡している — 鍵で読んだ AcpGetRow の答えではない)"
                                  "巡回が閉じる status を読み直した行から組んでいない(R49 — turn-record-ended-status の status に record-status を渡している — 書く行の status-object-of の答えではない)"
                                  "巡回が読み直した行で判断していない(R49 — turn-record-sweep-verdict の record に record を渡している — 書く行ではない)"))
       (SweepRoleCase :label "違反例 記録ではない鍵で読む" :key "job-id"
                      :expected #("巡回が鍵で読み直していない(R49 — 書く行を読む AcpGetRow の key に job-id を渡している — turn-record-key-of の答えではない)"))
       (SweepRoleCase :label "違反例 一覧の行で判断する" :verdict-row "row"
                      :expected #("巡回が読み直した行で判断していない(R49 — turn-record-sweep-verdict の record に row を渡している — 書く行ではない)"))
       (SweepRoleCase :label "違反例 一覧の行から閉じる status を組む" :status-row "row"
       ...
       (SweepRoleCase :label "違反例 組んだ status に書く前に足す" :put-status "(| ended {\"usage\" usage-total})"
                      :expected #("巡回が書く status に (| ended {\"usage\" usage-total}) を渡している(R49 — turn-record-ended-status の答えちょうどではない)"))
       (SweepRoleCase :label "違反例 組んだ status を書く前に書き換える" :after-ended "(setv (get ended \"usage\") usage-total)"
                      :expected #("巡回が組んだ status ended を書く前に (get ended \"usage\") で使っている(R49 — 書くのは turn-record-ended-status の答えちょうど)"))
       (SweepRoleCase :label "違反例 分割で束ねた行で書く" :row "copied" :verdict-row "copied" :status-row "copied"
                      :extra "(setv #(copied _) #(row None))"
                      :expected #("巡回が読み直した image で書いていない(R49 — AcpPutStatus の row に copied を渡している — 鍵で読んだ AcpGetRow の答えではない)"
                                  "巡回が閉じる status を読み直した行から組んでいない(R49 — turn-record-ended-status の status に record-status を渡している — 書く行の status-object-of の答えではない)"
                                  "巡回が読み直した行で判断していない(R49 — turn-record-sweep-verdict の record に copied を渡している — 書く行ではない)"))
       (SweepRoleCase :label "違反例 巡回が呼ぶ補助の defk の中で書く" :after-ended "(<- (sweep-case-close record ended))"
                      :helpers #("sweep-case-close")
                      :expected #("巡回が組んだ status ended を書く前に (sweep-case-close record ended) で使っている(R49 — 書くのは turn-record-ended-status の答えちょうど)"
                                  "巡回が呼ぶ sweep-case-close の中で AcpPutStatus を撃っている(R49 — 記録を閉じる書きは巡回の本体に置く — 検が読めるのは巡回の本体だけ)"))
       (SweepRoleCase :label "違反例 盲検 B の形(一覧の行で判断して status を組み、読み直しは書く直前の generation だけ)"
...
(defadr ADR-DOE-AGENTS-012
  ...
  :decision
    [(rule R1 "agentd の出口は ACP(GET /api/resources・POST /api/events・GET /api/watch/stream・POST /api/streams)と custody(POST /lease/*)だけ。agora の台帳 API(/api/state・turn-jobs・seat-*・headless・agmsg)の語を sessionhost の source に置かない。")
     ...
     (rule R11 "headless backend: host の backend の閉語彙は tmux | herdr | headless。headless の器は専用の program(sessionhost/headless.hy)と substrate(headless_effects.hy の Headless* → substrate_headless.hy → headless_process.py。語彙は 2026-09-26 に共有の effects.hy から backend の側の headless_effects.hy へ移した — agora-redesign #624: 共有の語彙を読むだけの module〔policy.hy → shell.py → doeff-agents の headless の adapter〕が旧い器の状態機械を import しないため)で、host.hy は backend の分岐だけ(RPC の語彙は同じ意味・session.interrupt を足す)。stdin / stdout の作法と手番の判断は headless_protocol.py の Dialogue / turn_verdict の純関数 1 点。claude の print mode の argv の家は impls/headless_argv.hy ちょうどで、semgrep doeff-agents-no-claude-print-mode の除外もその家と headless の substrate / program / 検だけ。admission と identity の準備は tui の launch と共有する(launch.hy admit-launch / prepare-launch-workspace)。【2026-09-25 付け替え(agora-redesign #604): claude の print mode の起動の家は、session host の外では doeff-claude-code(packages/doeff-claude-code/src/doeff_claude_code/argv.hy)ちょうど。doeff-agents の headless の handler(handlers/headless.hy)はその公開 effect への adapter で、起動の引数・子 process・pid を持たず session host を import しない。sessionhost の headless の器(impls/headless_argv.hy・headless_protocol.py・headless_process.py)は、本番の agentd がこの経路で走る間(削除計画 (b) の手順 5-1 で agent の実行側を切り替えるまで)だけ残る旧い家で、session host の外の新しい code はそれを呼ばない。検 = test-adr-doe-agents-012-print-mode-has-one-home(sessionhost の外の doeff-agents の code に print mode の綴りが無い)と packages/doeff-agents/tests/test_headless_adapter.hy。】")
     ...
     (rule R-a-lender-outage-is-a-lost-attempt-fd0f "**預かり所が機械の語で『今は貸せない・時間で晴れる』と言った断りは失った試みの記録で、接続が答えない断りは預かり所の途絶の窓の間だけ待つ**(card acp:kanban-issue:ki-fd0f3b234a38・設計 v2 §10.1〔herdr-hud docs/design-checks/lt-BJ284EVT30M939AQKG68Y2S9GF/design.md〕・R53 / R54 の追補・実弾 2026-09-23 = 預かり所の worker personal の heartbeat が 3 回 90 秒以上途切れ、その間の借りの 503 worker-unreachable を CredentialUnavailable〔別の運び手なら通る〕で Ended にしたので、配達が運び手の組み直し 2 回を使い切り郵便 22 通が failed になった — 503 は 4 台の機体で同じ答えで、機体を避ける組み直しでは晴れない): (1) **断りは処理ステージと機械の語を運ぶ** — LeaseRefused は断った処理ステージ(stage = lease〔master の貸与の口〕| redeem〔答えの workerUrl の worker の引換の口〕)と本文の code・why を運び、写すのは handlers.CustodyHttp._borrow の 1 点(以前は error の文だけを写し、処理ステージと code を捨てていた)。引換に失敗した拍は、その直前に master が出した貸与を POST /lease/{leaseId}/revoke で返す(接続が答えない間のやり直しで貸与の行を積まない)— 返せなくても断りの語は変えず、返せなかった id を断りに載せて呼び手が log に名乗る。(2) **class は 2 つ増え、順は time → lender → unanswered → nobody → another-carrier**(判定は judgment.custody-refusal-verdict-of の 1 点のまま): time は**貸与の処理ステージの** 409 + hold だけ(引換の 409 = 引換券の使用済みは master の hold が付いていても time にしない — 設計 F3)/ lender = 預かり所の契約が時間で晴れると宣言する (処理ステージ, status, code, why) の組(judgment.custody-lender-transient? — 今日の契約では lease/503/worker-unreachable/{heartbeat-stale, worker-store-unreadable} と redeem/503/master-unreachable〔why を読まない〕)→ 手番を Ended にせず CredentialLenderUnreachable{status True, reason = 預かり所の逐語, attempt, at, account, nodeRow, stage, code, why} を足して phase を離す(R53 の CredentialLeaseHeld と同じ形・REFUSED_ATTEMPT_CONDITION_TYPES に入れて同じ試みを起こさない・読み手は監督〔agora-controllers lifecycle と ACP Scheduling の lostRunner〕— 契約 scheduling.json の supervision.lostReasons credential-lender-unreachable〔数えない〕・retry.backoffOriginRecords)/ unanswered = 接続が答えない(status 0・どちらの処理ステージでも)で、同じ試みで最初に答えなかった刻から窓 W の中 → 行に何も書かない。(3) **時間で晴れる組と窓 W の定義点は預かり所の契約 1 か所** — doeff は契約の節 lenderAvailability の写し(effects と同じ dir の custody_lender_availability.json・形 custody.lender-availability.copy.v1 = agora-controllers の写しと同じ)を package に同梱し、表(effects.CUSTODY_LENDER_TRANSIENT)と W(effects.CUSTODY_UNANSWERED_WINDOW_MS = workerStaleSeconds + workerHeartbeatSeconds = 120 秒)をその写しから導く(直書きしない)。写しと正本(source.commit の custody-api.json#/lenderAvailability)の一致は package の検が撃ち、正本の checkout(env CUSTODY_CHECKOUT → ~/repos/custody)が無い機体では名指しで赤(黙って skip しない)。やり直しの間隔 15 秒(effects.CUSTODY_UNANSWERED_RETRY_MS)は契約に無い runner の数で、検が固定するのは『窓より短い』の関係だけ。(4) **unanswered の待ちは runner の memory(AgentdState.unanswered_borrows — 条件の型も契約も増やさない・設計 T8)**: 項 = (job, attempt) → 最初に答えなかった刻・次にやり直す刻・claim の拍の起こし方。行は claim で既に Running + sessionHandle なので、項の在る行は拾い直し(recover-job)に渡さず、刻の前の拍は借りを撃たず行に何も書かない。刻が来た拍は受けの腕が行を読み直し(judgment.list-mode-for は刻の来た項が在れば静かな拍でも window を読む)、claim の拍と同じ start-claimed を撃ち直す(第 2 の起こし口を作らない)。窓を過ぎても答えなければ今日どおり another-carrier(預かり所は worker を生きていると言い続けている = この機体の道の故障 → 機体を避けて組み直す)。項を捨てる点は 2 つ: 判定が unanswered 以外を答えた拍(judgment.unanswered-borrows-after)と、受けの拍に行が自分の Running の同じ試みでなくなった拍(judgment.unanswered-borrows-kept — 終わった・取り下げ・置き直し・記録を持った)。別の試みは窓を最初から数える。(5) ⚠ **既知の食い違い(設計 T8 の読み)**: 設計は『runner が入れ替われば窓を最初から数える』と書くが、claim は借りより先に着く(Running + sessionHandle)ので、同じ機体で agentd が入れ替わると次の process は memory に無い自分の Running の行として拾い直しの腕に渡し、器に session が無い手番を今日どおり SessionFailed で閉じる(配達は有界に組み直す — 前の窓を継いで黙って待つことは無い)。pool の pod の入れ替わりは別の行の化身なので行を拾わず、監督が置き直した新しい試みの窓は最初から数える。同じ機体の入れ替わりで窓を数え直すには行に待ちの印を書く(T8 を戻す)か claim を借りの後へ動かす変更が要り、この便の範囲の外。")
     (rule R-the-agentd-puts-its-own-record-destination-on-the-seat-1601 "**agentd は、自分が参加の門で受けた記録の service の宛先を、起こす手番の env に自分で置く**(card acp:kanban-issue:ki-e930b8506201・設計 docs/design/seat-record-destination-ki-e930/・実測 2026-09-24: agentd が起こす手番の env に RECORD_SERVICE_URL が無く、32,768 byte を超える本文の `ai tell` がどの手番からも『記録の service の宛先が宣言されていない』で断られた。agentd 自身は宣言 file の [record].url で宛先を知っていて、自分の追記に使っていた。pool の pod で同じ値を明示すると 34,077 byte の本文が通った = 欠けていたのは値だけで、認証の問題は無い・既知の形 = kubelet が Pod に置く service の env〔KUBERNETES_SERVICE_HOST — 宛先は node の側が知っていて、workload の宣言には写させない〕): (1) **名の集合は 1 か所**: 走行者が持つ手番の env の名は effects.RUNNER-OWNED-SEAT-ENV(会話の身元 CONVERSATION-ID-ENV / SEAT-OPENER-ENV と記録の宛先 RECORD-URL-ENV)で、名の綴りは effects.py の定義行だけ(semgrep doeff-agents-runner-owned-seat-env-spelled-once)。参加の門(join.seat-env-of の (c) と join.claude-settings-declaration-of の (e))はこの集合で断り、charter を組む側も同じ集合を読む。(2) **値は参加の門を通った 1 つ**: AgentdSettings.record_url は join.record-sink-of を通った [record].url で、agentd が RecordHttp に渡す値と同じ。record_enabled はその在否から導く property なので、『記録は有効なのに宛先が無い』settings は作れない。記録が無効なら名を置かない。(3) **node が手番へ渡す env は 1 つの関数が組む**: judgment.node-seat-env-of が宣言の seat_env(走行者の名は落とす — 参加の門を迂回した値にも走行者の値が勝つ)の後に記録の宛先を足して組む。incarnation-charter-of はその組を charter-with-seat-env で重ね(launch / resume / rehydrate / rebuild の 4 つの起こし方すべて)、session-attribution-of は同じ組の指紋(node-seat-env-digest-of = 名と値の列の sha256)を帰属の agentd の欄 nodeEnv に刻む。重ねる順は宣言 → 走行者の宛先 → 会話の身元で、後が勝つ(R51 (2) と同じ)。(4) **温かい session は指紋で起こし方を変える**: claude の headless は手番ごとに process が降り、次の send では host が行に保存した**生まれた時の env** に手番の資格だけを重ねて起こす(headless.hy continue-headless-process)。だから宣言か [record].url を変えて agentd だけを起こし直した後に send を選ぶと、手番は古い宛先のまま走る(設計の検討で再現した反例 — counterexamples/B_witness_continue.log)。next-arm-for-job は、温かい session の帰属の指紋が今の agentd の指紋と違う時・無い時に send を選ばず、effort が違う時と同じ resume(候補を片付け、同じ session id で起こし直す・cache は保つ)を選ぶ。配備の前に生まれた session は指紋を持たないので、配備の後の最初の手番で 1 度だけ resume になる。(5) **してはいけないこと**: 継承の名簿(policy.SPAWN-INHERITED-ENV-KEYS / headless-spawn-env)に名を足さない(R30 (4))。host(headless.hy / launch.hy / policy.hy の TURN-* と overlay-without-turn-auth)に走行者の名を教えない。送りの手番の env(judgment.turn-session-env-of)に走行者の名を足さない — 手番ごとの値を運ぶ口に node の値の 2 つ目の書き手ができる((4) の resume で解く)。宣言 file の seat_env に記録の宛先の写しを書かない(R51 (1)(c) の門が断る)。(6) **R51 (4) との違い**: R51 (4) は手番向けの宛先(ACP_BASE 等)の綴りを doeff に持たせない — 定義点が宣言の側に在るので、doeff の写しが 2 つ目の既定に育つから。記録の宛先は名も値も doeff が持つ(名 = effects.RECORD-URL-ENV・値 = doeff の宣言 schema の [record].url)ので、doeff が置いても 2 つ目の既定は生まれない。逆に宣言の側へ写させる方が 2 つ目の既定になる。")]
  :laws
    [(law a-dead-backend-is-not-a-live-session
     ...
     (law a-held-credential-lease-is-a-lost-attempt-not-the-end-of-the-turn
       :statement "for_all turn job j whose custody borrow is refused: if the refusal is 409 and carries holdExpiresAt, agentd appends exactly one CredentialLeaseHeld{status True, reason = the custody refusal verbatim, attempt = binding.attempt, at = now, until = holdExpiresAt, account = binding.account, nodeRow = binding.nodeRow} to j's status.conditions and changes nothing else (phase, binding, sessionHandle and result are byte-identical), keeps j out of memory, and does not start or recover any row whose current attempt already carries such a record; every other refusal (409 without a hold, 404, 503) still ends j with CredentialUnavailable as before (amended by card ki-fd0f3b234a38: only a refusal whose stage is lease is recorded as CredentialLeaseHeld, and the custody contract's transient refusals and an unanswered borrow inside the custody's window follow law a-lender-outage-is-a-lost-attempt-and-an-unanswered-borrow-waits-out-the-custody-window)."
       :counterexamples
     ...
     (law warm-send-is-decided-at-one-point
       ...
       :counterexamples
         [(counterexample "agentd.hy が『予め resume か launch か』を自分で分岐し、judgment にも同じ分岐を持つ — 判定点が 2 つになり memory の有無で起こし方が食い違う")
          ...
          (counterexample "『この手番の材料が結果を持つ』を読みごとに or で累積する(一度立ったら手番の終わりまで消えない)— queued の注入で result の後に続いた手番が続きの途中で降りると、1 つ目の result が立ったまま本物の死を completed で閉じ、作り直されずに続きの答えが消える(設計検証 lt-1NE5PT2APB43S0HKKC2T2GAZZG の盲検 A — fake の HeadlessWorld で帰結 completed を再現)。材料は読みの中の最後の手番の境界(結末の記録か続行の開始)を名乗り、agentd は境界を見た読みで上書きする")])
     ...
     (law headless-events-go-to-the-tiered-store
       :statement "for_all headless host h whose env names DOEFF_AGENTD_EVENTS_OTLP_URL (the pool pods): every stdout and stderr line of every child process of h (sessions and cache pings) is written through the event store handler that host.install-event-store installed (headless_events.HeadlessEventStore — the outbox table headless_event_outbox of h's store) and no line is written to a file; each line is one row (session_id, seq, op, stream, line, at, turn) with seq strictly increasing per session; every time on a row (at, shipped_at, held_at) and the prune cutoff come from the one clock of the registry (headless_events.EventClock = doeff-time GetTime answered by the time handler the composition root installed: sync_time_handler in production, sim_time_handler in the simulation) — at is carried by the caller in HeadlessEventAppend.at and no event store, shipper or prune reads the OS clock; the shipper (headless_outbox.OtlpShipper) sends unshipped rows as OTLP log records (resource service.name = doeff-agentd, host.name = the node; body = the verbatim line; attributes session_id, seq, stream, op, conversation_id, agent_job_id, turn — conversation_id / agent_job_id from the agentd block of the row's launch_attribution, empty when absent) to <url>/v1/logs, the collector routes them to agentd_records.headless_events (agora-controllers effect-telemetry: hot → warm → cold by TTL MOVE), and marks shipped_at only on a 2xx answer; one POST body never exceeds OtlpShipper.max_body_bytes (DOEFF_AGENTD_EVENTS_MAX_BODY_BYTES, default 8 MiB = the collector's declared receive limit), rows are packed into bodies under that limit, a row whose single-row body exceeds it — or that the collector answers 413 for when sent alone — is held (held_at + held_reason, a log line, counted in daemon.status.events_outbox.held), is never selected again by the shipper and never removed by prune (it stays readable through session.events_since), and no held row blocks the rows after it; any other non-2xx answer leaves the batch unshipped for the next tick; a row leaves the outbox only by headless_outbox.prune-shipped, i.e. iff shipped_at is set ∧ its session is terminal ∧ the session ended before now − DOEFF_AGENTD_EVENTS_PRUNE_GRACE_SECS (default 3600); without the env the handler stays the file store (Mac, operator ruling 2026-09-25 — no unauthenticated receiver on the tailnet)"
       :counterexamples
         [(counterexample "出来事を生の file に追記し続ける(2026-09-25 まで): 消す係も移す係も無く、pool の agentd-state(2Gi)が 555 file・1.9 GiB で満杯になり、sqlite の書き込みが全部落ちた")
          ...
          (counterexample "受けの上限を超える行(実測の最大 5.4 MB)をそのまま束に入れて送り続ける — 束ごと 413 で断られ、同じ束を毎拍読み直して送り手が永久に詰まり、後ろの行も段の DB へ行かない")
          (counterexample "置き場や送り手が OS の時計(datetime.now)で at / shipped_at を刻む — 模擬環境の仮想の時計で時刻が決まらず、順序を不変条件に使えない。時計は doeff-time の GetTime の 1 つで、呼び手が値として渡す")
          (counterexample "上限を超える行を切り詰めて送る / 捨てる — 段の DB の記録が逐語でなくなる・記録が消える。留めて数を名乗り、上限の見直しか別の置き場は operator が決める")]
       :enforcement ["packages/doeff-agents/tests/test_sessionhost_headless.py::test_host_headless_events_go_to_the_outbox_and_are_read_through_the_host"
                     ...
                     "packages/doeff-agents/tests/test_sessionhost_headless_events.py::test_a_line_the_tiered_store_cannot_take_is_held_locally_and_does_not_block_the_shipper"
                     "packages/doeff-agents/tests/test_sessionhost_headless_events.py::test_batches_are_split_by_body_bytes_and_a_413_is_narrowed_to_the_one_line"
                     "packages/doeff-agents/tests/test_sessionhost_headless_events.py::test_other_refusals_keep_the_rows_unshipped_for_the_next_tick"
                     "packages/doeff-agents/tests/test_sessionhost_headless_events.py::test_event_times_come_from_the_doeff_time_clock_not_the_os"
                     "packages/doeff-agents/tests/test_sessionhost_headless_events.py::test_prune_reads_the_same_doeff_time_clock"])
     ...
     (law the-control-plane-process-exits-without-closing-the-running-turns
       ...
       :counterexamples
         [(counterexample "役を分けた後も停止の hook(close_for_stop)を登録する形 — ACP 側の規則を変えるたびに走っている手番が AgentdRestart で切れる。分離の目的そのものが消える(この card の 0 節)")
          ...
          (counterexample "close_for_exit が排水(drain_for_stop)を撃つ形 — ACP 側の入れ替えは手番を 1 つも切らないので待つ理由が無い。待てば入れ替えが手番の壁時計(最大 4 時間)に縛られ、cordon 無しの軽い入れ替えという性質が消える(待つかどうかは宣言 drain_seconds が決め、腕そのものは待たない)")
          (counterexample "役 agentd の SIGTERM が宣言を読まずに close_for_exit だけを撃つ形(2026-09-22〜23 の形)— pool の pod は腕と器が同じ削除で降りるのに、宣言 drain_seconds = 14400 を読む者が pod の中に 1 つも居なくなり、腕は即座に降り器は SIGTERM で子と一緒に落ちる。実弾 = aj-1EDQ69WZH7Y78K55RCQDVZS30A(2026-09-23 14:57 JST・SessionFailed『sessionhost stopped (SIGTERM) while the turn was running』)")
          (counterexample "排水を signal handler の中で撃つ形 — handler が数時間 main thread を塞ぎ、2 度目の合図が handler を入れ子に走らせる")
          ...
          (counterexample "役 agentd の排水が器の不在を問わず上限まで待つ形(2026-09-23〜24 の形)— pool の pod で器の容器が先に落ちると、腕の拍は session の要求の socket の失敗で毎回落ちて手番の結末を読めず、待っても 0 にならないのに pod は 14400 秒居残り、pool の片方が 4 時間新しい手番を受けない")
          (counterexample "器の不在を connect の時間切れからも読む形 — 混んだ器の遅い connect を『降りた』と読み、手番を抱えた器が居るのに排水をやめて腕が降りる(実弾 2026-09-23 15:5x 会社 Mac の区画の観測と同じ誤読。判断は socket_may_have_host の 1 点)")]
       :enforcement ["docs/adr/defadr_doeff_agents_012_agentd_acp_arms.hy::test-adr-doe-agents-012-the-control-plane-process-exits-without-closing-turns"
                     ...
                     "packages/doeff-agents/tests/test_sessionhost_acp.py::test_exit_after_drain_waits_for_the_turns_by_the_declaration_and_closes_none"
                     "packages/doeff-agents/tests/test_sessionhost_acp.py::test_exit_after_drain_stops_waiting_when_the_host_is_gone_and_closes_none"
                     "packages/doeff-agents/tests/test_sessionhost_acp.py::test_session_routes_any_host_is_false_only_on_the_evidence_of_absence"])
     (law draining-is-one-signal-read-from-outside-and-capacity-is-still-decided-once
       ...
       :counterexamples
         [(counterexample "排水を signal(SIGUSR1)で立てる形 — 入れ替えの途中で ACP 側の process 自身が再起動すると意思が消え、host を降ろす拍に capacity が宣言値へ戻っている(edge-triggered の合図を level-triggered の入れ替えに使う形)")
          ...
          (counterexample "盲検 A(card ki-b5e0d04de958 設計検証): 「何をもって印ありとするか」を drain_port(os.path.exists)と host の停止の腕の 2 つの読み手に置く形 — 印の媒体を替える(中身 true / false で宣言する)変更が 2 か所に漏れ、片方だけ直した日に ACP 側の process は排水し host は計画された停止を名乗らない(実測 = evidence/exp_a_current.log・drain_marker へ寄せた差し替えは 1 file で済む = exp_a_step3.log)")]
       :enforcement ["docs/adr/defadr_doeff_agents_012_agentd_acp_arms.hy::test-adr-doe-agents-012-draining-is-one-signal-read-from-outside"
                     ...
                     "packages/doeff-agents/tests/test_sessionhost_acp.py::test_the_drain_port_and_the_host_read_the_same_answer_from_drain_marker"])
     (law a-host-stop-under-the-drain-marker-is-a-planned-stop-and-nothing-else-is
       :statement "for_all sessionhost host h with HostConfig.drain-file p (read once from env DOEFF_SESSIONHOST_DRAIN_FILE by parse-args; None when absent or empty) receiving SIGTERM: the first TERM evaluates d = drain_marker.declared(p) and m = drain_marker.reason_line(p) exactly once inside host.term-handler and hands (d, m) by value to graceful-stop; a later TERM raises SystemExit(0) and reads nothing; every mid-turn headless row cut by stop-headless-rows carries cause.category = headless_protocol.stop_cause_category(d) ∈ {host_drained, cancelled} (host_drained iff d), the row's prose may carry m but no decision reads it, and the stop log counts the rows under that same word; and h never creates, rewrites or removes the file at p — an undeclared SIGTERM leaves no file behind and a declared one leaves its bytes unchanged"
       :counterexamples
         [(counterexample "盲検 B(card ki-b5e0d04de958 設計検証): graceful-stop が『停止中に新しい手番が結ばれないように』drain file を排他作成してから読む形 — 外の宣言は上書きしないので宣言ありの検は通るが、印の無い SIGTERM も必ず host_drained になり計画された停止の予算へ移る(受入 2 に反する)。実 binary で再現 = evidence/exp_b_probe_blind_b.log(1 failed: the host created the declaration itself)・静的な針は L2141 / L2143 を名指して RED = exp_b_needle.log")
          (counterexample "印を停止の thread の中(hook の後)で読む形 — 役 both の hook は排水で最長 drain_seconds 待つので、読む拍が TERM から数時間ずれ、その間に立った・消えた印で語が決まる(停止の拍の事実ではなくなる)")
          (counterexample "印の path を host の argv の flag(--drain-file)で渡す形 — host の CLI の語彙は oracle parse_args の凍結物理で(exit-when-orphaned の註)、flag を足すと役 both の argv も変わる(設計の改訂 1c)")
          (counterexample "host が drain_file_path を自分で導く形 — 置き場の定義点が ACP 側の 1 つ(runtime.drain_file_path)と host 側の 2 つになり、host は ACP 側の module を import しない約束も崩れる。env を渡さない起動(役 both)も既定の置き場の印を読んでしまう")
          (counterexample "stop-headless-row が make-cause \"cancelled\" のまま・または印の有無で散文だけを変える形 — ACP の配達は語でしか分けないので、計画された停止も運び手の失敗として数え続ける(09-22 の 6 通の形)")]
       :enforcement ["docs/adr/defadr_doeff_agents_012_agentd_acp_arms.hy::test-adr-doe-agents-012-the-host-only-reads-the-drain-marker-once"
                     "packages/doeff-agents/tests/test_sessionhost_headless.py::test_stop_cause_category_names_the_planned_stop_only_under_the_drain_marker"
                     "packages/doeff-agents/tests/test_sessionhost_headless.py::test_the_host_reads_the_drain_marker_path_only_from_its_env"
                     "packages/doeff-agents/tests/test_sessionhost_headless.py::test_the_term_handler_reads_the_marker_once_at_the_first_term_and_never_again"
                     "packages/doeff-agents/tests/test_sessionhost_headless.py::test_host_headless_stop_under_the_drain_marker_cuts_the_mid_turn_row_as_host_drained"
                     "packages/doeff-agents/tests/test_sessionhost_headless.py::test_real_host_sigterm_under_the_drain_marker_cuts_the_turn_as_host_drained_and_leaves_the_marker_alone"
                     "packages/doeff-agents/tests/test_sessionhost_headless.py::test_real_host_sigterm_without_the_marker_stays_cancelled_and_the_host_writes_no_marker"
                     "packages/doeff-agents/tests/test_sessionhost_headless.py::test_real_host_sigterm_without_the_env_stays_cancelled_even_with_a_marker_at_the_default_place"
                     "packages/doeff-agents/tests/test_sessionhost_acp_ended_cause.py::test_a_host_drained_row_carries_agentd_stopped_host_drained_with_the_host_drained_condition"
                     "packages/doeff-agents/tests/test_sessionhost_acp_ended_cause.py::test_job_outcome_of_maps_only_the_host_drained_row_to_the_planned_stop"])
     (law the-drain-marker-path-reaches-the-host-role-only
       :statement "for_all entry.main with argv = join … : iff join_role(argv) = host, after apply_join_env the process env carries DOEFF_SESSIONHOST_DRAIN_FILE = runtime.drain_file_path(os.environ) (the same file drain_port reads); for role both (no --role) the env bundle and the host argv are byte-identical to the ones without this change (the key is absent), and JoinPlan never carries it; the env name is spelled exactly at host.ENV-DRAIN-FILE and entry.HOST_DRAIN_FILE_ENV, and the two values are equal"
       :counterexamples
         [(counterexample "役 both にも env を立てる形 — join_role の『--role を付けない起動は env の束も argv も今日と 1 byte 差なく同じ』の約束が崩れる。役 both では停止の hook が行より先に job を drain-deadline で閉じるので、立てても ACP に届く語は変わらない(払うだけの差)")
          (counterexample "env の名を 3 か所目(runtime や join.hy)でも綴る形 — 綴りを直す変更が 1 か所に漏れ、host は印の path を受け取らずに黙って cancelled を書き続ける(印の無い起動と見分けがつかない)")]
       :enforcement ["docs/adr/defadr_doeff_agents_012_agentd_acp_arms.hy::test-adr-doe-agents-012-the-drain-marker-path-env-is-spelled-at-two-points"
                     "packages/doeff-agents/tests/test_sessionhost_acp.py::test_entry_main_hands_the_drain_marker_path_to_the_host_role_only"])
     (law the-drain-marker-module-only-reads
       :statement "sessionhost/drain_marker.py defines exactly the public functions declared(path) and reason_line(path) and contains no call that writes, creates, removes or renames a file (no open with a mode containing w / a / x / +, no write_text / write_bytes / touch / unlink / remove / rename / replace / mkdir / makedirs / os.open); declared(path) = path is not None ∧ exists(path) and reason_line(path) = the first non-empty-stripped line of the file or \"\" (unreadable / empty / None)"
       :counterexamples
         [(counterexample "drain_marker に `declare(path, reason)` を足して host の停止の腕や agentd の排水の腕から呼ぶ形 — 書き手が doeff の中に生まれ、印が『外からの宣言』ではなくなる(盲検 B の形が 1 関数の呼びで戻る)")]
       :enforcement ["docs/adr/defadr_doeff_agents_012_agentd_acp_arms.hy::test-adr-doe-agents-012-the-drain-marker-module-only-reads"
                     "packages/doeff-agents/tests/test_sessionhost_headless.py::test_the_drain_marker_is_read_by_presence_and_its_line_is_only_a_reason"])
     ...
     (law turn-events-are-appended-to-the-record-per-tick
       ...
       :counterexamples
         [(counterexample "出来事を手番の終わりにだけ書く(旧の形・実弾 2026-09-13: 本番の turn-record 97 行の entries が最後の本文 1 行・at は全部同じ・途中で落ちた手番は空)— 会話の面に agent の出力の全史が無く、途中で落ちた手番は何も残らない")
          ...
          (counterexample "本文の同一性の材料に選ぶ欄を本線(judgment.record-body-of)と偽の記録の service(fake.record_body_bytes)が別々の literal の列で持つ(実弾 2026-09-23 まで: 本線は data を欠いた 4 欄・fake は data を含む 5 欄)— 添付の data が違う 2 つの出来事が本線では同じ sha256 になり、fake の検は中央と同じ 5 欄で緑のまま本線の食い違いを撃てない。欄は effects.RECORD_BODY_FIELDS の 1 点(正本 = 契約 record-service.json eventFields.body)を両方が読む(card acp:kanban-issue:ki-651086f48560)")])
     ...
     (law turn-work-dir-is-read-on-this-node-and-a-missing-one-is-not-launched
       ...
       :enforcement ["docs/adr/defadr_doeff_agents_012_agentd_acp_arms.hy::test-adr-doe-agents-012-work-dir-is-one-judgment-on-this-node"
                     ...
                     "packages/doeff-agents/tests/sessionhost_acp_verify_deftests.hy::test-verify-job-whose-work-dir-is-missing-here-is-not-run"
                     "packages/doeff-agents/tests/sessionhost_acp_verify_deftests.hy::test-verify-job-whose-work-dir-is-here-gets-the-expanded-path-in-one-env"
                     "packages/doeff-agents/tests/sessionhost_acp_verify_deftests.hy::test-verify-job-without-a-work-dir-keeps-todays-shape"
                     "packages/doeff-agents/tests/sessionhost_acp_verify_deftests.hy::test-verify-row-whose-work-dir-is-missing-does-not-take-the-slot"])
     ...
     (law a-continued-turn-points-at-the-mail-it-already-handed
       ...
       :enforcement ["docs/adr/defadr_doeff_agents_012_agentd_acp_arms.hy::test-adr-doe-agents-012-a-continued-turn-points-at-the-handed-mail"
                     ...
                     "packages/doeff-agents/tests/test_sessionhost_acp.py::test_continuation_of_carries_handed_verbatim_even_when_it_overlaps_the_inputs"
     ...
     (law a-lender-outage-is-a-lost-attempt-and-an-unanswered-borrow-waits-out-the-custody-window
       :statement "for_all turn job j of agentd a whose custody borrow is refused with LeaseRefused r (r.stage in {lease, redeem}, r.code and r.why copied from the refusal body by handlers.CustodyHttp._borrow alone): the class is decided by judgment.custody-refusal-verdict-of alone in the order time, lender, unanswered, nobody, another-carrier and reads only r.stage, r.status, r.code, r.why and the memory's first-unanswered time (never r.error, never the node): time iff r.stage = lease and r.status = 409 and r carries a hold; lender iff (r.stage, r.status, r.code) is a key of T and (T[key] = null or r.why in T[key]) where T = transientRefusals of the custody contract copy custody_lender_availability.json (effects.CUSTODY_LENDER_TRANSIENT is derived from the copy, the copy equals custody-api.json#/lenderAvailability at its source.commit); on lender a appends exactly one CredentialLenderUnreachable{status True, reason = r.error verbatim, attempt = binding.attempt, at = now, account, nodeRow, stage, code, why when present} and changes nothing else, and never starts or recovers that attempt again; unanswered iff r.status = 0 and now - s < W where s = the first unanswered time of (j, attempt) in a's memory (now when absent) and W = (workerStaleSeconds + workerHeartbeatSeconds) * 1000 of the copy; on unanswered a writes nothing to j's row, keeps (j, attempt, s, now + R) in memory with R < W, does not borrow for j before now + R, and re-runs the same start-claimed at or after it; past W the refusal is another-carrier (CredentialUnavailable, as before); a memory entry is dropped when the verdict is not unanswered or when j is no longer a's Running row of the same attempt; a failed redeem revokes the lease the master granted and the revoke's outcome does not change r."
       :counterexamples
         [(counterexample "503 worker-unreachable を別の運び手なら通る(another-carrier)に畳む形(2026-09-23 まで): 預かり所の worker が黙った数分の間、4 台の機体が同じ 503 を受け、配達が運び手の組み直し 2 回を数秒で使い切り郵便 22 通が failed。機体を避ける組み直しでは晴れない断り")
          (counterexample "503 worker-unreachable を why を読まずに全部 lender にする形: 失効(worker-revoked)・名乗りなし・名簿に無い・url なしの worker は名簿を直すまで晴れないのに、監督が数えない置き直しを繰り返し、郵便が失敗も成功もしないまま滞留する(T7 — 時間で晴れない why は今日どおり another-carrier)")
          (counterexample "時間で晴れる組の表を runner の code に直書きする形: 預かり所が語を足した(例 worker-restarting)拍に doeff の code を直すまで古い分類が続き、定義点が預かり所の契約と doeff と agora-controllers の 3 か所になる。表は写しから導く(設計 §11 S1)")
          (counterexample "分類を error の散文(『heartbeat』『届かない』)で決める形: 預かり所が文を書き換えた拍に class が黙って変わる(R54 (7) の既知の借りと同じ病)— 散文を乱数に替えても class が変わらないことを検が撃つ")
          (counterexample "接続が答えない断り(status 0)を最初の拍に another-carrier にする形: master が worker の途絶に気付くまでの最初の 90 秒は引換の口の接続の失敗しか出ないので、途絶の最初の拍で運び手を失う(盲検 A の反例 (b))。窓 W の間は待つ")
          (counterexample "接続が答えない断りを数えない失った試み(記録)にする形: 失った試みは機体を避けず、機体の道の故障で同じ機体に 3 回置かれて郵便が死ぬ(退役 FailAgentJobRetired は組み直さない)。窓の後は今日どおり機体を避けて組み直す")
          (counterexample "接続が答えない間、拍ごとに借りを撃ち直す形: 1 秒の拍なら窓の間に 120 回の貸与を master に積む(引換の失敗の度に貸与が残る — 返さない限り hold の 900 秒)。やり直しは 15 秒の刻まで撃たず、引換に失敗した貸与は返す")
          (counterexample "接続が答えない待ちの起点を最初の拍ではなく最後の断りの刻にする形: 窓が永遠に閉じず、機体の道の故障の手番が 4 時間の手番の期限まで塞がる")
          (counterexample "待ちの項を手番の終わり・取り下げ・置き直しで捨てない形: 閉じた手番の借りを撃ち直すか、置き直した次の試みが前の試みの起点を継いで最初の拍で窓を閉じる。項を捨てる点は判定の答えと受けの拍の 2 つ")
          (counterexample "引換の 409(引換券の使用済み — 使い回した接続の送り直しで生まれる)を master の hold が付いているので time にする形(2026-09-23 まで): 時間では晴れない断りを置き直し待ちに見せ、同じ引換券の失敗を数えない試みで繰り返す(設計 F3)")]
       :enforcement ["docs/adr/defadr_doeff_agents_012_agentd_acp_arms.hy::test-adr-doe-agents-012-a-lender-outage-is-judged-by-the-custody-contract"
                     "packages/doeff-agents/tests/sessionhost_acp_lender_deftests.hy::test-the-lender-faces-that-custody-names-are-lender"
                     "packages/doeff-agents/tests/sessionhost_acp_lender_deftests.hy::test-standing-worker-unreachable-reasons-stay-another-carrier"
                     "packages/doeff-agents/tests/sessionhost_acp_lender_deftests.hy::test-an-unanswered-borrow-waits-out-the-custody-window-then-blames-the-carrier"
                     "packages/doeff-agents/tests/sessionhost_acp_lender_deftests.hy::test-a-redeem-409-is-not-a-held-lease"
                     "packages/doeff-agents/tests/sessionhost_acp_lender_deftests.hy::test-the-class-never-reads-the-prose"
                     "packages/doeff-agents/tests/sessionhost_acp_lender_deftests.hy::test-the-class-does-not-read-the-machine"
                     "packages/doeff-agents/tests/sessionhost_acp_lender_deftests.hy::test-an-unanswered-borrow-writes-nothing-and-borrows-again-only-at-its-retry-time"
                     "packages/doeff-agents/tests/sessionhost_acp_lender_deftests.hy::test-custody-naming-the-outage-inside-the-window-records-the-lost-attempt"
                     "packages/doeff-agents/tests/sessionhost_acp_lender_deftests.hy::test-the-memory-forgets-a-job-that-left-its-attempt-and-a-new-attempt-counts-afresh"
                     "packages/doeff-agents/tests/test_sessionhost_acp_lender.py::test_a_failed_redeem_names_its_stage_and_returns_the_lease"
                     "packages/doeff-agents/tests/test_sessionhost_acp_custody_lender_copy.py::test_the_copy_is_the_custody_contract_at_the_pinned_commit"
                     "packages/doeff-agents/tests/test_sessionhost_acp_custody_lender_copy.py::test_the_classifier_reads_exactly_the_derivation_of_the_copy"])
     (law the-agentd-puts-its-own-record-destination-on-the-seat
       :statement "for_all agentd a with settings s and for_all turn t that a starts by launch | resume | rehydrate | rebuild: charter(t).session_env[effects.RECORD_URL_ENV] = s.record_url when s.record_url is not None (s.record_url = the [record].url that passed join.record-sink-of, the value a hands to RecordHttp), and the name is absent when s.record_url is None; the runner's value wins over any s.seat_env pair of the same name (join refuses such a pair, and judgment.node-seat-env-of drops it); the pairs charter-with-seat-env lays on the charter are exactly node-seat-env-of(s), and attribution(t).agentd.nodeEnv = node-seat-env-digest-of(node-seat-env-of(s)); for_all warm candidate session v of the next turn of the same conversation in the same home: next-arm-for-job chooses send only if nodeEnv(v) = the digest of the current agentd, and otherwise (different or absent) chooses resume with v retired; the runner-owned names effects.RUNNER_OWNED_SEAT_ENV are spelled only in effects.py, and neither policy.SPAWN-INHERITED-ENV-KEYS nor judgment.turn-session-env-of carries any of them"
       :counterexamples
         [(counterexample "宣言 file の [agentd].seat_env に記録の宛先の写しを書かせる形: 宛先の定義点が [record].url と seat_env の 2 か所になり、片方だけ変えた日に agentd の追記と手番の ai tell が別の service へ向かう(R51 (4) が防ぐ病を、逆向きに作る)")
          (counterexample "継承の名簿に記録の宛先の名を足す形: agentd の process の env が宛先を持つとは限らない(宛先は宣言 file から読む)うえ、R30 (4) の閉じた継承を開き、手番が node の env を丸ごと継いだ形(実弾 #95)へ 1 歩戻る")
          (counterexample "charter に置くだけで、起こし方の判断を変えない形: claude の headless の次の send は host が行に保存した生まれた時の env を再生するので、[record].url を変えて agentd だけを起こし直した後も温かい session は古い宛先のまま走る(設計の反例 B・counterexamples/B_witness_continue.log)")
          (counterexample "送りの手番の env(turn-session-env-of)に記録の宛先を足す形: 手番ごとの資格を運ぶ口に node の値の 2 つ目の書き手ができ、host の関所と overlay-without-turn-auth に走行者の名を教えることになる")
          (counterexample "指紋を持たない温かい session(配備の前に生まれた)を『同じ』と読む形: 配備の後も send が選ばれ続け、宛先の無い手番が残る")
          (counterexample "走行者の名を参加の門と charter で別々の名簿に書く形: 片方に名を足した日に、門が断らない名を charter が上書きするか、宣言が走行者の値を黙って上書きする")
          (counterexample "記録の有効と宛先を別々の欄に持つ形: 『有効なのに宛先が無い』settings が作れて、charter が空の宛先を置くか黙って落とす")]
       :enforcement ["docs/adr/defadr_doeff_agents_012_agentd_acp_arms.hy::test-adr-doe-agents-012-the-agentd-puts-its-own-record-destination-on-the-seat"
                     "docs/adr/defadr_doeff_agents_012_agentd_acp_arms.hy::test_runner_owned_seat_env_spelled_once_defsemgrep"
                     "packages/doeff-agents/tests/test_sessionhost_headless.py::test_the_seat_reads_the_record_destination_of_the_agentd_that_woke_it"
                     "packages/doeff-agents/tests/test_sessionhost_acp.py::test_join_refuses_seat_env_that_copies_the_record_destination"
                     "packages/doeff-agents/tests/test_sessionhost_acp.py::test_settings_carry_the_record_destination_that_passed_the_join_gate"
                     "packages/doeff-agents/tests/test_sessionhost_acp.py::test_runner_owned_seat_env_names_have_one_home"
                     "packages/doeff-agents/tests/test_sessionhost_acp.py::test_the_runner_record_destination_wins_over_a_seat_env_that_bypassed_the_gate"
                     "packages/doeff-agents/tests/test_sessionhost_acp.py::test_next_arm_resumes_a_warm_session_launched_with_another_node_env"
                     "packages/doeff-agents/tests/test_sessionhost_acp.py::test_join_refuses_seat_settings_whose_env_block_names_a_runner_owned_name"])]
  :enforcement
    [(deftest test-adr-doe-agents-012-history-thins-tool-items-before-dropping-turns
       ...
       ;; 出来事の畳み(並べ方と手番への割り方)は history-groups-of の 1 点へ抽出された(card acp:kanban-issue:ki-c3aace97d825・
       ;; 8a3e06a8 — 読み手は rehydrate と transcript の 2 つ)。薄い項はそこで 1 度だけ組み、k は rehydrate が上限から導いて渡す。
       (assert (any (gfor line fold-body (in "(history-groups-of conversation-id messages source exclude fetched floor-at thin-bytes)" line)))
               "k は畳みの 1 点へ渡す(R35)")
       (setv groups-body (defk-body judgment-lines "history-groups-of"))
       (assert (= (len (lfor line groups-body :if (in "(history-event-thin-line event thin-bytes)" line) line)) 1)
               "薄い項は出来事の畳みで 1 度組む(R35)")
       (assert (not (any (gfor line fold-body (in "(history-event-thin-line " line))))
               "rehydrate が畳みの外で薄い項を組み直している(R35)")
     ...
     (deftest test-adr-doe-agents-012-turn-records-are-not-left-to-one-write
       ...
       ;; 巡回は記録の鍵で読み直した行で判断し、その行から閉じる status を組み、その行へ書く(一覧の image で判断も書きもしない)。
       ;; 閉じる status に手番の終わりだけの材料(effects の宣言 — usage・entries・cache-observation・responses)を置かない。
       ;; 行の字面ではなく、巡回の本体の form を呼び先(turn-record-ended-status・AcpGetRow・AcpPutStatus・status-object-of・
       ;; turn-record-sweep-verdict)の定義から導いた引数の役で読む(sweep-violations・反例の検
       ;; test-adr-doe-agents-012-sweep-roles-are-read-from-the-callee-not-the-spelling)。
       (<- callees SweepCallees (real-sweep-callees))
       (setv materials (MaterialClasses :end-only (dict TURN-RECORD-END-ONLY-MATERIALS) :row TURN-RECORD-ROW-MATERIALS))
     ...
     (deftest test-adr-doe-agents-012-sweep-roles-are-read-from-the-callee-not-the-spelling
       ...
       ;; 宣言する・呼びが引数を足す・註・折れ方・keyword の既定値つき引数・局所変数の改名)では緑のまま、手番の終わりだけの材料に
       ;; 値を渡す変更・分類の無い役を足す変更・呼びを消す変更・一覧の行で判断する / 組む / 書く変更・記録ではない鍵で読む変更・
       ;; 組んだ status を書く前に変える変更・書きを補助の defk へ移す変更・呼び先の役の改名では、意図した失敗文ちょうどで赤になる。
       ;; 作り物の呼び先・材料の分類・巡回の本体(SWEEP-ROLE-CASES)で撃つ — 本物の綴りや本物の宣言を書き換えて作らない。
     ...
     (deftest test-adr-doe-agents-012-provider-limit-is-the-containers-cause-read-as-a-field
       ...
       ;; rate_limited)と ACP の検 2 本(cause → 条件・限度でない終端では条件なし)。2026-09-24 の追補の反例は
       ;; 下の在処の検め(器の表・host・制御面の各 1 本)。
       ...
       (setv judgment-text-early (.join "\n" (code-lines (/ ACP-DIR "judgment.hy"))))
       ...
       (assert (is-not (re.search r"\(import doeff_agents\.sessionhost\.impls\.markers \[is-api-limit-refusal api-limit-reading-of api-limit-resets-at\b" headless-text) None)
               "限度の判定・範囲と理由・戻りの時刻は markers の 1 点を呼ぶ(R33・2026-09-23 / 2026-09-24 追補)")
       ;; R33 追補 2026-09-23: 範囲(limit_scope)と戻りの時刻(limit_resets_at_ms)も器の側が当てて cause の欄に載せ、
       ;; 制御面はその欄を読む(文を読まない)。
       ;; R33 追補 2026-09-24(card acp:kanban-issue:ki-5d4849d22a4e): 表は範囲と理由の対を返す 1 点(api-limit-reading-of)で、
       ;; 器の側はそれを手番の終わりの文へ当てる。制御面は 2 つの欄を読み、閉語彙の表に在る語だけを写す(呼び先と引数の役で撃つ)。
       (assert (in ["verdict.detail"] (call-args-of headless-lines "api-limit-reading-of"))
               "範囲と理由を器の側で手番の終わりの文へ当てていない(R33 追補 2026-09-23 / 2026-09-24)")
       (assert (is-not (re.search r"\(defk api-limit-reading-of \[detail\]" (collapsed-code (/ SESSIONHOST-DIR "impls" "markers.hy"))) None)
               "範囲と理由の表が markers の 1 点に無い(R33 追補 2026-09-24)")
       (assert (in "(.get cause \"limit_scope\")" judgment-text-early) "制御面が範囲を cause の欄から読んでいない(R33 追補 2026-09-23)")
       (assert (in "(.get cause \"limit_reason\")" judgment-text-early) "制御面が理由を cause の欄から読んでいない(R33 追補 2026-09-24)")
       (setv judgment-membership (call-args-of (code-lines (/ ACP-DIR "judgment.hy")) "in"))
       (for [vocabulary ["PROVIDER-LIMIT-SCOPES" "PROVIDER-LIMIT-REASONS"]]
         (assert (any (gfor args judgment-membership (and (= (len args) 2) (= (get args 1) vocabulary))))
                 f"制御面が cause の語を閉語彙 {vocabulary} で絞っていない(R33 追補 2026-09-24)"))
       ;; 反例(挙動)の検の在処(R33 追補 2026-09-24): 器の表(markers)・器の側の cause(host)・制御面の条件(judgment)。
       (setv limit-tests-dir (/ (. (Path __file__) parent parent parent) "packages" "doeff-agents" "tests"))
       (for [[file name] [["sessionhost_impls_deftests.hy" "(deftest test-api-limit-reading-names-the-range-and-the-reason-the-text-says"]
                          ["test_sessionhost_headless.py" "def test_host_headless_turn_refused_by_the_group_usage_limit_leaves_the_range_unknown("]
                          ["sessionhost_acp_turn_events_deftests.hy"
         ...
         (assert (in name (.read-text (/ limit-tests-dir file) :encoding "utf-8")) f"R33 追補 2026-09-24 の反例の検が無い: {file} {name}"))
     ...
     (deftest test-adr-doe-agents-012-work-dir-is-one-judgment-on-this-node
       ...
       ;; R32 (4) / R36 (6)(card ki-0a50e47ac56d): work_dir を名乗る verify も同じ展開・同じ門を通り、在否の読みは work-dir-step-here の 1 点。
       (assert (= (len (lfor line agentd-lines :if (.startswith line "(defk work-dir-step-here ") line)) 1) "作業場の在否の読みは agentd.work-dir-step-here の 1 点(R32 (4))")
       (setv verify-claim-start (next (gfor [i line] (enumerate agentd-lines) :if (.startswith line "(defk claim-verify-job ") i)))
       (setv verify-claim-lines (cut agentd-lines verify-claim-start (+ verify-claim-start 60)))
       (assert (any (gfor line verify-claim-lines (in "(plan-with-node-home planned settings.home)" line))) "claim-verify-job が node の家で展開していない(R32 (4))")
       (assert (any (gfor line verify-claim-lines (in "(work-dir-ready settings row plan now-ms)" line))) "claim-verify-job が作業場の門を通らない(R32 (4))")
     ...
     (deftest test-adr-doe-agents-012-verify-command-runs-a-script-not-a-session
       ...
       (setv judgment-lines (code-lines (/ ACP-DIR "judgment.hy")))
       (for [name ["(defk job-kind-of " "(defk verify-plan-of " "(defk verify-argv-of " "(defk verify-env-of " "(defk verify-step-of "
       ...
       (assert (in "(CommandStart :argv argv :cwd settings.home :env env)" verify-body) "verify の腕が CommandStart で起こしていない(R36)")
       (assert (in "(<- env tuple (verify-env-of plan))" verify-body) "verify の env が judgment.verify-env-of の 1 点でない(R36 (6))")
     ...
     (deftest test-adr-doe-agents-012-summaries-are-triggered-at-turn-end-and-folded-first-on-rehydrate
       ...
       ;; 再開の材料の読みは history-material-for の 1 点・畳みは history-fold-of の 1 点(card acp:kanban-issue:ki-c3aace97d825 で
       ;; history-for から抽出 — 畳み先が rehydrate と transcript の 2 つになったので、読みを 1 度にするため)。history-for は 2 つを繋ぐだけ。
       (setv history-body (defk-body agentd-lines "history-for"))
       (assert (any (gfor line history-body (in "(history-material-for settings subject exclude)" line))) "history-for が材料の読みの 1 点を通らない(R38)")
       (assert (any (gfor line history-body (in "(history-fold-of settings subject exclude material)" line))) "history-for が畳みの 1 点を通らない(R38)")
       (setv material-body (defk-body agentd-lines "history-material-for"))
       (for [needle ["(summaries-for settings subject)" "(summary-floor-of summaries)" "(record-turns-for settings subject floor)"
                     ...
                     ":summaries summaries :floor-at floor-at"]]
         (assert (any (gfor line material-body (in needle line))) f"再開の読みが要約を先に読んでいない / 郵便の下限を読んでいない(R38・追補 6): {needle}"))
       (setv summaries-at (next (gfor [i line] (enumerate material-body) :if (in "(summaries-for settings subject)" line) i) None))
       (setv raw-at (next (gfor [i line] (enumerate material-body) :if (in "(record-turns-for settings subject floor)" line) i) None))
       (assert (< summaries-at raw-at) "要約を原文より先に読んでいない(R38)")
       (setv fold-of-body (defk-body agentd-lines "history-fold-of"))
       (assert (any (gfor line fold-of-body (in "material.summaries material.floor-at))" line))) "畳みが要約と郵便の下限を受けていない(R38・追補 6)")
       (assert (any (gfor line fold-of-body (in "settings.rehydrate-history-byte-budget material.fetched" line))) "畳みが上限の宣言を受けていない(R38)")
     ...
     (deftest test-adr-doe-agents-012-print-mode-has-one-home
       ...
       ;; 2026-09-25 の付け替え(#604): session host の外の doeff-agents(headless の handler = doeff-claude-code への adapter を含む)
       ;; は print mode の綴りを 1 つも持たない — 起動の引数の家は doeff-claude-code の argv.hy。
       (setv package-dir SESSIONHOST-DIR.parent)
       (setv outside (lfor path (sorted (.rglob package-dir "*.hy"))
                           :if (not (.is-relative-to path SESSIONHOST-DIR))
                           :if (any (gfor line (code-lines path)
                                          (re.search r"\"-p\"\s+\"--(input|output)-format\"|\"--print\"|stream-json" line)))
                           (str (.relative-to path package-dir))))
       (assert (= outside []) f"session host の外の doeff-agents に print mode の綴り: {outside}")
       (setv claude-code-argv (/ package-dir.parent.parent.parent "doeff-claude-code" "src" "doeff_claude_code" "argv.hy"))
       (assert (any (gfor line (code-lines claude-code-argv) (in "\"-p\" \"--input-format\" \"stream-json\"" line)))
               "print mode の起動の家は doeff-claude-code の argv.hy")
     ...
     (deftest test-adr-doe-agents-012-conversation-cache-only-on-the-same-node-and-home
       ...
       ;; 読みは history-material-for へ抽出された(ki-c3aace97d825)ので、禁止は読みの 1 点と繋ぎの両方に掛ける。
       (setv history-body (+ (defk-body agentd-lines "history-for") (defk-body agentd-lines "history-material-for")))
     ...
     (deftest test-adr-doe-agents-012-record-body-fields-are-read-from-one-declaration
       ;; R19 の針(構造・card acp:kanban-issue:ki-651086f48560): 本文の同一性の材料に選ぶ欄は effects.RECORD_BODY_FIELDS の
       ;; 1 点で宣言し、本線(judgment.record-body-of)と偽の記録の service(fake.record_body_bytes)の両方がそれを読む。
       ;; 反例: 片方が欄の literal の列を持ち直す(本線の 4 欄 と fake の 5 欄が 2026-09-23 まで別の列だった)。
       ;; 値が中央と一致することは tests/sessionhost_acp_record_deftests.hy の golden が撃つ。
       (setv effects-lines (code-lines (/ ACP-DIR "effects.py")))
       (assert (= (len (lfor line effects-lines :if (.startswith line "RECORD_BODY_FIELDS: tuple[str, ...] = (") line)) 1)
               "本文の欄の宣言 RECORD_BODY_FIELDS が effects.py に 1 点で無い(R19)")
       ;; 読む形は註記や docstring で満たせないよう、本体の code 行(反復の頭)で撃つ。
       (setv judgment-src (.read-text (/ ACP-DIR "judgment.hy") :encoding "utf-8"))
       (setv fake-src (.read-text (/ ACP-DIR "fake.py") :encoding "utf-8"))
       (setv body-of-lines (lfor line (.splitlines (get (.split (get (.split judgment-src "(defk record-body-of [") 1) "\n\n\n") 0))
                                 (.strip line)))
       (assert (in "(dfor name RECORD-BODY-FIELDS" body-of-lines) "judgment.record-body-of が RECORD_BODY_FIELDS を反復しない(R19)")
       (setv fake-body-lines (lfor line (.splitlines (get (.split (get (.split fake-src "def record_body_bytes(") 1) "\n\n\n") 0))
                                   (.strip line)))
     ...
     (deftest test-adr-doe-agents-012-capability-table-and-agent-setting-ignored-are-one-judgment
       ...
       (assert (= (len (lfor line agentd-lines :if (in "(next-arm-for-job candidate view home effort compact node-env-digest)" line) line)) 1) "agentd.hy が effort を腕へ渡さない(R24)")
       (for [line agentd-lines]
     ...
     (deftest test-adr-doe-agents-012-stop-drains-declared-nodes-before-closing
       ...
       ;; 排水を呼ぶ腕の**名の集合**(R58 改訂 2026-09-23 で役 agentd の exit_after_drain が加わった)— 数ではなく名で撃つ。
       (setv drain-callers #{})
       (setv current-method None)
       (for [line runtime-lines]
         (when (.startswith line "    def ") (setv current-method (get (.split (cut line 8 None) "(") 0)))
         (when (.startswith line "def ") (setv current-method None))
         (when (in "self.drain_for_stop(reason)" line) (.add drain-callers current-method)))
       (assert (= drain-callers #{"close_for_stop" "exit_after_drain"})
               f"排水を呼ぶ停止の腕は close_for_stop と exit_after_drain ちょうど(R39・R58): {drain-callers}")
       (setv close-at (next (gfor [i line] (enumerate runtime-lines) :if (.startswith line "    def close_for_stop(") i) None))
       (setv drain-at (next (gfor [i line] (enumerate runtime-lines) :if (and (is-not close-at None) (> i close-at) (in "self.drain_for_stop(reason)" line)) i) None))
     ...
     (deftest test-adr-doe-agents-012-every-terminal-write-carries-a-cause
       ...
       ;; 追補 2026-09-24(card ki-b5e0d04de958 D2): 器の行の語 host_drained → {agentd-stopped, host-drained} の写しは
       ;; judgment.job-outcome-of の 1 点(第 2 の写しを agentd や別の判断に置かない)。
       (for [word ["CAUSE-CATEGORY-HOST-DRAINED" "CAUSE-REASON-HOST-DRAINED" "CONDITION-HOST-DRAINED"]]
         (setv mapped (readers-of [(/ ACP-DIR "judgment.hy") (/ ACP-DIR "agentd.hy")] word))
         (assert (= (set mapped) #{"job-outcome-of"}) f"計画された停止の写しが job-outcome-of の外に在る(R47 追補): {word} {mapped}"))
       ...
       (for [needle ["def test_the_closed_categories_are_the_contracts_five_words"
                     ...
                     "def test_a_provider_limit_refusal_is_not_a_terminal_and_carries_no_result"
                     ...
                     "def test_a_host_drained_row_carries_agentd_stopped_host_drained_with_the_host_drained_condition"
     ...
     (deftest test-adr-doe-agents-012-one-conversation-keeps-one-warm-session
       ...
       ;; 器の一覧の読みは agentd.live-sessions の 1 点(器が status を SQL で絞る — 2026-09-23 会社 Mac の終端 6,087 行の実弾で
       ;; 絞らない一覧を置き換えた)。起こす腕はその 1 点から読む(行からではない)。
       (assert (>= (len (lfor line agentd-lines :if (in "(<- warm tuple (live-sessions))" line) line)) 1) "起こす腕が器の温かい session の一覧を読んでいない(R45)")
       (setv live-body (defk-body agentd-lines "live-sessions"))
       (assert (any (gfor line live-body (in "(SessionList :lifecycle LIFECYCLE-MULTI-TURN :statuses statuses)" line)))
               "live-sessions が器の一覧を読んでいない(R45)")
     ...
     (deftest test-adr-doe-agents-012-conversations-compact-at-their-threshold
       ...
       (assert (any (gfor line judgment-lines (.startswith line "(defk next-arm-for-job [candidate view home effort compact node-env-digest]"))) "圧縮は next-arm-for-job の 5 つ目の引数(R27)")
       ...
       (assert (= (len (lfor line agentd-lines :if (in "(next-arm-for-job candidate view home effort compact node-env-digest)" line) line)) 1) "腕の呼び手は 1 つ(R27)")
     ...
     (deftest test-adr-doe-agents-012-seat-facing-env-is-declared-once-and-loses-to-the-conversation
       ...
       (assert (= (len (lfor line judgment-lines :if (in "(charter-with-seat-env with-id node-env)" line) line)) 1)
               "incarnation-charter-of が 1 度呼ぶ(R51)")
       (setv seat-at (next (gfor [i line] (enumerate judgment-lines)
                                 :if (in "(charter-with-seat-env with-id node-env)" line) i)))
     ...
     (deftest test-adr-doe-agents-012-a-held-credential-lease-is-a-lost-attempt
       ...
       (setv agentd-lines (code-lines (/ ACP-DIR "agentd.hy")))
       ;; 改訂(card acp:kanban-issue:ki-fd0f3b234a38・R-a-lender-outage-is-a-lost-attempt-fd0f): 呼びは 4 つ目の引数(memory が
       ;; 覚えた接続が答えなかった最初の刻)を運ぶ — 針は引数の数ではなく「呼ぶ点が 1 つ」を撃つ(行の字面の終わりを焼き付けない)。
     ...
     (deftest test-adr-doe-agents-012-a-custody-refusal-names-who-can-answer
       ...
       (setv effects-lines (code-lines (/ ACP-DIR "effects.py")))
       ;; 改訂(card ki-fd0f3b234a38・R-a-lender-outage-is-a-lost-attempt-fd0f): class に lender / unanswered を足した(順は
       ;; time → lender → unanswered → nobody → another-carrier — 足した 2 語の針は同じ ADR の R-…-fd0f の検)。
       (for [needle ["CustodyRefusalAnswerer = Literal[\"nobody\", \"another-carrier\", \"time\", \"lender\", \"unanswered\"]"
     ...
     (deftest test-adr-doe-agents-012-a-lender-outage-is-judged-by-the-custody-contract
       ;; R-a-lender-outage-is-a-lost-attempt-fd0f の針(構造): 時間で晴れる組の表と窓は契約の写しから導く(runner の code に
       ;; 語を直書きしない)・lender の腕は verdict の中の 1 判断・記録の型は失った試みの集合に在る・断りの語を写すのは
       ;; _borrow の 1 点・agentd は断りの語を自分で比べない・memory の項を捨てる受けの点は 1 つ。反例(挙動)は
       ;; sessionhost_acp_lender_deftests.hy・test_sessionhost_acp_lender.py・test_sessionhost_acp_custody_lender_copy.py。
       (import doeff_agents.sessionhost.acp.effects [CONDITION-CREDENTIAL-LENDER-UNREACHABLE CUSTODY-LENDER-COPY
                                                     CUSTODY-LENDER-TRANSIENT CUSTODY-UNANSWERED-RETRY-MS
                                                     CUSTODY-UNANSWERED-WINDOW-MS REFUSED-ATTEMPT-CONDITION-TYPES
                                                     custody-lender-section-of custody-lender-terms-of])
       ;; (3) 表と窓は同梱の写しの導出ちょうど・写しは package の中(wheel に入る場所)に在る。
       (assert (.is-file (/ ACP-DIR "custody_lender_availability.json")) "預かり所の契約の写しが package の中に無い(R-…-fd0f (3))")
       (setv terms (custody-lender-terms-of (custody-lender-section-of CUSTODY-LENDER-COPY)))
       (assert (= (dict CUSTODY-LENDER-TRANSIENT) (dict terms.transient)) "分類の表が写しの導出でない(R-…-fd0f (3))")
       (assert (= CUSTODY-UNANSWERED-WINDOW-MS terms.unanswered-window-ms) "窓が写しの導出でない(R-…-fd0f (3))")
       (assert (< 0 CUSTODY-UNANSWERED-RETRY-MS CUSTODY-UNANSWERED-WINDOW-MS) "やり直しの間隔が窓より短くない(R-…-fd0f (3))")
       ;; 契約の why の語を runner の code に綴らない(表の直書きの禁止 — 註と docstring の説明は数えない:
       ;; 引用符つきの literal だけを撃つ)。
       (setv why-words (list (.keys (get (custody-lender-section-of CUSTODY-LENDER-COPY) "why"))))
       (assert why-words "写しに why の語彙が無い(R-…-fd0f (3))")
       (for [file ["effects.py" "judgment.hy" "agentd.hy" "handlers.py"]]
         (for [line (code-lines (/ ACP-DIR file))]
           (for [word why-words]
             (assert (not-in (+ "\"" word "\"") line) f"契約の why の語 {word} を runner が直書きしている(R-…-fd0f (3)): {file}: {line}"))))
       ;; (2) lender の腕は verdict の中の 1 判断(第 2 の判定を置かない)・記録の型は失った試みの集合に在る。
       (setv judgment-lines (code-lines (/ ACP-DIR "judgment.hy")))
       (assert (= (len (lfor line judgment-lines :if (.startswith line "(defk custody-lender-transient? ") line)) 1)
               "lender の判断の点は 1 つ(R-…-fd0f (2))")
       (assert (= (len (lfor line judgment-lines :if (in "(custody-lender-transient? refusal)" line) line)) 1)
               "lender の判断は verdict の中から 1 度だけ呼ばれる(R-…-fd0f (2))")
       (assert (in CONDITION-CREDENTIAL-LENDER-UNREACHABLE REFUSED-ATTEMPT-CONDITION-TYPES)
               "CredentialLenderUnreachable が失った試みの集合に無い — 同じ試みを起こし直す(R-…-fd0f (2))")
       ;; (1) 断りの処理ステージと機械の語を写すのは _borrow の 1 点(handlers の外で LeaseRefused に stage を名乗らせない)。
       (setv handler-lines (code-lines (/ ACP-DIR "handlers.py")))
       (for [word ["CUSTODY_STAGE_LEASE," "CUSTODY_STAGE_REDEEM,"]]
         (assert (>= (len (lfor line handler-lines :if (in word line) line)) 1) f"_borrow が処理ステージを名乗らない(R-…-fd0f (1)): {word}"))
       ;; agentd は断りの語を自分で比べない(判定は verdict の 1 点)・memory の項を捨てる受けの点は 1 つ。
       (setv agentd-lines (code-lines (/ ACP-DIR "agentd.hy")))
       (for [line agentd-lines]
         (for [shape ["(= refusal." "(!= refusal." "(in refusal." "(= CUSTODY-UNANSWERED-STATUS"]]
           (assert (not-in shape line) f"agentd が断りの語を自分で判じている(R-…-fd0f (2)): {line}")))
       (assert (= (len (lfor line agentd-lines :if (in "(unanswered-borrows-kept " line) line)) 1)
               "memory の項を受けの拍に捨てる点は 1 つ(R-…-fd0f (4))")
       (setv lender-tests (.read-text (/ (. (Path __file__) parent parent parent) "packages" "doeff-agents" "tests" "sessionhost_acp_lender_deftests.hy") :encoding "utf-8"))
       (for [name ["test-the-lender-faces-that-custody-names-are-lender"
                   "test-standing-worker-unreachable-reasons-stay-another-carrier"
                   "test-an-unanswered-borrow-waits-out-the-custody-window-then-blames-the-carrier"
                   "test-a-redeem-409-is-not-a-held-lease"
                   "test-the-class-never-reads-the-prose"
                   "test-the-class-does-not-read-the-machine"
                   "test-an-unanswered-borrow-writes-nothing-and-borrows-again-only-at-its-retry-time"
                   "test-custody-naming-the-outage-inside-the-window-records-the-lost-attempt"
                   "test-the-memory-forgets-a-job-that-left-its-attempt-and-a-new-attempt-counts-afresh"]]
         (assert (in (+ "(deftest " name) lender-tests) f"R-…-fd0f の反例の検が無い: {name}")))
     ...
     (deftest test-adr-doe-agents-012-the-halves-split-without-a-second-declaration
       ...
       ;; 器が 2 つ並ぶ入れ替え(blue/green・host_slots — 87ee1cfd)の後、腕が持つのは経路 SessionRoutes で、
       ;; 経路は区画ごとの socket へ SessionRpc(socket の client)を通して撃つ。境界が socket のままで第 2 の口が
       ;; 無いことは「腕が経路を socket の path から作る」と「経路の唯一の運び手が SessionRpc」の 2 つで読む。
       (assert (any (gfor line runtime-lines (in "sessions = SessionRoutes(socket_path)" line)))
               "器との境界が socket でなくなっている(R57)")
       (setv handlers-lines (code-lines (/ ACP-DIR "handlers.py")))
       (assert (any (gfor line handlers-lines (in "rpc_of: Callable[[str], SessionRpc] = SessionRpc," line)))
               "経路(SessionRoutes)の器への運び手が socket の SessionRpc でなくなっている(R57)")
       (assert (any (gfor line handlers-lines (in "self._client = AgentdClient(socket_path)" line)))
               "SessionRpc が socket の client を話していない(R57)")
       (setv tests (.read-text (/ (. (Path __file__) parent parent parent) "packages" "doeff-agents" "tests" "test_sessionhost_acp.py") :encoding "utf-8"))
     ...
     (deftest test-adr-doe-agents-012-the-control-plane-process-exits-without-closing-turns
       ...
       ;; 改訂 2026-09-23: 役 agentd の停止 = 排水(宣言の 1 点)→ close_for_exit。exit_after_drain はその 2 手ちょうどで、
       ;; job を閉じない。run_agentd_only は handler の外(main thread)でそれを撃つ。
       (setv after-at (next (gfor [i line] (enumerate runtime-lines)
                                  :if (.startswith line "    def exit_after_drain(self, reason: str) -> int:") i) None))
       (assert (is-not after-at None) "役 agentd の停止の腕 exit_after_drain が無い(R58 改訂)")
       (setv after-body [])
       (for [line (cut runtime-lines (+ after-at 1) None)]
         (when (.startswith line "    def ") (break))
         (.append after-body line))
       (setv drain-line (next (gfor [i line] (enumerate after-body) :if (in "self.drain_for_stop(reason)" line) i) None))
       (setv exit-line (next (gfor [i line] (enumerate after-body) :if (in "self.close_for_exit(reason)" line) i) None))
       (assert (and (is-not drain-line None) (is-not exit-line None) (< drain-line exit-line))
               "exit_after_drain が 排水 → close_for_exit の順で撃っていない(R58 改訂)")
       (assert (not (any (gfor line after-body (in "run_close_for_stop(" line)))) "exit_after_drain が job を閉じている(R58)")
       (setv only-at (next (gfor [i line] (enumerate runtime-lines) :if (.startswith line "def run_agentd_only(") i) None))
       (setv only-body [])
       (for [line (cut runtime-lines (+ only-at 1) None)]
         (when (and line (not (.startswith line " "))) (break))
         (.append only-body line))
       (assert (any (gfor line only-body (in "run.exit_after_drain(" line))) "役 agentd の SIGTERM が排水を通らない(R58 改訂)")
       (setv handler-at (next (gfor [i line] (enumerate only-body) :if (in "def on_signal(" line) i) None))
       (setv handler-body [])
       (for [line (cut only-body (+ handler-at 1) None)]
         (when (not (.startswith line "        ")) (break))
         (.append handler-body line))
       (assert (not (any (gfor line handler-body (or (in "exit_after_drain" line) (in "drain_for_stop" line) (in "close_for_exit" line)))))
               "signal handler の中で停止の腕を撃っている(R58 改訂 — 排水は main thread)")
       (setv entry-lines (code-lines (/ ACP-DIR "entry.py")))
       ...
       (setv tests (.read-text (/ (. (Path __file__) parent parent parent) "packages" "doeff-agents" "tests" "test_sessionhost_acp.py") :encoding "utf-8"))
       ;; 追補 2026-09-24(card acp:kanban-issue:ki-18d6c4851b21): 排水は器の不在の読みを渡し、読みの配線は組み立ての 1 点
       ;; (SessionRoutes.any_host — socket_may_have_host の 1 点)。
       (assert (any (gfor line runtime-lines (in "host_gone=None if present is None else lambda: not present()" line)))
               "排水が器の不在で待ちをやめない(R58 追補)")
       (assert (= (len (lfor line runtime-lines :if (in "host_present=routes.any_host" line) line)) 1)
               "器の不在の読みの配線は start_agentd_thread の 1 点(R58 追補)")
       (for [name ["test_close_for_exit_leaves_the_running_turns_to_the_host_process"
                   "test_exit_after_drain_waits_for_the_turns_by_the_declaration_and_closes_none"
                   "test_exit_after_drain_stops_waiting_when_the_host_is_gone_and_closes_none"
                   "test_session_routes_any_host_is_false_only_on_the_evidence_of_absence"]]
     ...
     (deftest test-adr-doe-agents-012-draining-is-one-signal-read-from-outside
       ...
       (assert (not (any (gfor line reason-body (in "return True" line))))
               "合図の中身が判断に使われている(R59)")
       ;; 追補 2026-09-24(card ki-b5e0d04de958 改訂 1a・盲検 A): 印の解釈は drain_marker の 1 点 — drain_port は在否を・
       ;; _drain_reason は中身を自分で読まず、drain_marker の答えを呼ぶ(解釈を替える変更が 2 つの読み手に漏れない)。
       (setv port-at (next (gfor [i line] (enumerate runtime-lines) :if (.startswith line "def drain_port(") i) None))
       (setv port-body [])
       (for [line (cut runtime-lines (+ port-at 1) None)]
         (when (.startswith line "def ") (break))
         (.append port-body line))
       (assert (any (gfor line port-body (in "drain_marker.declared(" line))) "drain_port が印の解釈の 1 点(drain_marker.declared)を読まない(R59 追補)")
       (for [word ["os.path.exists(" "os.stat(" "Path("]]
         (assert (not (any (gfor line port-body (in word line)))) f"drain_port が印の在否を自分で読んでいる(R59 追補): {word}"))
       (assert (any (gfor line reason-body (in "drain_marker.reason_line(" line))) "理由の読みが drain_marker.reason_line を通らない(R59 追補)")
       (assert (not (any (gfor line reason-body (in "open(" line)))) "_drain_reason が印の中身を自分で読んでいる(R59 追補)")
       (setv tests (.read-text (/ (. (Path __file__) parent parent parent) "packages" "doeff-agents" "tests" "test_sessionhost_acp.py") :encoding "utf-8"))
       (for [name ["test_the_drain_file_raises_and_lowers_the_same_draining_signal"
                   ...
                   "test_the_drain_port_and_the_host_read_the_same_answer_from_drain_marker"]]
         (assert (in (+ "def " name "(") tests) f"R59 の反例の検が無い: {name}")))
     (deftest test-adr-doe-agents-012-the-host-only-reads-the-drain-marker-once
       ;; R-the-host-only-reads-the-drain-marker-once-002c の針(構造 — 2 層目。1 層目 = law の :enforcement の実 binary の対):
       ;; 印を読む呼び(drain-declared / drain-reason-line)は term-handler の 1 つ・その 1 度目の枝の中だけ・印の path の欄を
       ;; 読む頂点の form は parse-args(env の 1 読みと受け渡し)と term-handler(2 つの読みへの引数)ちょうど・停止の腕と
       ;; headless.hy / headless_protocol.py は印を知らない(答えを値で受けるだけ)・行の語は stop-cause-category の 1 点で
       ;; stop-headless-row は cancelled の逐語を持たない・停止の log の語は行の語から導く・3 つの語の写しは同じ綴り。
       (import doeff_agents.sessionhost.headless_protocol [stop-cause-category])
       (import doeff_agents.sessionhost.policy [TERMINAL-CAUSE-RETRYABLE])
       (import doeff_agents.sessionhost.acp.effects [CAUSE-CATEGORY-HOST-DRAINED])
       (setv host-path (/ SESSIONHOST-DIR "host.hy"))
       (for [word ["(drain-declared " "(drain-reason-line "]]
         (assert (= (set (readers-of [host-path] word)) #{"term-handler"})
                 f"印を読む点が TERM の handler の外に在る(R-…-002c — 停止の拍に 1 回): {word} {(readers-of [host-path] word)}"))
       (setv path-readers (readers-of [host-path] "drain-file"))
       (assert (= (set path-readers) #{"parse-args" "term-handler"})
               f"印の path の欄を読む点が名簿と違う(R-…-002c — 欄の宣言・env の読みと受け渡し・2 つの読みへの引数だけ): {path-readers}")
       (setv host-lines (code-lines host-path))
       (setv handler-at (next (gfor [i line] (enumerate host-lines) :if (.startswith line "(defn term-handler ") i) None))
       (assert (is-not handler-at None) "TERM の handler(term-handler)が無い(R-…-002c)")
       (setv handler-body [])
       (for [line (cut host-lines (+ handler-at 1) None)]
         (when (.startswith line "(") (break))
         (.append handler-body line))
       (setv second-at (next (gfor [i line] (enumerate handler-body) :if (in "(raise (SystemExit 0))" line) i) None))
       (setv first-at (next (gfor [i line] (enumerate handler-body) :if (in "(.set stopping)" line) i) None))
       (setv read-at (next (gfor [i line] (enumerate handler-body) :if (in "(drain-declared " line) i) None))
       (assert (and (is-not second-at None) (is-not first-at None) (is-not read-at None) (< second-at first-at read-at))
               "印の読みが 1 度目の TERM の枝(stopping を立てた後)の中に無い — 2 度目で読み直す形(R-…-002c)")
       (for [path [(/ SESSIONHOST-DIR "headless.hy") (/ SESSIONHOST-DIR "headless_protocol.py")]]
         (setv proof (if (= path.suffix ".hy") "(defk stop-headless-row " "def stop_cause_category("))
         (for [word ["drain-file" "drain_file" "drain_marker" "drain-marker" "drain-declared"]]
           (assert (not (any (gfor line (live-bare-lines path [proof]) (in word line))))
                   f"{path.name} が印を知っている — 答えを値で受けるだけのはず(R-…-002c): {word}")))
       (setv row-body (defk-body (code-lines (/ SESSIONHOST-DIR "headless.hy")) "stop-headless-row"))
       (assert (any (gfor line row-body (in "(stop-cause-category declared)" line))) "stop-headless-row が語を stop-cause-category の 1 点から受けない(R-…-002c)")
       (assert (not (any (gfor line row-body (in "\"cancelled\"" line)))) "stop-headless-row が cancelled の逐語を持つ — 印の答えを捨てる形(R-…-002c)")
       (assert (not (any (gfor line host-lines (in "stopped/cancelled" line)))) "停止の log が語を固定の文字列で書いている(R-…-002c)")
       (assert (any (gfor line host-lines (in "(stop-cause-category declared)" line))) "停止の log の語が行の語から導かれていない(R-…-002c)")
       (assert (= (stop-cause-category True) CAUSE-CATEGORY-HOST-DRAINED "host_drained") "3 つの語の写しが食い違う(R-…-002c / R47)")
       (assert (= (stop-cause-category False) "cancelled") "印の無い停止の語が今日の cancelled でない(R-…-002c)")
       (assert (is (get TERMINAL-CAUSE-RETRYABLE CAUSE-CATEGORY-HOST-DRAINED) True) "host_drained が policy の閉語彙に retryable で無い(R-…-002c)")
       (setv tests (.read-text (/ (. (Path __file__) parent parent parent) "packages" "doeff-agents" "tests" "test_sessionhost_headless.py") :encoding "utf-8"))
       (for [name ["test_real_host_sigterm_under_the_drain_marker_cuts_the_turn_as_host_drained_and_leaves_the_marker_alone"
                   "test_real_host_sigterm_without_the_marker_stays_cancelled_and_the_host_writes_no_marker"
                   "test_real_host_sigterm_without_the_env_stays_cancelled_even_with_a_marker_at_the_default_place"
                   "test_the_term_handler_reads_the_marker_once_at_the_first_term_and_never_again"]]
         (assert (in (+ "def " name "(") tests) f"R-…-002c の反例の検が無い: {name}")))
     (deftest test-adr-doe-agents-012-the-drain-marker-path-env-is-spelled-at-two-points
       ;; R-the-drain-marker-path-env-is-spelled-at-two-points-c530 の針(構造): env の名の literal を持つ source は host.hy
       ;; (ENV-DRAIN-FILE の定義)と entry.py(HOST_DRAIN_FILE_ENV の写し)の 2 つちょうどで、2 つの値は同じ・entry が
       ;; env を立てる点は `role == JOIN_ROLE_HOST` の枝の中だけ(役 both の挙動は law の :enforcement の検が撃つ)。
       (import doeff_agents.sessionhost [host :as host-module])
       (import doeff_agents.sessionhost.acp [entry])
       (assert (= host-module.ENV-DRAIN-FILE entry.HOST-DRAIN-FILE-ENV "DOEFF_SESSIONHOST_DRAIN_FILE")
               "印の path の env の 2 つの綴りが食い違う(R-…-c530)")
       (setv spelled (sfor path (source-files)
                           :if (in "\"DOEFF_SESSIONHOST_DRAIN_FILE\"" (.read-text path :encoding "utf-8"))
                           path.name))
       (assert (= spelled #{"host.hy" "entry.py"}) f"印の path の env の綴りが 2 点の外に在る(R-…-c530): {spelled}")
       (setv entry-lines (code-lines (/ ACP-DIR "entry.py")))
       (setv writes (lfor [i line] (enumerate entry-lines) :if (in "os.environ[HOST_DRAIN_FILE_ENV]" line) i))
       (assert writes "entry が印の path の env を立てていない(R-…-c530)")
       (for [at writes]
         (assert (in "drain_file_path(os.environ)" (get entry-lines at)) "entry が置き場を drain_file_path の 1 点から導いていない(R-…-c530)")
         (setv guard (next (gfor line (reversed (cut entry-lines 0 at)) :if (.startswith (.lstrip line) "if ") line) ""))
         (assert (in "role == JOIN_ROLE_HOST" guard) f"entry が役 host の枝の外で印の path を立てている(R-…-c530): {guard}"))
       (setv tests (.read-text (/ (. (Path __file__) parent parent parent) "packages" "doeff-agents" "tests" "test_sessionhost_acp.py") :encoding "utf-8"))
       (assert (in "def test_entry_main_hands_the_drain_marker_path_to_the_host_role_only(" tests)
               "R-…-c530 の反例の検が無い: test_entry_main_hands_the_drain_marker_path_to_the_host_role_only"))
     (deftest test-adr-doe-agents-012-the-drain-marker-module-only-reads
       ;; R-drain-marker-has-no-writer-7de2 の針(構造・ast): drain_marker.py の公開は declared / reason_line の 2 つちょうどで、
       ;; file を書く・作る・消す・名を変える呼びを持たない。針そのものが生きていることを変異体で示す(書きを足した source は赤)。
       (import ast)
       ...
       (defn #^ list writes-in [#^ str source]
         ...
         (for [node (ast.walk (ast.parse source))]
           (when (isinstance node ast.Call)
             (setv func node.func)
             (when (and (isinstance func ast.Attribute) (in func.attr forbidden))
               (.append found func.attr))
             (when (and (isinstance func ast.Attribute) (= func.attr "open")
                        (isinstance func.value ast.Name) (= func.value.id "os"))
               (.append found "os.open"))
             (when (and (isinstance func ast.Name) (= func.id "open"))
               (setv mode (if (> (len node.args) 1)
                              (get node.args 1)
                              (next (gfor kw node.keywords :if (= kw.arg "mode") kw.value) None)))
               (when (not (or (is mode None)
                              (and (isinstance mode ast.Constant) (isinstance mode.value str)
                                   (not (any (gfor ch "wax+" (in ch mode.value)))))))
                 (.append found "open(write)")))))
         found)
       (setv source (.read-text (/ SESSIONHOST-DIR "drain_marker.py") :encoding "utf-8"))
       (setv tree (ast.parse source))
       (setv public (sfor node tree.body
                          :if (and (isinstance node ast.FunctionDef) (not (.startswith node.name "_")))
                          node.name))
       (assert (= public #{"declared" "reason_line"}) f"drain_marker の公開が読みの 2 つちょうどでない(R-…-7de2): {public}")
       (assert (= (writes-in source) []) f"drain_marker が書きの呼びを持つ(R-…-7de2): {(writes-in source)}")
       (for [mutant ["def declare(path):\n    open(path, 'x').close()\n"
                     "def declare(path):\n    open(path, mode='w').write('x')\n"
                     "from pathlib import Path\ndef declare(path):\n    Path(path).touch()\n"
                     "import os\ndef clear(path):\n    os.unlink(path)\n"]]
         (assert (writes-in mutant) f"書きの針が変異体を見逃す(針が死んでいる): {(repr mutant)}"))
       (setv tests (.read-text (/ (. (Path __file__) parent parent parent) "packages" "doeff-agents" "tests" "test_sessionhost_headless.py") :encoding "utf-8"))
       (assert (in "def test_the_drain_marker_is_read_by_presence_and_its_line_is_only_a_reason(" tests)
               "R-…-7de2 の反例の検が無い: test_the_drain_marker_is_read_by_presence_and_its_line_is_only_a_reason"))
     ...
     (deftest test-adr-doe-agents-012-the-agentd-puts-its-own-record-destination-on-the-seat
       ;; R-the-agentd-puts-its-own-record-destination-on-the-seat-1601 の検査(構造): 走行者の名の集合は effects の 1 か所・
       ;; node が手番へ渡す env を組むのは judgment.node-seat-env-of の 1 関数で、agentd.hy では charter と帰属の指紋が同じ組を読む・
       ;; 起こし方の判断と片付けの理由は指紋を引数に取る・継承の名簿と送りの手番の env と host の file は走行者の名を知らない。
       ;; 挙動の反例は law の :enforcement に並べた検が持つ。
       (assert (= (set RUNNER-OWNED-SEAT-ENV) #{CONVERSATION-ID-ENV SEAT-OPENER-ENV RECORD-URL-ENV})
               f"走行者が持つ名の集合が動いた(R-…-1601 (1)): {RUNNER-OWNED-SEAT-ENV !r}")
       ;; 組は 1 関数: 宣言の対(走行者の名は落とす)→ 記録の宛先。記録が無効なら宛先を置かない。
       (setv declared #(#("SERVICE_URL" "http://service.test") #(RECORD-URL-ENV "http://bypassed.test")))
       (assert (= (run (node-seat-env-of (AgentdSettings :node-name "mac-1" :seat-env declared
                                                         :record-url "http://record.test:8874")))
                  #(#("SERVICE_URL" "http://service.test") #(RECORD-URL-ENV "http://record.test:8874")))
               "node が手番へ渡す env は宣言の対の後に走行者の宛先(門を迂回した同じ名は落ちる)(R-…-1601 (3))")
       (assert (= (run (node-seat-env-of (AgentdSettings :node-name "mac-1" :seat-env declared)))
                  #(#("SERVICE_URL" "http://service.test")))
               "記録が無効なら宛先の名を置かない(R-…-1601 (2))")
       (setv judgment-lines (code-lines (/ ACP-DIR "judgment.hy")))
       (for [name ["node-seat-env-of" "node-seat-env-digest-of" "session-node-env-of"]]
         (assert (= (len (lfor line judgment-lines :if (.startswith line f"(defk {name} ") line)) 1)
                 f"組・指紋・帰属の読みは judgment の 1 点ずつ(R-…-1601 (3)): {name}"))
       (assert (any (gfor line (defk-body judgment-lines "session-attribution-of") (in "(node-seat-env-digest-of " line)))
               "帰属の指紋は node-seat-env-digest-of の 1 点から(R-…-1601 (3))")
       (assert (any (gfor line (defk-body judgment-lines "next-arm-for-job") (in "(session-node-env-of " line)))
               "起こし方の判断が温かい session の指紋を読まない(R-…-1601 (4))")
       ;; agentd.hy で組を読む腕は名で決まる: incarnate(charter と帰属)・claim-job(起こし方)・start-claimed(片付けの理由)。
       (setv agentd-path (/ ACP-DIR "agentd.hy"))
       (setv readers (set (readers-of [agentd-path] "(node-seat-env-of ")))
       (assert (= readers #{"incarnate" "claim-job" "start-claimed"})
               f"node が手番へ渡す env を読む腕が名簿と違う(R-…-1601 (3)): {readers}")
       (setv incarnate-body (defk-body (code-lines agentd-path) "incarnate"))
       (setv attributed (call-args-of incarnate-body "session-attribution-of"))
       (setv chartered (call-args-of incarnate-body "incarnation-charter-of"))
       (assert (and (= (len attributed) 1) (= (len chartered) 1)) f"incarnate の帰属と charter の呼びが 1 つずつでない: {attributed} {chartered}")
       (assert (= (get attributed 0 -1) (get chartered 0 -1))
               f"帰属の指紋と charter が同じ組を読まない(R-…-1601 (3)): {(get attributed 0 -1)} / {(get chartered 0 -1)}")
       ;; (5) してはいけないこと: 継承の名簿・送りの手番の env・host の file は走行者の名を知らない。
       (assert (not (& SPAWN-INHERITED-ENV-KEYS (set RUNNER-OWNED-SEAT-ENV)))
               "継承の名簿に走行者の名が在る(R30 (4)・R-…-1601 (5))")
       (for [word ["RUNNER-OWNED-SEAT-ENV" "RECORD-URL-ENV" "node-seat-env-of"]]
         (assert (not (any (gfor line (defk-body judgment-lines "turn-session-env-of") (in word line))))
                 f"送りの手番の env が走行者の名を運ぶ(R-…-1601 (5)): {word}")
         (for [name ["headless.hy" "launch.hy" "policy.hy" "substrate_headless.hy"]]
           (for [line (bare-code-lines (/ SESSIONHOST-DIR name))]
             (assert (not-in word line) f"host が走行者の名を知っている(R-…-1601 (5)): {name} {line}"))))
       ;; 反例の検が在る(字の分割は台帳の数え方への配慮)。
       (setv root (. (Path __file__) parent parent parent))
       (setv acp-tests (.read-text (/ root "packages" "doeff-agents" "tests" "test_sessionhost_acp.py") :encoding "utf-8"))
       (for [name ["test_join_refuses_seat_env_that_copies_the_record_destination"
                   "test_settings_carry_the_record_destination_that_passed_the_join_gate"
                   "test_runner_owned_seat_env_names_have_one_home"
                   "test_the_runner_record_destination_wins_over_a_seat_env_that_bypassed_the_gate"
                   "test_next_arm_resumes_a_warm_session_launched_with_another_node_env"
                   "test_join_refuses_seat_settings_whose_env_block_names_a_runner_owned_name"]]
         (assert (in (+ "def " name "(") acp-tests) f"R-…-1601 の反例の検が無い: {name}"))
       (setv headless-tests (.read-text (/ root "packages" "doeff-agents" "tests" "test_sessionhost_headless.py") :encoding "utf-8"))
       (assert (in "def test_the_seat_reads_the_record_destination_of_the_agentd_that_woke_it(" headless-tests)
               "R-…-1601 の反例の検が無い: 器が起こした process の env を読む検"))
     ;; R-the-agentd-puts-its-own-record-destination-on-the-seat-1601 (1) の綴りの禁止(installed rule・.semgrep.yaml):
     ;; 走行者の名は effects.py の定義行だけで綴る。
     (defsemgrep runner-owned-seat-env-spelled-once
       "doeff-agents-runner-owned-seat-env-spelled-once"
       [{"relative-path" "packages/doeff-agents/src/doeff_agents/sessionhost/acp/judgment.hy"
         "source" "(setv pairs (+ declared #(#(\"RECORD_SERVICE_URL\" settings.record-url))))\n"}
        {"relative-path" "packages/doeff-agents/src/doeff_agents/sessionhost/acp/join.hy"
         "source" "(setv owned #{\"AGORA_CONVERSATION_ID\" \"AGORA_SEAT_OPENER\"})\n"}]
       [{"relative-path" "packages/doeff-agents/src/doeff_agents/sessionhost/acp/effects.py"
         "source" "RECORD_URL_ENV = \"RECORD_SERVICE_URL\"\nCONVERSATION_ID_ENV = \"AGORA_CONVERSATION_ID\"\n"}
        {"relative-path" "packages/doeff-agents/src/doeff_agents/sessionhost/acp/judgment.hy"
         "source" "(setv pairs (+ declared #(#(RECORD-URL-ENV settings.record-url))))\n"}])
  ...
  :plans ["docs/impl-requests/stage2-lane-prompts/lane-2b-agentd.md(agora-redesign)"
          ...
          "agora-redesign の盤 card acp:kanban-issue:ki-e930b8506201(追補 R-the-agentd-puts-its-own-record-destination-on-the-seat-1601 と R30 (4) / R51 (1)(c) / R51 (4) の改訂・依頼 lt-2R3K7D7E9WZTC049BFYATMK2S9: agentd は参加の門を通った [record].url を起こす手番の env に自分で置き、温かい session は node が手番へ渡す env の指紋で send と resume を分ける — 設計と実装依頼書 = docs/design/seat-record-destination-ki-e930/)"])
