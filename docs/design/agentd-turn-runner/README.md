# agentd を doeff の中で置き換える — 呼び名・10 件の決定・移る順番

- 決めた人: operator(方針 "B"・2026-09-25)/ 細目 10 件はこの設計の持ち主の会話 c-01M369MCR3BRGFK80EZ8C5C1QG が
  「戻せる設計の決定」として決めた(2026-09-25)
...
- 関係する文書: herdr-hud `docs/design-checks/simplify-irai-2026-09-25/design.md`(依頼の抽象の設計 第 4 版 —
  手番のスケジューラの持ち主 = 会話 7fb8632d)。この file の呼び名はそちらへも写す(写すのは向こうの持ち分)
...
どれも戻せる決定。報告書の推奨を採り、operator の既存の指示とぶつかる所は指示に合わせた(該当は「指示との照合」に書く)。

### 決定 1 — 機体の生存の正本

- 決めたこと: 当面は、手番の実行係が doeff coordinator の worker の生存(heartbeat)を ACP の node の行へ写す。
...
### 決定 2 — agent 実行 task の寿命

- 決めたこと: 「依頼 id で 2 度実行しない + 送り直された時は CLI の transcript から `--resume`」で受ける。
...
### 決定 3 — report_result の MCP 中継(relaymain)

- 決めたこと: headless の経路では外す。結果は stream-json の result 行(claude)/ `turn/completed`(codex)から取る。
...
### 決定 4 — 実況(events_since)の読み口

- 決めたこと: 実況は出来事の記録の hot 層を effect(`HeadlessEventsSince`)で読む。本番の handler = ClickHouse の hot 層、
...
- 決めたこと: 全機体で 1 service。書き手は lease-fence で 1 つ。機体は task の `requires`(role=agent・host=<名>)で選ぶ。
...
### 決定 6 — fake の店が 2 つある件

- 決めたこと: agora_sim の店へ FakeAcp の検め(principal・書き手の名簿・schema)を移して 1 つにする。移し先は ACP の repo の
  fake の店(`clients/hy/acp_client/runtime/handlers_fake.hy` の正本 — agora_sim はその写し)で、持ち主は agora_sim の取りまとめ役
  (合意済み 2026-09-25)。実行係の模擬はその検めが入った店でも通るように書く。
- 理由: 契約の検めが 2 か所にあると、片方でだけ通る書き(本番で断られる書き)を模擬が見逃す。
- 戻し方: 実行係の模擬を `sessionhost/acp/fake.py` の FakeAcp に向け直す。

### 決定 7 — 実行係の code の置き場

- 決めたこと: 判断の純関数(`judgment.hy`)と agent 実行 task の親(argv の組み立て・stream-json の解釈・終わりの判定)は
...
### 決定 8 — cache-operation の行を作る側

- 決めたこと: 当面は今のまま(acp-messaging / acp-cache-keepalive が作る)。実行係は行を実行するだけ。模擬に ping の筋書きを足す。
- 理由: 作り手を動かすとスケジューラの設計(第 4 版)に波及する。実行の側の置き換えと同時に動かさない。
- 戻し方: 不要(今と同じ)。

### 決定 9 — 所有者不明の常駐 `doeff-sessionhost serve`(Mac・PID 48736)

- 決めたこと: 止めない・消さない。読むだけで正体を調べて記録し(下の 4 節)、誰が使っているかを 1 日の socket の接続で測ってから
  止めるかを決める。止める・unit を消すのは operator の明示の指示があった時だけ(自分で作っていない物は消さない規則)。
- 戻し方: 不要。

### 決定 10 — pool の pod の home と checkout

- 決めたこと: home の PVC(seat-home-state)は残す。cache と transcript の身元は git の状態 + env で持つ(operator 決定 2026-09-21)。
...
| 順 | すること | 今回 |
|---|---|---|
| R0 | 呼び名をこの文書と図に写す・`sessionhost/acp/README.md` に「agentd = ACP への腕」 | する |
| R1 | 実行係の Program(今の agentd の Program そのもの)を agora_sim に載せ、fake の agent を置き換える。業務の不変量と反例 | する |
| R2 | 境界を CLI の process まで下げる: agent 実行 task の親を子 process の effect の上に書き、模擬では筋書きの fake が stream-json を返す | する |
| R3 | 出来事の effect(`HeadlessEventAppend` / `HeadlessEventsSince`)を task の親から出す。模擬 = メモリ | する |
| R4 | (影の実行は採らない)本番の記録からの backtest(3 節) | する(手元の模擬だけ) |
