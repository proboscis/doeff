...
**次に着手できること**
- 段 1: doeff-agents の実装と検(この worktree で進行中)。開始条件 = 無し。
- 段 2: ACP の `charterFor` に欄を通し、起票側が役ごとの値を宣言する。開始条件 = 段 1 の着地。
- 配備: agentd は uv tool として導入済みなので、再導入と常駐の入れ替えが要る(段 1 の完了条件に含む)。
