...
## 5. 読む材料(読み取りだけ・作業樹 /home/kento/.worktrees/doeff-wt-639-sessionhost-defk・HEAD = a3f34bb68d394c4153c7c7351f07801ebf2827ab)

⚠ 作業樹の `.scratch/` の下は読まないこと(評価の対象外の作業置き場)。

- packages/doeff-agents/src/doeff_agents/sessionhost/store.hy — db-immediate-transaction・lease の本体と入口(1000〜1125 行付近)、StoreActor(1180〜1275 行付近)
- packages/doeff-agents/src/doeff_agents/sessionhost/host.hy — store-write-failure-limit と dispatch-method の daemon.status(1335〜1380 行付近)、lease の呼び手(`db-heartbeat-once` / `db-acquire-lease` / `db-release-lease` を grep)
- packages/doeff-agents/src/doeff_agents/sessionhost/store_health.py
- packages/doeff-agents/tests/sessionhost_store_deftests.hy(lease の検 420〜560 行付近)・packages/doeff-agents/tests/sessionhost_host_deftests.hy
- docs/adr/defadr_doeff_hy_004_defk_only.hy・docs/adr/defadr_doeff_agents_004_effects_session_host.hy(R15 と法 2 本)
- packages/doeff-hy/src/doeff_hy/macros.hy(defk の展開)・doeff/run.py(run の例外の扱い)
- .semgrep.yaml(静的規則)
- 変更の差分: `git -C /home/kento/.worktrees/doeff-wt-639-sessionhost-defk show a3f34bb6`

## 6. 検査の実体と実行の命令(この変更で使った範囲)

- 台帳の針: `uv run pytest -q "docs/adr/defadr_doeff_hy_004_defk_only.hy::test_adr_doe_hy_004_deff_ratchet"` — file ごとの `(deff` の本数 <= DEFF-ROSTER(正規表現 `\(deff\s`)。
- R15 の振る舞いの検: `uv run pytest -q packages/doeff-agents/tests/test_sessionhost_store.py packages/doeff-agents/tests/test_sessionhost_host.py`(sessionhost_store_deftests.hy / sessionhost_host_deftests.hy の deftest をそのまま公開)。
- semgrep: `semgrep --config .semgrep.yaml --error <file>`(repo の静的規則。この変更の 3 file で 0 件)。
- Hy の型の検査: `uv run doeff-hy-check --root packages/doeff-agents/src <file>`(pyright へ投影。既存の誤りが在る)。
- 変更箇所の品質検査: dotfiles の `code-quality --scope changed`(fast の段・型は撃たない)。
- 全体の実行はしない(日次が行う)。
