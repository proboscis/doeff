...
- 追補(2026-09-26 07:20 JST): 依頼書 T の検収の後、日次の機体 zeus で M5 が赤になった(F11)。調べる中で ADR-DOE-ENFORCE-001 R6 の印の伸ばしが効いていないことが分かった(F12)。M5 の予算の改訂と M10 を足し、S9・C29〜C33 に記録した。盲検は取り直していない — 反例は盲検ではなく日次の機体の実測(C29)で見つかり、直し方は試作を同じ機体で走らせて確かめた(C33)。
...
| F11: 日次の機体での冷えた compile の所要(追補・依頼書 T の検収の後に測った) | 日次の機体 zeus(36 コア・負荷 約 1.0)・子の bytecode の置き場を 1 つ共有・期限なし: `adapters.base` 6.7 秒 → `host_slot_cli` 73.0 秒 → `runtime` 78.2 秒 → `runtime` の 2 回目 1.5 秒。**`runtime` を単独で冷えた状態から: 156.8 秒**。同じ 2 本は Mac で各 19〜35 秒(F8)で、zeus は同じ負荷率でも約 2.5〜4 倍遅い。本線 5eef352d(依頼書 T の実装)の fresh interpreter の検を zeus で走らせると 2 failed 20 passed(子の期限 59 秒 = 負荷 x1.2 の検 1 本の期限 74 秒 − 15 秒) |
...
| C29(追補) | S9 | 反 | 本線 5eef352d(依頼書 T の実装)の fresh interpreter の検を日次の機体 zeus で | 2 failed 20 passed・`host_slot_cli` と `runtime` が「did not finish within 59s」 | `~/.local/state/design-check/lt-FPBECBQ3W22ESC92VAMCENR5JR/evidence/M5-C29-zeus-5eef352d.log` |
