...
- 追補(2026-09-26 07:20 JST): 依頼書 T の検収の後、Linux の機体 zeus で M5 が赤になった(F11)。調べる中で ADR-DOE-ENFORCE-001 R6 の印の伸ばしが効いていないことが分かった(F12)。M5 の予算の改訂と M10 を足し、S9・C29〜C33 に記録した。盲検は取り直していない — 反例は盲検ではなく zeus の実測(C29)で見つかり、直し方は試作を同じ機体で走らせて確かめた(C33)。
- 訂正(2026-09-26 07:55 JST・依頼者の指摘): 初版の追補は zeus を「日次の機体」と書いたが誤り。日次の log(09-25・09-26)の remote-check の行は `ran=local node=zeus … reason="… s22625@zeus: Permission denied (publickey)."` で、日次は cluster の pod(`land-verify-doeff-*`・linux)の中で走っている。zeus は日次の代わりの Linux の機体として測った。pod での所要の見積もりは F13。
...
| F11: Linux の機体 zeus での冷えた compile の所要(追補・依頼書 T の検収の後に測った) | zeus(36 コア・負荷 約 1.0・日次の実行環境ではない — F13)・子の bytecode の置き場を 1 つ共有・期限なし: `adapters.base` 6.7 秒 → `host_slot_cli` 73.0 秒 → `runtime` 78.2 秒 → `runtime` の 2 回目 1.5 秒。**`runtime` を単独で冷えた状態から: 156.8 秒**。同じ 2 本は Mac で各 19〜35 秒(F8)で、zeus は同じ負荷率でも約 2.5〜4 倍遅い。本線 5eef352d(依頼書 T の実装)の fresh interpreter の検を zeus で走らせると 2 failed 20 passed(子の期限 59 秒 = 負荷 x1.2 の検 1 本の期限 74 秒 − 15 秒) |
| F13: 日次の実行環境(pod)での所要の見積もり(訂正で足した) | 日次は cluster の pod の中で走る(zeus への ssh が断られ、remote-check が手元の実行に倒れる — 09-26 の log 605・1371 行)。pod での冷えた import は直接測っていない。上限の見積もり: 09-26 の日次(pod・負荷 x1.5・検 1 本の期限 90 秒・直す前の形 = 子ごとに全部を compile)で、`host_slot_cli` の子は 90 秒の内に通り、`runtime` の子が 90 秒を超えた。zeus で `host_slot_cli` を冷えた状態から import すると約 78 秒(`adapters.base` の分を含む)なので、pod の所要は zeus の約 1.15 倍以内。`runtime` を単独で冷えた状態から import しても 157 × 1.15 ≈ 181 秒で、T2 の子の期限(240 秒 × 負荷の係数 − 15 秒 ≥ 225 秒)に収まる。日次の並び(`host_slot_cli` の後に `runtime`)ではそれぞれ 90 秒前後の見込み |
| F12: 検ごとの期限の印が負荷で伸びていない(追補) | 根の conftest の `pytest_collection_modifyitems` は伸ばした timeout の印を `add_marker` の既定(後ろに足す)で足す。pytest-timeout と `_watchdog_timeout_for_item` は `get_closest_marker` で最初の印(伸ばす前)を読むので、`@pytest.mark.timeout(10)` の検の実効の期限は負荷 x2.0 でも 10.0 秒のまま。ADR-DOE-ENFORCE-001 R6 の「marker の締切も同じ係数で伸ばす」が 2026-08-17(156d1cc1)から効いておらず、それを確かめる検も無い |
...
| M5 fresh-interpreter-import-test(doeff-agents) | Hy を import する Python の module が、新しい interpreter でも自分で hy を読み込んでから import できることを確かめる | 子の bytecode の置き場 = **pytest の一時 dir の下の、この module の走行だけの dir**(`tmp_path_factory`)・子の期限 = 検 1 本の期限 − 15 秒 | 走行全体の bytecode の設定(M9 の固定)は変えない | module scope の fixture `private_bytecode_dir` と `child_deadline`。子の env だけ `PYTHONDONTWRITEBYTECODE` を外し `PYTHONPYCACHEPREFIX` をその dir へ向ける。子の期限切れは `pytest.fail("<module> did not finish within Ns …")` | 一時 dir・子 process | module の走行 | 子の超過はその検の赤で、process を殺さない。木に bytecode を書かない(M9 が走行の終わりに確かめる) | **改訂**: 静的な検を足す予定をやめた(共有の私的な置き場だけで 22 本 42 秒 — 実測 C16)。木を汚さないことの強制を M5 の約束から M9 の検査へ移した(盲検 B)。**追補の改訂**(F11): 予算を Mac だけで確かめていた。検が自分の予算を Linux の機体 zeus の実測と日次の pod の上限の見積もり(F13)から**宣言する**(`@pytest.mark.timeout(240)` — zeus で `runtime` を単独で冷えた状態から 157 秒 + 余裕・印は M10 で負荷に伸びる)。子の期限は `config.option.timeout` ではなく**その検自身の実効の期限** − 15 秒 |
...
| **M10 marker-deadline-scaling(根の conftest・追補で改訂)** | 検が印で宣言した期限を、ini の期限と同じ負荷の係数で伸ばし、pytest-timeout と watchdog がその伸ばした値を読むようにする(ADR-DOE-ENFORCE-001 R6) | 負荷の係数(`deadline_scale` の 1 点)と、pytest-timeout・watchdog が印を読む規則(最も近い印) | 検ごとに宣言した期限の元の値の持ち方 | 伸ばした印を最も近い印として置く(先頭に足す)。`_watchdog_timeout_for_item` は同じ印から 30 秒以上上に居る | なし(収集の時に印を足すだけ) | 収集 1 回 | 印つきの検の実効の期限 = 宣言 × 係数。watchdog は常にその上 | **追補で改訂**(F12)。元の実装は 156d1cc1 |
...
| S9(追補) | hardware | 日次の実行環境(cluster の pod)や Linux の機体が、開発機より同じ負荷率でも数倍遅い(zeus は Mac の約 2.5〜4 倍・pod は zeus の約 1.15 倍以内 — F13)。検の所要は負荷の係数では説明されない分だけ機体に依る | なし(S5・S6 の主張: 共有の置き場で所要は compile 1 回分、期限は負荷で伸びる) | **M5 と M10**(依頼書 T の検収の後、zeus で 2 failed — F11) | 設計の誤り: M5 の予算を Mac の実測だけで確かめ、検 1 本の既定の期限(60 秒 × 負荷)で足りると置いた。検が自分の所要を宣言する口(timeout の印)は在ったが、F12 のとおり印は負荷で伸びていなかった — 宣言しても R6 の約束が守られない。知識の漏れではなく、**予算の宣言の欠けと、R6 の強制の欠け**(伸ばす実装が在っても、実効の値を確かめる検が無かった) | M10 を直し(伸ばした印を最も近い印に・C31 で旧形の実効 10.0、C32 で 21.2)、M5 が zeus の実測と pod の見積もり(F13)から予算を宣言する(240 秒)。試作を zeus で走らせて 22 passed・`runtime` 単独 passed(C33)。本実装は依頼書 T2 |
...
| M5 の予算と置き場 | 子の期限を検 1 本の実効の期限から導く(固定値を持たない)・検の予算は開発機より遅い Linux の機体の実測(と日次の pod の上限の見積もり F13)から timeout の印で宣言し、根拠(機体・日付・実測値)を印の隣に書く(追補)・子の置き場は `tmp_path_factory` の下 | `packages/doeff-agents/tests/test_hy_loader_declared_by_importers.py` | packages 処理ステージ | cache 無しの compile 1 回分が宣言した予算 × 負荷の係数をそれでも超える機体では、その検が名前つきの赤になる。予算の値は測った日の compile の所要に依る(Hy の stack が重くなれば測り直す) |
| M10 の「印の期限も負荷で伸びる」(追補) | R6 の挙動本体 `tests/test_deadline_load_scaling.py` に、印つきの検の実効の期限(pytest-timeout 自身の解決で読む)が係数倍になり、watchdog がその 30 秒以上上に居ることを確かめる検を足す。負荷は固定して決定的に | 根の `conftest.py`・`tests/test_deadline_load_scaling.py` | root 処理ステージ | pytest-timeout が印を読む規則(最も近い印)を変えたら検が赤で知らせる |
...
| C34(訂正で足した) | S9 | 反 | 09-26 の日次の log を読み直す: 日次は pod の中で走り(`ran=local node=zeus … Permission denied (publickey)`)、直す前の形で `host_slot_cli` の子は 90 秒の内に通り `runtime` の子が 90 秒を超えた | 15 本目まで通り 16 本目(`runtime`)が Timeout(負荷 x1.5・検 1 本の期限 90 秒)— F13 の見積もりの根拠 | `~/.local/state/design-check/lt-FPBECBQ3W22ESC92VAMCENR5JR/evidence/daily-20260926-033008.log`(605・1371・1881・1909 行) |
| C29(追補) | S9 | 反 | 本線 5eef352d(依頼書 T の実装)の fresh interpreter の検を Linux の機体 zeus で | 2 failed 20 passed・`host_slot_cli` と `runtime` が「did not finish within 59s」 | `~/.local/state/design-check/lt-FPBECBQ3W22ESC92VAMCENR5JR/evidence/M5-C29-zeus-5eef352d.log` |
| C30(追補) | S9 | 反 | zeus で期限なしの import の所要(予算の根拠) | `host_slot_cli` 73.0 秒・続く `runtime` 78.2 秒・`runtime` 単独で冷えた状態から 156.8 秒 | 同じ dir の `M5-C30-zeus-import-timing.log` |
| C31(追補) | S9 | 反 | 本線 5eef352d で `@pytest.mark.timeout(10)` の検の実効の期限を読む | 負荷 x2.0 で印は `[(10,), (20.2,)]`・実効 10.0(伸びていない) | 同じ dir の `M10-C31-mark-scaling-before.log`・`proto/m10_mark_probe.py.txt` |
| C32(追補) | S9 | 正 | 試作(伸ばした印を先頭に足す)で同じ検 | 印は `[(21.2,), (10,)]`・実効 21.2 | 同じ dir の `M10-C32-mark-scaling-after.log`・`proto/m10_mark_scaling_and_budget.patch` |
| C33(追補) | S9 | 正 | 試作(M10 + M5 の予算 240 秒 + 子の期限を実効の期限から)を zeus で | 22 passed in 164 秒(遅い 2 本 79 秒・70 秒)・`-k runtime` 単独 1 passed(171 秒) | 同じ dir の `M10-zeus-prototype.log` |
...
| P・H・K(追補・設計の module の外) | 09-26 の 7 本の外の赤 4 本(2 日間測られていなかった doeff-agents の範囲から名指しの実行で見つけた — policy の固定の表・cache_maintenance の世界の handler・herdr の `argv0`・custody の checkout の前提)。K は M3 を「道具の起動」の外の前提へ広げる | P・H は受付経由、K は C の担当(依頼書は #639 comment 5840485606) | 独立 |
| T2(追補) | M10 → M5 の予算 | T の担当 c-X0TQHCF12MFMSWKKBFD9WT4B4Z(依頼 lt-M3YQGYDE30Y35A83WYX81FN1T9・依頼書は #639 comment 5840228500) | T の後。M10 を先に(M5 の印が負荷で伸びる前提)。受入条件に zeus での 22 passed と `-k runtime` 単独を含む |
...
- (追補)sessionhost の Hy の stack を冷えた状態から compile する所要そのもの(zeus で 157 秒)は直さない。M5 はその所要を予算として宣言するだけ。所要が伸びれば M5 は名前つきの赤で知らせ、予算を測り直す。
