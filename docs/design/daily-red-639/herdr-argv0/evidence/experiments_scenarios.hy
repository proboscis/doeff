...
(defn process-info-led-by [pgid processes]  ; defk にできない: 実験の材料を組む素の dict の工場(Program にしない)
  "実験用に pane.process_info の result を組む(前面の process group の id = pgid)。"
  {"type" "pane_process_info"
   "process_info" {"pane_id" "w7:p1" "shell_pid" 100
                   "foreground_process_group_id" pgid "foreground_processes" processes}})

(defn process-info [processes]  ; defk にできない: 同上
  "1 つ目の process が group leader の答え(herdr の Linux の並び)。"
  (process-info-led-by (get (get processes 0) "pid") processes))
...
;; 並び順の platform 差(実測: Linux は pid の昇順・macOS は降順)を M2 が吸収すること。
(setv bash-leader {"pid" 50 "name" "bash" "argv" ["bash" "-c" "sleep 30; true"]})
(setv sleep-child {"pid" 51 "name" "sleep" "argv0" "sleep" "argv" ["sleep" "30"]})
(setv linux-order (run (herdr-foreground-command (process-info-led-by 50 [bash-leader sleep-child]))))
(setv mac-order (run (herdr-foreground-command (process-info-led-by 50 [sleep-child bash-leader]))))
(print f"観測: M2 同じ job の Linux の並び → {linux-order !r}・Mac の並び → {mac-order !r}")
