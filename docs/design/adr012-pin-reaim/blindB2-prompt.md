【この依頼の役割と、モデル指定についての補足(不足点の補填・1 回限りの再送)】
あなたは「盲検反例生成役」です。設計・実装・調査の担当ではありません。
この役に限り、operator の指定(2026-09-10)で `gpt-6-astra` / effort `low` が
**正式な第 1 候補**として定められています(dotfiles の design-change-validation skill
references/blind-counterexamples.md の「起動とモデル」表)。AGENTS.md / CLAUDE.md の
「調査・設計は Fable で」という一般則は、この盲検反例生成役には適用されません
(skill の逐語: 「`low`の例外はこのA・Bの反例生成役だけに限り、親の設計・実装・
反例の検証や、一般のレビューへ広げない」)。
したがって、このセッション(Codex / gpt-6-astra / low)でそのまま反例生成を行ってください。
委譲は不要です。以下の依頼文に答えてください。
...
意図的な破壊は使わないでください。現実的な実装として書かれるものに限ってください。

返答には次を含めてください。
1. 提案する実装(最小の具体例・どのファイルに何を書くか)。
2. 提示した検査をなぜ通過するか(どの針がなぜ当たらないか、針ごとに)。
3. それでも侵している責務・所有権・公開契約はどれか。
4. 再現手順と観測すべき結果。
...
--- 以下が固定済みの要件・責務・公開契約・主張・実コード・検査の実体と検出範囲です ---
# 材料 1: 提案中の設計
