...
| B6 | 状態×継続 共有 / スナップショット | **共有**(cells捕獲時コピーなし) | src/var_store.rs、vm/var_store.rs(コピー箇所なし) | 分岐意味論、トランザクショナルロールバック、multi-shot拡張 | **owner: 意図的・確定**(one-shot前提で共有が正しい)。law未明文 → Task 4で昇格。SPEC-EFF-002のknown issue(gh#157 snapshot isolation)は本回答と矛盾、spec更新要。※追記: VMの生きた状態面は**cells+スコープ束縛のみ** — `global_state`/`writer_log`は呼び出し元ゼロの死設備と判明(§4-⑥)。正統のState/Writerハンドラはクロージャ実装(algebra-draft.md §0) |
...
| B14 | 変数の所在 セグメント所有(owner_segment) / ヒープセル | **owner_segment現役** | vm/var_store.rs:140,156,179 | — | **spec間矛盾**: SPEC-VM-020:191は「存在しないべき」、翌日のSPEC-VM-019 Rev 2(`f4166b12`)が「segments are the scope」。後者が後勝ちと推定 [推測]。要決着(§4-④) |
...
| ③ | `append_writer_log(_seg_id無視)`(src/var_store.rs:100)— Tellのスコープ意味論が未定 | law不在の症状 | Task 4でTellのlawを書く際に決着 |
| ④ | `VarId.owner_segment`: SPEC-VM-020:191「存在しないべき」vs 現役コード+SPEC-VM-019 Rev 2 | spec間矛盾 | どちらが正かspecを改訂して一本化 |
| ⑤ | SPEC-EFF-002のknown issue「snapshot isolation」(gh#157)vs owner回答「共有が確定」 | spec陳腐化 | spec更新issue(共有semanticsをlawとして明記) |
| ⑥ | `VarStore.global_state`/`writer_log`(src/var_store.rs:10,12)— get/put/modify/handler_state*/append_writer_log等の呼び出し元がブリッジ・vm-core実装にゼロ | 死設備(v1のVM特権State/Writerの残骸) | 削除issue(検証条件: 全文grep+両featureでcargo test+Python統合テスト) |
