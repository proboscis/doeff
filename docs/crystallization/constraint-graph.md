...
| # | 項目 | 状態 | 対応案 |
|---|---|---|---|
| ① | `PromptBoundary.types`(segment.rs:47)がランタイム探索で未参照 [agent読解・要追検証] | **issue化済み: [#464](https://github.com/proboscis/doeff/issues/464)**(検証先行+STOP条件付き) | ブリッジ層での使用有無を検証 → 未使用なら削除 |
| ② | `MaskSpec`(segment.rs:51-63)定義のみ、dispatch未参照 | **issue化済み: [#465](https://github.com/proboscis/doeff/issues/465)** | 死スキャフォールド削除(Mask拡張自体はSPEC-VM-020:214の通り将来可) |
