...
Install everything (including dev tools) with `uv sync --group dev`. Use `uv run pytest` for the full suite, `uv run pytest tests/test_cache.py::test_cache_eviction` to target a single scenario, and `uv run pyright` for static typing. Lint and format with `uv run ruff check` followed by `uv run ruff format` to keep imports and style aligned. Build distributable artifacts via `uv run python -m build` before publishing packages.
