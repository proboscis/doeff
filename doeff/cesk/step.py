...
def _find_handler_in_k(
    ...
    if debug:
        print(
            f"[_find_handler_in_k] k len={len(k)}, start_depth={start_depth}, k_types={[type(f).__name__ for f in k[:10]]}"
        )
    ...
    for i, frame in enumerate(k):
        if isinstance(frame, HandlerFrame):
            if handlers_to_skip > 0:
                ...
                if debug:
                    print(
                        f"[_find_handler_in_k] found target HandlerFrame at {i}, delimited_k len={len(delimited_k)}"
                    )
...
def _make_suspended_from_suspend_on(suspend_on: SuspendOn) -> Suspended:
    ...
    pending_io = suspend_on.pending_io
    ...
    if suspend_on.awaitable is None and pending_io:
        first_task_id = next(iter(pending_io))
        first_awaitable = pending_io[first_task_id]["awaitable"]
        effect: Any = FutureAwaitEffect(awaitable=first_awaitable)
    elif suspend_on.awaitable is not None:
        effect = FutureAwaitEffect(awaitable=suspend_on.awaitable)
    else:
        from doeff.cesk.errors import InterpreterInvariantError

        raise InterpreterInvariantError("SuspendOn with neither awaitable nor pending_io")
...
def step(state: CESKState) -> StepResult:
    ...
    if isinstance(C, Value) and K:
        ...
        if isinstance(frame, HandlerResultFrame):
            ...
            if debug:
                print(
                    f"[step] HandlerResultFrame.on_value returned: {type(result).__name__}, value_type={type(C.v).__name__}"
                )
            if isinstance(result, ContinueValue):
                if debug:
                    print(
                        f"[step] ContinueValue: k_len={len(result.k)}, value={str(result.value)[:50]}"
                    )
                return CESKState(C=Value(result.value), E=result.env, S=result.store, K=result.k)
            if isinstance(result, ContinueError):
                return CESKState(
            ...
            if isinstance(result, SuspendOn):
                if debug:
                    print(
                        f"[step] SuspendOn: awaitable={result.awaitable}, k_len={len(result.stored_k or [])}"
                    )
    ...
    if isinstance(C, Error) and K:
        ...
        if isinstance(frame, HandlerResultFrame):
            ...
            raise InterpreterInvariantError(
                f"Unexpected HandlerResultFrame error result: {type(result)}"
            )
