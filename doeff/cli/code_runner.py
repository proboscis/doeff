"""AST transformation to wrap inline code as a @do function.

Transforms top-level yield statements and last-expression-as-return
so that `doeff run -c 'yield Ask("key")'` works.
"""
...
def transform_doeff_code(source: str, filename: str = "<doeff-code>") -> TransformResult:
    tree = ast.parse(source, filename=filename, mode="exec")
    transformed_tree = _wrap_in_do_function(tree)
    code = compile(transformed_tree, filename, "exec", dont_inherit=True)
